...
integer,parameter,dimension(2,3,1,1) :: c = minloc(b,dim = 3, mask = b> 'd')

integer,dimension(2,3,1,1) :: d

integer,parameter::p = kind(minloc(b,dim =3, mask = b> 'd'))

a = reshape((/'a','b','c','d','e','f','g','h','i','j','k','l'/), (/2,3,2,1,1/))

d =  minloc(a,dim =3, mask = a> 'd')
...
call check(p, kind(minloc(a,dim = 3, mask = a> 'd')))
