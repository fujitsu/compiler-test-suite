...
logical*1,parameter::m = .true.
integer,parameter,dimension(3,2,1) :: c = maxloc(b,dim =1, mask = m)
integer,dimension(3,2,1) :: d
integer::p = kind(maxloc(b,dim =1, mask = m))
a = reshape((/0.0_16,-30.0_16,-100.0_16,-50.0_16,4.0_16,5.0_16,8.0_16,-11.0_16,6.0_16,-3.0_16,2.0_16,-400.0_16/), (/2,3,2,1/))
d =  maxloc(a,dim =1, mask = m)
do i = 1,3
do j = 1,2
do k = 1,1
...
enddo
enddo
call check(p, kind(maxloc(a,dim =1,mask = m)))
