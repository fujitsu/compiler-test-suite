...
integer,parameter,dimension(3,2,1) :: c = maxval(b,dim =1, mask = b> 500)

integer,dimension(3,2,1) :: d

integer,parameter::p = kind(maxval(b,dim =1, mask = b> 500))

a = reshape((/0,-30,-100,-50,4,5,8,-11,6,-3,2,-400/), (/2,3,2,1/))

d =  maxval(a,dim =1, mask = a> 500)
...
enddo
enddo
call check(p, kind(maxval(a,dim =1, mask = a> 500)))
end
