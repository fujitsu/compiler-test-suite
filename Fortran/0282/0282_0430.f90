...
real,parameter,dimension(2,2,1,1,1) :: c = product(b,dim = 2, mask = b> -30.0)


integer::p = kind(product(b,dim =1, mask = b> -30.0))
a = reshape((/0.0,-30.0,-100.0,-50.0,4.0,5.0,8.0,-11.0,6.0,-3.0,2.0,-400.0/), (/2,3,2,1,1,1/))

d =  product(a,dim =2, mask = a> -30.0)
...
call check(p, kind(product(a,dim =1, mask = a> -30.0)))
