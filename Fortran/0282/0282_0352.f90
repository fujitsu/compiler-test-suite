program main
 ...
 integer,parameter,dimension(2,2)::frontend = minval(b,dim=3)
 integer::n(2)
 integer,parameter::p = kind(minval(b,dim=3))
 integer,parameter::g(2) = shape(minval(b,dim=3))
 a = reshape((/2,3,4,6/),(/2,2,0/))
 backend = minval(a,dim=3)
 print*,frontend
 print *, backend
 n  = shape(minval(b,dim=3))
 do i = 1,2
 do j = 1,2
 ...
 call check(p, kind(minval(a,dim=3)))
 call check1(g,n)
end
