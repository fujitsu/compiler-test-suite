...
complex,parameter,dimension(2,3,1,1) :: c = sum(b,dim = 3, mask = b == maskvalue)


integer,parameter::p = kind(sum(b,dim =3, mask = b == maskvalue))

a = reshape((/(0.0,-30.0),(-100.0,-50.0),(4.0,5.0),(8.0,-11.0),(6.0,-3.0),(2.0,-400.0)/), (/2,3,1,1,1/))

d =  sum(a,dim =3, mask = a == maskvalue)
...
call check(p, kind(sum(a,dim = 3, mask = a == maskvalue )))
