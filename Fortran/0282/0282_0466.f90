...
real,parameter,dimension(3,2,1) :: c = sum(b,mask = b> -30.0,dim = 1)

integer::p = kind(sum(b,mask = b> -30.0,dim =1))

a = reshape((/0.0,-30.0,-100.0,-50.0,4.0,5.0,8.0,-11.0,6.0,-3.0,2.0,-400.0/), (/2,3,2,1/))

d = sum(a,mask = a> -30.0,dim = 1)
...
call check(p, kind(sum(a,mask = a> -30,dim =1)))
