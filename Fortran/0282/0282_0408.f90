...
integer,parameter,dimension(3,2,1) :: c = product(b,dim =1, mask = b> 500)

integer,dimension(3,2,1) :: d

integer::p = kind(product(b,dim =1, mask = b> 500))

a = reshape((/0,-30,-100,-50,4,5,8,-11,6,-3,2,-400/), (/2,3,2,1/))

d =  product(a,dim =1, mask = a> 500)
do i = 1,3
do j = 1,2
do k = 1,1
...
call check(p, kind(product(a,dim =1,mask = a > 500)))
