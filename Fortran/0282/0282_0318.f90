...
character,parameter,dimension(2,3,1,1) :: c = minval(b,dim = 3, mask = b> 'd')

character,dimension(2,3,1,1) :: d

integer,parameter::p = kind(minval(b,dim =3, mask = b> 'd'))

a = reshape((/'a','b','c','d','e','f','g','h','i','j','k','l'/), (/2,3,2,1,1/))

d =  minval(a,dim =3, mask = a> 'd')
...
call check(p, kind(minval(a,dim = 3, mask = a> 'd')))
