...
logical*8,parameter,dimension(2,3,2,1)::n = reshape(m,(/2,3,2,1/))


real*16,parameter,dimension(3,2,1) :: c = maxval(b,dim =1, mask = n)

real*16,dimension(3,2,1) :: d

integer::p = kind(maxval(b,dim =1, mask = n))

a = reshape((/0,-30,-100,-50,4,5,8,-11,6,-3,2,-400/), (/2,3,2,1/))

d =  maxval(a,dim =1, mask = n)
do i = 1,3
do j = 1,2
do k = 1,1
...
enddo
enddo
call check(p, kind(maxval(a,dim =1,mask = n)))
