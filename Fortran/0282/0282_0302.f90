...
integer,parameter,dimension(3,2,1) :: c = minloc(b,dim =1, mask = b> -30.0)

integer,parameter::p = kind(minloc(b,dim =1, mask = b> -30.0))

a = reshape((/0.0,-30.0,-100.0,-50.0,4.0,5.0,8.0,-11.0,6.0,-3.0,2.0,-400.0/), (/2,3,2,1/))

d = minloc(a,dim =1, mask = a> -30.0) 
...
call check(p, kind(minloc(a,dim =1, mask = a> -30.0)))
