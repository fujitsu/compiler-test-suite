...
real,parameter,dimension(2,2,1,1,1) :: c = maxval(b,dim = 2, mask = b> -30.0)

real,dimension(2,2,1,1,1) :: d

integer,parameter::p = kind(maxval(b,dim =2, mask = b> -30.0))

a = reshape((/0.0,-30.0,-100.0,-50.0,4.0,5.0,8.0,-11.0,6.0,-3.0,2.0,-400.0/), (/2,3,2,1,1,1/))

d =  maxval(a,dim =2, mask = a> -30.0)
...
call check(p, kind(maxval(a,dim =1, mask = a> -30.0)))
