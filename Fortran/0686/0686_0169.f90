      call sub()
      print *,'pass'
      contains
        subroutine sub()
        character(len=9),pointer,dimension(:,:)::p_string
        character(len=9),pointer,dimension(:,:)::pa_string
        character(len=9),target,dimension(1,3) ::string1='OPENMP V2'
        character(len=9),target,dimension(2,2)::string2= &
         reshape((/'openmp v2','openmp v3','openmp v4','openmp v5'/),(/2,2/))
        character(len=9),target,dimension(3,1) ::string3='OpenMP V2'
        p_string=>string1
        allocate(pa_string(5,5))
        pa_string='OPENMP v2'
  !$omp parallel private(p_string,pa_string)
        p_string=>string2
  !$omp single
        p_string=>string3
        allocate(pa_string(1,1))
        pa_string='OpenMP v2'
  !$omp end single copyprivate(p_string,pa_string)
        if (.not.associated(p_string)) print *,"fail"
        if (size( p_string ) /= 3 ) print *,"fail"
        if (any( p_string /= 'OpenMP V2' )) print *,"fail"
        p_string=>null()
        if (.not.associated(pa_string)) print *,"fail"
        if (size( pa_string ) /= 1 ) print *,"fail"
        if (any( pa_string /= 'OpenMP v2' )) print *,"fail"
  !$omp barrier
  !$omp single
        deallocate(pa_string)
  !$omp end single
  !$omp end parallel
        if (.not.associated(p_string)) print *,"fail"
        if (.not.associated(pa_string)) print *,"fail"
        if (any( p_string /= 'OPENMP V2' )) print *,"fail"
        if (any( pa_string /= 'OPENMP v2' )) print *,"fail"
        p_string=>null()
        deallocate(pa_string)
        end subroutine sub
      end
