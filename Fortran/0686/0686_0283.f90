...
!$omp threadprivate(p_i1,p_i2,p_i4,p_i8)
    ...
    program abc
     use mod
      call sub0()
      call sub(me)
      if (associated(p_i1)) print *,"fail"
      if (associated(p_i2)) print *,"fail"
      if (any(pack(p_i4,mask=.true.) /= (/100,101,102,103/))) print *,"fail"
...
!$omp end single copyprivate(p_i2,p_i4,p_i8)
      if (OMP_GET_THREAD_NUM()==me) then
