    program abc
     ...
     character(len=9),allocatable,dimension(:,:)::allo_string
      allocate(allo_string(2,2))
      allo_string(:,1)=(/'OPENMP v2','OPENMP v3'/)
      allo_string(:,2)=(/'OPENMP v2','OPENMP v3'/)
      call sub1(allo_string)
!$omp parallel private(allo_string)
      call sub2(allo_string)
!$omp end parallel
      if (.not.allocated(allo_string)) print *,"fail"
      if (size( allo_string ) /= 4 ) print *,"fail"
      if (any( allo_string(:,1) /= (/'OPENMP v2','OPENMP v3'/) )) print *,"fail"
      if (any( allo_string(:,2) /= (/'OPENMP v2','OPENMP v3'/) )) print *,"fail"
...
!$omp parallel private(allo_string)
      ...
      allocate(allo_string(5:5,1:1))
!$omp single
      allo_string(5,1)='OpenMP V2'
...
!$omp end parallel
    ...
    subroutine sub2(allo_string)
     character(len=9),allocatable,dimension(:,:)::allo_string
      if (allocated(allo_string)) deallocate(allo_string)
      allocate(allo_string(5:5,2:2))
!$omp single
      allo_string(5,2)='OpenMP V2'
!$omp end single copyprivate(allo_string)
      if (.not.allocated(allo_string)) print *,"fail"
      if (size( allo_string ) /= 1 ) print *,"fail"
      if (lbound( allo_string,1 ) /= 5 ) print *,"fail"
