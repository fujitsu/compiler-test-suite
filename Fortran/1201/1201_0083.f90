     ...
     subroutine s1
     use mod
     class(*   ), allocatable :: var,var2,var3
     allocate(ext::var,var2,var3)
     select type(var)
     type is(ext)
       allocate(character(m)::var%name)
       write (346,"('var     :',z16.16)") loc(var)
       write (347,"('var%name:',z16.16)") loc(var%name)
       write (348,"(z16.16)") loc(var)
       write (349,"(z16.16)") loc(var%name)
       !!!!!deallocate(var%name)
     end select
     select type(var2)
     type is(ext)
