     ...
     contains
      ...
      select type(p=>var(2))
       type is(e)
         ...
         if (same_type_as(p%ma,base_x)) print *,7451
         if (same_type_as(p%qa,base_x)) print *,7452
         if (same_type_as(p%mame,ext_x)) print *,7453
         k=1
       end select
       if (k/=1) print *,3628
       end subroutine s2
...
subroutine ss
     ...
     type is(ext)
       ...
       write(1401,"(z16.16)") loc(p%name)
       write(1402,"(z16.16)") loc(p%na)
       k=1
     end select
     if (k/=1) print *,1002
     k=0
     ...
     type is(ext)
       allocate(p  %name,source= a)
       allocate(p  %na,source= a)
       write(1403,"(z16.16)") loc(p  %name)
       write(1404,"(z16.16)") loc(p  %na)
       k=1
     end select
     if (k/=1) print *,1002
     end select
