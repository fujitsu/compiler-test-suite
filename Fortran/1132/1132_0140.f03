      PROGRAM CV5208
      DATA      N1/10/,N2/10/

      type STR
      INTEGER*4   IA1(10),IB1(10),IC1(10),IS1
      INTEGER*2   IHA1(10),IHB1(10),IHC1(10)
      LOGICAL*4   LA1(10),LB1(10),LC1(10)
      LOGICAL*1   LBA1(10),LBB1(10),LBC1(10)
      REAL        RA1(10),RB1(10),RC1(10),RS1
      REAL*8      DA1(10),DB1(10),DC1(10),DS1
      REAL*8      QA1(10),QB1(10),QC1(10)
      COMPLEX*8   CA1(10),CB1(10),CC1(10)
      COMPLEX*16  CDA1(10),CDB1(10),CDC1(10)
      COMPLEX*16  CQA1(10),CQB1(10),CQC1(10)
   end type STR
   TYPE(STR) STR_DATA

      INTEGER*4   IA2(10,10),IB2(10,10),IC2(10,10)
      INTEGER*2   IHA2(10,10),IHB2(10,10),IHC2(10,10)
      LOGICAL*4   LA2(10,10),LB2(10,10),LC2(10,10)
      LOGICAL*1   LBA2(10,10),LBB2(10,10),LBC2(10,10)
      REAL        RA2(10,10),RB2(10,10),RC2(10,10)
      REAL*8      DA2(10,10),DB2(10,10),DC2(10,10)
      REAL*8      QA2(10,10),QB2(10,10),QC2(10,10)
      COMPLEX*8   CA2(10,10),CB2(10,10),CC2(10,10)
      COMPLEX*16  CDA2(10,10),CDB2(10,10),CDC2(10,10)
      COMPLEX*16  CQA2(10,10),CQB2(10,10),CQC2(10,10)
      INTEGER*4   IA3(10,5,2),IB3(10,5,2),IC3(10,5,2)
      LOGICAL*4   LA3(10,5,2),LB3(10,5,2),LC3(10,5,2)
      REAL        RA3(10,5,2),RB3(10,5,2),RC3(10,5,2)
      REAL*8      DA3(10,5,2),DB3(10,5,2),DC3(10,5,2)
      COMPLEX*8   CA3(10,5,2),CB3(10,5,2),CC3(10,5,2)
      COMPLEX*16  CDA3(10,5,2),CDB3(10,5,2),CDC3(10,5,2)

      STR_DATA%RS1 = 0
      STR_DATA%IS1 = 0
      STR_DATA%DS1 = 0

      J = 3
      DO 1 I=1,N2
        STR_DATA%IA1(I) = 0
        STR_DATA%IB1(I) = I+J
        STR_DATA%IC1(I) = I-J
        STR_DATA%RA1(I) = 0
        STR_DATA%RB1(I) = I+J
        STR_DATA%RC1(I) = I-J
        STR_DATA%DA1(I) = 0
        STR_DATA%DB1(I) = I+J
        STR_DATA%DC1(I) = I-J
        STR_DATA%QA1(I) = 0
        STR_DATA%QB1(I) = I+J
        STR_DATA%QC1(I) = I-J
        STR_DATA%IHA1(I) = 0
        STR_DATA%IHB1(I) = I+J
        STR_DATA%IHC1(I) = I-J
        STR_DATA%LA1(I) = .FALSE.
        STR_DATA%LB1(I) = .TRUE.
        STR_DATA%LC1(I) = .FALSE.
        STR_DATA%LBA1(I) = .FALSE.
        STR_DATA%LBB1(I) = .TRUE.
        STR_DATA%LBC1(I) = .FALSE.
        STR_DATA%CA1(I) = 0
        STR_DATA%CB1(I) = I-1
        STR_DATA%CC1(I) = I+1
        STR_DATA%CDA1(I) = 0
        STR_DATA%CDB1(I) = I-1
        STR_DATA%CDC1(I) = I+1
        STR_DATA%CQA1(I) = 0
        STR_DATA%CQB1(I) = I-1
        STR_DATA%CQC1(I) = I+1
 1    CONTINUE

      DO 2 J=1,N1
        DO 2 I=1,N2
          IA2(I,J) = 0
          IB2(I,J) = I+J
          IC2(I,J) = I-J
          RA2(I,J) = 0
          RB2(I,J) = I+J
          RC2(I,J) = I-J
          DA2(I,J) = 0
          DB2(I,J) = I+J
          DC2(I,J) = I-J
          QA2(I,J) = 0
          QB2(I,J) = I+J
          QC2(I,J) = I-J
          IHA2(I,J) = 0
          IHB2(I,J) = I+J
          IHC2(I,J) = I-J
          LA2(I,J) = .FALSE.
          LB2(I,J) = .TRUE.
          LC2(I,J) = .FALSE.
          LBA2(I,J) = .FALSE.
          LBB2(I,J) = .TRUE.
          LBC2(I,J) = .FALSE.
          CA2(I,J) = 0
          CB2(I,J) = I-1
          CC2(I,J) = I+1
          CDA2(I,J) = 0
          CDB2(I,J) = I-1
          CDC2(I,J) = I+1
          CQA2(I,J) = 0
          CQB2(I,J) = I-1
          CQC2(I,J) = I+1
 2    CONTINUE

      DO 3 I=1,10
       DO 3 J=1,5
        DO 3 K=1,2
          IA3(I,J,K) = 0
          IB3(I,J,K) = I*J*K
          IC3(I,J,K) = I - J*K
          RA3(I,J,K) = I + J -K
          RB3(I,J,K) = I*J*K
          RC3(I,J,K) = I - J*K
          DA3(I,J,K) = I + J -K
          DB3(I,J,K) = I*J*K
          DC3(I,J,K) = I - J*K
          CA3(I,J,K) = I + J -K
          CB3(I,J,K) = I*J*K
          CC3(I,J,K) = I - J*K
          CDA3(I,J,K) = I + J -K
          CDB3(I,J,K) = I*J*K
          CDC3(I,J,K) = I - J*K
          LA3(I,J,K) = .FALSE.
          LB3(I,J,K) = 2.EQ.MOD(I,2)
          LC3(I,J,K) = .TRUE.
   3  CONTINUE

      WRITE(6,*) '***** CVCT5208 *****'

      WRITE(6,*) '----- TEST NO.1 ----- UPPER SAB    '
      DO 10 I=1,10
        IF(STR_DATA%LA1(I)) THEN
          STR_DATA%IS1 = STR_DATA%IS1 + STR_DATA%IA1(I)
        ELSE
          STR_DATA%RS1 = AMAX1(STR_DATA%RA1(I),STR_DATA%RS1 )
        ENDIF
        STR_DATA%RA1(I) = FLOAT(I)
        CONTINUE
        DO 10 J=1,2
          DO 11 K=1,2
            DA3(I,J,K) = DA3(I,J,K) + DB3(I,J,K)
            RA2(I,J) = SQRT(RB2(I,J)) + RA2(I,J) + STR_DATA%RA1(I)
            IF(LA2(I,J)) STR_DATA%DS1 = STR_DATA%DS1 + DB2(I,J)
   11     CONTINUE
          CA2(I,J) = STR_DATA%IA1(I) + STR_DATA%DA1(I)
          DB2(I,J) = DB2(I,J) * 2.0 + STR_DATA%DA1(I)
          RA2(I,J) = 0.0
          LC2(I,J) = DB2(I,J).GT.DC2(I,J)
   10   CONTINUE

      WRITE(6,*) 'IS1 = ',STR_DATA%IS1
      WRITE(6,*) 'RS1 = ',STR_DATA%RS1
      WRITE(6,*) 'RA1'
      WRITE(6,990) STR_DATA%RA1
      WRITE(6,*) 'DA3'
      WRITE(6,990) DA3
990   FORMAT(5(1X,F14.7))
      WRITE(6,*) 'RA2'
      WRITE(6,989) RA2
989   FORMAT(5(1X,D14.7))
      WRITE(6,*) 'DS1 = ',STR_DATA%DS1
      WRITE(6,*) 'CA2'
      WRITE(6,991) ((CA2(I,J),I=1,10),J=1,10)
      WRITE(6,*) 'DB2 = '
      WRITE(6,991) DB2
991   FORMAT(5F10.5)
      WRITE(6,*) 'LC2 = '
      WRITE(6,992) LC2
992   FORMAT(10L5)

      WRITE(6,*) '----- TEST NO.2 ----- LOWER SAB    '
      DO 20 I=1,10
        DO 21 J=1,5
          LC2(I,J) = DB2(I,J).NE.DC2(I,J)
          IF(LC2(I,J)) CA2(I,J) = CA2(I,J)*CB2(I,J)
          DO 22 K=1,2
            DA3(I,J,K) = DB3(I,J,K) + DB2(I,J)
            CA2(I,J) = CA3(I,J,K) - CB3(I,J,K)
            IF(LA2(I,J)) LB2(I,J) = .NOT.LC2(I,J)
   22     CONTINUE
          CA2(I,J) = STR_DATA%IA1(I) + STR_DATA%DA1(I)
          DB2(I,J) = DA2(I,J) * 2.0 + STR_DATA%DA1(I) + DFLOAT(STR_DATA%IA1(I))
          LC2(I,J) = DB2(I,J).EQ.DC2(I,J)
   21   CONTINUE
        IF(STR_DATA%LA1(I)) STR_DATA%DA1(I) = STR_DATA%DB1(I)*STR_DATA%DC1(I)
        STR_DATA%IA1(I) = - I
        STR_DATA%DA1(I) = DFLOAT(STR_DATA%IA1(I))
   20   CONTINUE

      WRITE(6,*) 'LC2 = '
      WRITE(6,993) LC2
993   FORMAT(10L5)
      WRITE(6,*) 'CA2'
      WRITE(6,994) ((CA2(I,J),I=1,10),J=1,10)
      WRITE(6,*) 'DA3 = '
      WRITE(6,994) DA3
994   FORMAT(5F10.5)
      WRITE(6,*) 'LB2 = '
      WRITE(6,995) LB2
995   FORMAT(10L5)
      WRITE(6,*) 'DB2 = '
      WRITE(6,994) DB2
      WRITE(6,*) 'DA1 = '
      WRITE(6,994) STR_DATA%DA1
      WRITE(6,*) 'IA1 = ',STR_DATA%IA1

      WRITE(6,*) '----- TEST NO.3 ----- UPPER/LOWER SAB '
      DO 30 I=1,10
        STR_DATA%DA1(I) =DFLOAT(I)
        STR_DATA%DS1 = STR_DATA%DS1 + STR_DATA%DA1(I)
        STR_DATA%RS1 = AMAX1(STR_DATA%RS1,STR_DATA%RA1(I))
        DO 31 J=1,10
          DA2(I,J) = RA2(I,J) + I
          RB2(I,J) = RC2(I,J)*3.0
          STR_DATA%IB1(I) = STR_DATA%IA1(I) + STR_DATA%IC1(J)
          STR_DATA%IS1 = STR_DATA%IS1 + STR_DATA%IB1(I)
          DO 31 K=1,2
            N = J
            IF(N.GT.5) N=N-5
            IF(DA3(I,N,K)-10.0D0) 32,33,34
   32        DB3(I,N,K) = DC3(I,N,K) - DA2(I,J)
             GO TO 31
   33        DC3(I,N,K) = RB2(I,J) + DFLOAT(STR_DATA%IB1(I))
             GO TO 31
   34        CONTINUE
             RA2(I,J) = SQRT(ABS(RB2(I,J)))
             IF(LA2(I,J)) LB2(I,J) = .NOT.LC2(I,J)
   31   CONTINUE
        IF(STR_DATA%LA1(I)) STR_DATA%DA1(I) = STR_DATA%DB1(I)*STR_DATA%DC1(I)
        STR_DATA%IA1(I) = - I
        STR_DATA%DA1(I) = DFLOAT(STR_DATA%IA1(I))
   30   CONTINUE

      WRITE(6,*) 'DA1'
      WRITE(6,996) STR_DATA%DA1
  996 FORMAT(5(1X,F14.7))
      WRITE(6,*) 'DS1 = ',STR_DATA%DS1
      WRITE(6,*) 'RS1 = ',STR_DATA%RS1
      WRITE(6,*) 'DA2 = '
      WRITE(6,994) DA2
      WRITE(6,*) 'RB2 = '
      WRITE(6,994) RB2
      WRITE(6,*) 'IB1 = ',STR_DATA%IB1
      WRITE(6,*) 'IS1 = ',STR_DATA%IS1
      WRITE(6,*) 'DB3 = '
      WRITE(6,994) DB3
      WRITE(6,*) 'DC3 = '
      WRITE(6,994) DC3
      WRITE(6,*) 'RA2'
      WRITE(6,997) RA2
  997 FORMAT(5(1X,D14.7))
      WRITE(6,*) 'LB2'
      WRITE(6,998) LB2
  998 FORMAT(20(1X,L2))
      WRITE(6,*) 'IA1 = ',STR_DATA%IA1

      STOP
      END
