  !             CVCT5909            LEVEL=1        DATE=84.02.04
  !***<< CVCT5909 >>***
  ! CHANGED DO INDEX AND VOLUME VECTRIZATION
  !
  !********************
  type st1
  ...
  DATA str%T4/100*.TRUE.,100*.FALSE.,56*.TRUE./
  DATA str%A6,str%B6/6400*0.0040D-2,6400*0.0008D-2/
              ...
              DO  K=1,4
                 DO  L=1,4
                    ...
                    IF(str%T4(I,J,K,L)) THEN
                       str%A4(I,J,K,L)=str%B4(I,J,K,L)/(0.001D-2+str%A6(I,J,K,L,N,M))
                    ELSE
                       str%B4(I,J,K,L)=str%B6(I,J,K,L,N,M)/str%A4(I,J,K,L)+0.001D-2
                    ENDIF
                    str%R4(I,J,K,L)=str%A4(I,J,K,L)-str%B4(I,J,K,L)
                    IF(str%R6(I,J,K,L,N,M).GT.str%B4(I,J,K,L)) THEN
                       IF(str%T4(I,J,K,L).AND.str%T6(I,J,K,L,N,M)) THEN
                          str%R4(I,J,K,L)=str%R4(I,J,K,L)/str%A4(I,J,K,L)
                       ELSE
                          str%R4(I,J,K,L)=str%R6(I,J,K,L,N,M)/str%A6(I,J,K,L,N,M)
                       ENDIF
                    ELSE
                       ...
                       ENDIF
                    ENDIF
                    TT=str%R4(I,J,K,L)/100.0D0
                    HH=str%R6(I,J,K,L,N,M)/100.0D0
                    SN=SIN(TT)
                    XSIN=SIN(HH)
                    CS=1.0D0-SN
                    XCOS=1.0D0-XSIN
                    str%IS4(I,J,K,L)=SN*100.0D0
                    str%IC4(I,J,K,L)=CS*100.0D0
                    str%IXS6(I,J,K,L,N,M)=XSIN*100.0D0
                    str%IXC6(I,J,K,L,N,M)=XCOS*100.0D0
                    IF(SN.GT.CS) THEN
