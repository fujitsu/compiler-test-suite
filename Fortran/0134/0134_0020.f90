...
integer,pointer:: a1(:)
character(*)::c1,c11
call ss( [ character(len= k + &
              kf( [ character(len=k+k) :: c1 ] )  +k ) :: & 
                c11 ] , a1)
end subroutine 
function kf( x1 )
character(*) ::x1(:)
if (len(x1)/=2)print *,1001,len(x1)
if (size(x1)/=1) print *,1002,size(x1)
if (x1(1)/='12')print *,2001,x1(1)
kf=2
end function 
subroutine ss(x1,a1)
character(*) ::x1(:)
