...
CLASS(EXT),ALLOCATABLE :: OBJ(:)
CLASS(EXT),ALLOCATABLE :: OBJ2(:)
...
ALLOCATE(EXT::OBJ2(1)%PREV_NODE(2)%NEXT_NODE(2)%PREV_NODE(4))
...
CALL SUB(OBJ, [EXT([OBJ2(2)%VALUE], [OBJ2(2)%NEXT_NODE], [OBJ2(1)%PREV_NODE], [10, 20, 30])])

CONTAINS
SUBROUTINE SUB (DMY1, DMY2)
CLASS(EXT),ALLOCATABLE :: DMY1(:)
TYPE(EXT) :: DMY2(:)

DMY1 = DMY2

if (DMY1(1)%VALUE(2) /= 200) print*, 912
...
SELECT TYPE(X=>DMY1(1)%NEXT_NODE(3)%PREV_NODE)
 TYPE IS(EXT)
 CLASS DEFAULT
  PRINT*,911
END SELECT

SELECT TYPE(X=>DMY1(1)%PREV_NODE(2)%NEXT_NODE(2)%PREV_NODE)
 TYPE IS(EXT)
 CLASS DEFAULT
  PRINT*,912
END SELECT

SELECT TYPE(X=>DMY1(1)%NEXT_NODE(3)%PREV_NODE(3)%NEXT_NODE)
 TYPE IS(NODE)
 CLASS DEFAULT
  PRINT*,913
END SELECT
