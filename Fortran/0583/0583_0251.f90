...
if (.NOT.EXTENDS_TYPE_OF(BASE%OBJ(2)%NEXT_NODE(3)%PREV_NODE, BASE%OBJ(1)%PREV_NODE(2)%NEXT_NODE(2)%PREV_NODE)) print*, 911
if (.NOT.SAME_TYPE_AS(BASE%OBJ(1)%PREV_NODE(2)%NEXT_NODE(2)%PREV_NODE, BASE%OBJ(2)%NEXT_NODE)) print*, 912
...
SELECT TYPE(X=>BASE%OBJ(2)%NEXT_NODE(3)%PREV_NODE(3)%NEXT_NODE)
 TYPE IS(NODE)
  PRINT*, "PASS"
 CLASS DEFAULT
  PRINT*,913
