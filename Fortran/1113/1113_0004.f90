...
character(10),target::ctrg21= "0000000002"
...
character(10),target::ctar21= "0000000003"
        interface test
                ...
                module procedure cfun
                module procedure lfun
        end interface
...
inquire (unit=1,ACCESS =  test(ctrg),&
         ACTION =  test(ctrg1),ASYNCHRONOUS =  test(ctrg2),&
         BLANK =   test(ctrg3),DECIMAL =  test(ctrg4),&
         DELIM =  test(ctrg5),DIRECT =  test(ctrg6),&
         ENCODING =  test(ctrg7),EXIST =  test(ltrg),&
         FORM =  test(ctrg8),FORMATTED =  test(ctrg9),IOMSG= test(ctrg20),&
         IOSTAT =  test(trg),&
         NAME =  test(ctrg10),NAMED =  test(ltrg1),NEXTREC =  test(trg1),NUMBER =  test(trg2),&
         OPENED = test(ltrg2),&
         PAD = test(ctrg11),PENDING =  test(ltrg3),POS =  test(trg3),&
         POSITION =  test(ctrg12),READ =  test(ctrg13),&
         READWRITE =  test(ctrg14),RECL =  test(trg4),ROUND =  test(ctrg15),&
         SEQUENTIAL =   test(ctrg16), SIGN =   test(ctrg21),SIZE =  test(trg5),&
         STREAM =  test(ctrg17),UNFORMATTED =  test(ctrg18),&
         WRITE =   test(ctrg19))

inquire (IOLENGTH =  test(trg6))aa1
...
if (ctrg21 /= ctar21) print *,127,ctrg21
