...
subroutine test(A,B,C,N)
  implicit none
  integer(kind=8),dimension(M)::A,B,C
  integer(kind=8)::tmp
  integer::i, N
...
subroutine init(A,B,C,res,N)
  implicit none
  integer::I,N
  integer(kind=8)::A(M),B(M),C(M),res(M)
  integer(kind=8)::tmp, tmp2

  tmp=-1
  tmp2 = tmp
!ocl nosimd  
  DO I=1,N
     A(I) = MOD(I*2,130)*INT(SIGN(1.0,SIN(I*2.21)))
     B(I) = tmp2
     C(I) = MOD(-I*3,200)*INT(SIGN(1.0,COS(I*0.2)))
     res(I) = C(I)
     tmp2 = tmp2 * tmp
  enddo
