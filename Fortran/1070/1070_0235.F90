...
subroutine init(b,c)
  USE_SIMFUNC
  real(kind=4),dimension(1:N) :: b,c
#ifdef ILFUNC_TEST
  real(kind=4),parameter :: max=3.402823E+38
#else
  real(kind=4),parameter :: max=3.402823E+38/4._4
#endif
  real(kind=4) :: val, xtmp, ytmp
  val = max/(N-1)
  xtmp = 0.000001_4
  ytmp = -max
  !ocl nosimd
...
subroutine check(res, inx, iny, fs)
  USE_SIMFUNC
  real(kind=4) :: res, inx, iny, fs, fsres
  external :: fs
  fsres = fs(inx, iny)
