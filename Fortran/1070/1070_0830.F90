...
subroutine test(A,B,C,N)
  implicit none
  integer(kind=1),dimension(M)::A,B,C
  integer(kind=1)::tmp
  integer::i, N
...
subroutine init(A,B,C,res,N)
  implicit none
  integer::I,N
  integer(kind=1)::A(M),B(M),C(M),res(M)
  integer(kind=1)::tmp, tmp2

  tmp=-1
  tmp2 = tmp
!ocl nosimd  
  DO I=1,N
     A(I) = MOD(I,101)*INT(SIGN(1.0,SIN(I*2.2)))
     B(I) = tmp2
     C(I) = MOD(I,23)*INT(SIGN(1.0,COS(I*1.2)))
     res(I) = C(I)
     tmp2 = tmp2 * tmp
  enddo
