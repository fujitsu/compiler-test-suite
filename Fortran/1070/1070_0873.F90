...
subroutine target(dest1, src1, src2)
  real(kind=8),dimension(1:N)::dest1, src1, src2
  integer::i
!ocl nosimd
  do i=1,N, 4
     dest1(i)   = src1(i)   * src2(i)
     dest1(i+1) = src1(i+1) * src2(i+1)
     dest1(i+2) = src1(i+2) * src2(i+2)
     dest1(i+3) = src1(i+3) * src2(i+3)
  enddo
...
subroutine init(src1, src2)
  ...
  do i=1,N
     src1(i) = sin(i*1.1)
     src2(i) = cos(i*0.2) + src1(i)
  end do
end subroutine init

subroutine check(dest1, dest1_res)
  USE_SIMFUNC
  real(kind=8),dimension(1:N)::dest1, dest1_res
  integer::i
  interface
  ...
  !ocl loop_nofission
  do i=1,N
     if ( equal_d(dest1(i), dest1_res(i)) .neqv. .true.)then
        PRINT_NG
     endif
  end do
  PRINT_OK
...
program main
  USE_SIMFUNC
  real(kind=8),dimension(1:N)::dest1, src1, src2
  real(kind=8),dimension(1:N)::dest1_res, src1_res, src2_res
  call init(src1, src2)
