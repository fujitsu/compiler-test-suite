...
forall (n2=2:k3,n4=2:i2,n5=i2:2)
  w(i2,i2,i2)%z(n4)%a(:,:, :, n2,:,:,: )%v(1)=w(i2,i2,i2)%z(n4)%a(:,:  , : , n2,:,:,: )%v(1)+dble(n2)
  w(i2,i2,i2)%z(n4)%a(:,:, :, n2,:,:,: )%v(2)=w(i2,i2,i2)%z(n4)%a(:,:  , : , n2,:,:,: )%v(2)+dble(n2)
end forall
end subroutine
...
do      n4=2,i2
do      n2=2,k3
  c(:,: ,: , n2,:,:,:)=w(i2,i2,i2)%z(n4)%b(:,:,:, n2,:,:,:  )%v(1)+dble(n2)
  d(:,: ,: , n2,:,:,:)=w(i2,i2,i2)%z(n4)%b(:,:,:, n2,:,:,:  )%v(2)+dble(n2)
end do
end do
end do
do      n5=i2,2
do      n4=2,i2
do      n2=2,k3
  w(i2,i2,i2)%z(n4)%b(:,: , :, n2,:,:,:)%v(1)=c(:,:, : , n2,:,:,: )
  w(i2,i2,i2)%z(n4)%b(:,: , :, n2,:,:,:)%v(2)=d(:,:, : , n2,:,:,: )
end do
end do
end do
end subroutine
