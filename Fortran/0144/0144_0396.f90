 module m1
  ...
  type x
   ...
   integer(8),dimension(2,1,1,2)::e1=reshape((/11,-1,21,1,31,0/),(/2,1,1,2/))
   integer(8),dimension(2,1,1,2)::e2=reshape((/11,-1,21,1,31,0/),(/2,1,1,2/))
   integer(8),dimension(2,1,1,2)::e3=reshape((/11,-1,21,1,31,0/),(/2,1,1,2/))
   integer(8),dimension(2,1,1,2)::e4=reshape((/11,-1,21,1,31,0/),(/2,1,1,2/))
   integer(8),dimension(2,1,1,2)::f1=reshape((/11,-1,21,1,31,0/),(/2,1,1,2/))
   integer(8),dimension(2,1,1,2)::f2=reshape((/11,-1,21,1,31,0/),(/2,1,1,2/))
   integer(8),dimension(2,1,1,2)::f3=reshape((/11,-1,21,1,31,0/),(/2,1,1,2/))
   integer(8),dimension(2,1,1,2)::f4=reshape((/11,-1,21,1,31,0/),(/2,1,1,2/))
  end type
...
use m1
 type(x)::z
   ...
   forall(m=j1:j2,&
     ...
     z%e1(i+1:1+i,j:j,k:k,m:m)=z%e1(i:i,j:j,k:k,m:m)
     z%e2(i+1:1+i,j:j,k:k,m:m)=z%e2(i:i,j:j,k:k,m:m)
     z%e3(i+1:1+i,j:j,k:k,m:m)=z%e3(i:i,j:j,k:k,m:m)
     z%e4(i+1:1+i,j:j,k:k,m:m)=z%e4(i:i,j:j,k:k,m:m)
     ...
     z%f1(i+1:1+i,j:j,k:k,m:m)=z%f1(i:i,j:j,k:k,m:m)
     z%f2(i+1:1+i,j:j,k:k,m:m)=z%f2(i:i,j:j,k:k,m:m)
     z%f3(i+1:1+i,j:j,k:k,m:m)=z%f3(i:i,j:j,k:k,m:m)
     z%f4(i+1:1+i,j:j,k:k,m:m)=z%f4(i:i,j:j,k:k,m:m)
     end forall
     end forall
     end forall
   end forall
   if (any((/z%a1/)/=(/11,11,21,21/)))print *,'fail'
   if (any((/z%a2/)/=(/11,11,21,21/)))print *,'fail'
   if (any((/z%a3/)/=(/11,11,21,21/)))print *,'fail'
   if (any((/z%a4/)/=(/11,11,21,21/)))print *,'fail'
   if (any((/z%b1/)/=(/11,11,21,21,31,31,41,41/)))print *,'fail'
   if (any((/z%b2/)/=(/11,11,21,21,31,31,41,41/)))print *,'fail'
   if (any((/z%b3/)/=(/11,11,21,21,31,31,41,41/)))print *,'fail'
   if (any((/z%b4/)/=(/11,11,21,21,31,31,41,41/)))print *,'fail'
   if (any((/z%e1/)/=(/11,11,21,21/)))print *,'fail'
   if (any((/z%e2/)/=(/11,11,21,21/)))print *,'fail'
   if (any((/z%e3/)/=(/11,11,21,21/)))print *,'fail'
   if (any((/z%e4/)/=(/11,11,21,21/)))print *,'fail'
   if (any((/z%f1/)/=(/11,11,21,21/)))print *,'fail'
   if (any((/z%f2/)/=(/11,11,21,21/)))print *,'fail'
   if (any((/z%f3/)/=(/11,11,21,21/)))print *,'fail'
   if (any((/z%f4/)/=(/11,11,21,21/)))print *,'fail'
end
subroutine ss2
use m1
 type(x)::z
   ...
   if (any(.not.z%d1))print *,'fail'
   if (any(.not.z%d2))print *,'fail'
   if (any(.not.z%d3))print *,'fail'
   if (any(.not.z%d4))print *,'fail'
   if (any(.not.z%c1))print *,'fail'
   if (any(.not.z%c2))print *,'fail'
   if (any(.not.z%c3))print *,'fail'
   if (any(.not.z%c4))print *,'fail'
end
