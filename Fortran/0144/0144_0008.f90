...
forall (n2=2:k3)
  w(i2,i2,i2)%z(i2)%a(:,:, :, n2,:,:,: )%v(1_8)=w(i2,i2,i2)%z(i2)%a(:,:  , : , n2,:,:,: )%v(1_8)+dble(n2)
  w(i2,i2,i2)%z(i2)%a(:,:, :, n2,:,:,: )%v(2_8)=w(i2,i2,i2)%z(i2)%a(:,:  , : , n2,:,:,: )%v(2_8)+dble(n2)
end forall
end subroutine
...
do      n2=2,k3
  c(:,: ,: , n2,:,:,:)=w(i2,i2,i2)%z(i2)%b(:,:,:, n2,:,:,:  )%v(1_8)+dble(n2)
  d(:,: ,: , n2,:,:,:)=w(i2,i2,i2)%z(i2)%b(:,:,:, n2,:,:,:  )%v(2_8)+dble(n2)
end do
do      n2=2,k3
  w(i2,i2,i2)%z(i2)%b(:,: , :, n2,:,:,:)%v(1_8)=c(:,:, : , n2,:,:,: )
  w(i2,i2,i2)%z(i2)%b(:,: , :, n2,:,:,:)%v(2_8)=d(:,:, : , n2,:,:,: )
end do
end subroutine
end
