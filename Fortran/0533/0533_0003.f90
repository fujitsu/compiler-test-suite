subroutine test1()
   type :: cap_parameters
    ...
    allocate(ptr_array(5),source=cap_parameters( 1,root_set_services,2))
if (ptr_array(2)%i1.ne.1) print *,'err'
if (ptr_array(2)%i2.ne.2) print *,'err'
    call ptr_array(2)%set_services
contains
...
subroutine test2()
  ...
  type :: cap_parameters_wrapper
   ...
   allocate(wrapper%ptr(2),source=cap_parameters( root_set_services_2,3))
if (wrapper%ptr(1)%i3.ne.3) print *,'err'
  call wrapper%ptr(1)%set_services
contains
  subroutine set_services_interface
  end subroutine set_services_interface
end
