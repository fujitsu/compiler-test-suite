...
allocate(character(len=6)::objTT%obj1(2,2)%msg(2,3))
allocate(character(len=6)::objTT%obj1(2,3)%msg(2,2))

objTT%obj1(2,3)%alc = 8
objTT%obj1(1,2)%alc2 = 9
objTT%obj1(2,3)%alc2 = 10
objTT%obj1(2,2)%msg = "ABCDEF"
objTT%obj1(2,3)%msg = "HELLOO"

allocate(objTS%obj2(1,2)%alc(2))
allocate(objTS%obj2(2,2)%alc(2))
allocate(objTS%obj2(1,2)%alc2)
allocate(objTS%obj2(2,2)%alc2)
allocate(character(len=6)::objTS%obj2(2,2)%msg(3,2))
...
if(LEN(objTT%obj1(2,2)%msg(2,2)) /= LEN(objTS%obj2(2,2)%msg(3,2))) print*, 102
if(objTT%obj1(1,2)%alc(1) /= 4) print*,103
if(objTT%obj1(2,2)%alc(2) /= 5) print*,104
