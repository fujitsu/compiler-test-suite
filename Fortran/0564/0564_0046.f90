...
objTT(2)%obj2 = TRANSPOSE(objTT(2)%obj1)
...
IF(objTT(2)%obj2(1,1)%jj .EQ. 20 .AND. objTT(2)%obj2(2,1)%jj .EQ. 10 .AND. objTT(2)%obj2(1,3)%jj .EQ. 40 .AND. objTT(2)%obj2(1,2)%jj .EQ. 10 .AND. objTT(2)%obj2(2,2)%jj .EQ. 30) THEN
PRINT*,"PASS"
ELSE
  PRINT*,"ERROR"
END IF
