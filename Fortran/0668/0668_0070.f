   ...
   11 continue
      do 30 i = 1,100
        do 40 j = 1,100
          if(mask(j,i)) then
            b(j,i) = atan2(a(j,i),aa(j,i))
          endif
   40   continue
   30 continue
      call sub1(a,aa,c,mask)
      fl = 0
      do 50 i = 1,100
        do 60 j = 1,100
          if( mask(j,i) ) then
            ans(j,i) = b(j,i) - c(j,i)
            if(ans(j,i).ne.0.0e0) then
  ...
  200 format(1h ,'=== test ok ===')
      ...
      subroutine sub1(a1,a11,c1,mask)
      real*4    a1(100,100),a11(100,100),c1(100,100)
      logical*4 mask(100,100)
      do 10 i = 1,100
        do 20 j = 1,100
          if(mask(j,i)) then
            c1(j,i) = atan2(a1(j,i),a11(j,i))
          endif
   20   continue
   10 continue
      return
