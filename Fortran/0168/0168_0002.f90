...
call sx02( iparity( [3 , 7],mask=mf([.true._8,.true._8])) ,12)
call sx02( iparity( a      ,mask=mf(a==a ) ),13)
call sx02( iparity( b      ,mask=mf(m) ) ,14)
call sx01( iparity( b,dim=1,mask=mf(m) ),[  4,2,1], 15)
call sx01( iparity( b,dim=2,mask=mf(m) ),[2,  2,1], 16)
call sx01( iparity( b,dim=3,mask=mf(m) ),[2,4,  1], 17)
call sx01( iparity( b,dim=4,mask=mf(m) ),[2,4,2  ], 18)
call sx01( iparity( dim=1,array=b,mask=mf(m) ),[  4,2,1], 19)
call sx01( iparity( dim=2,array=b,mask=mf(m) ),[2,  2,1], 20)
call sx01( iparity( dim=3,array=b,mask=mf(m) ),[2,4,  1], 21)
call sx01( iparity( dim=4,array=b,mask=mf(m) ),[2,4,2  ], 22)
contains
subroutine sx(z)
integer::z(:)
