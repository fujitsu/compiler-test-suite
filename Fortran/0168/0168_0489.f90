...
call sx01( iany( f( b),dim=k1,mask=m ),[  4,2,1], 15)
call sx01( iany( f( b),dim=k2,mask=m ),[2,  2,1], 16)
call sx01( iany( f( b),dim=k3,mask=m ),[2,4,  1], 17)
call sx01( iany( f( b),dim=k4,mask=m ),[2,4,2  ], 18)
call sx01( iany( dim=k1,array=f(b),mask=m ),[  4,2,1], 19)
call sx01( iany( dim=k2,array=f(b),mask=m ),[2,  2,1], 20)
call sx01( iany( dim=k3,array=f(b),mask=m ),[2,4,  1], 21)
call sx01( iany( dim=k4,array=f(b),mask=m ),[2,4,2  ], 22)
end
call s1
print *,'pass'
end
