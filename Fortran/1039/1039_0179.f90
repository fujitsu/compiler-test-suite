...
subroutine foo(a,idum,k,aa,jdum,aaa,kdum)
  ...
  if (any(kdum/=(/1,2,3/)+m))print *,'error-13'
end subroutine foo
...
nn=2
 ...
 write(28,*) 2
  ...
  call foo3(p,max(grid(2)%g1(2)%g2(2)%idum(n),idum1(v),idum2(v)),3,p,max(idum1,grid(2)%g1(2)%g2(2)%idum(n),idum2),p,max(grid(nn+n-1)%g1(2)%g2(nn)%idum(n),idum1(v),idum2(v))) 
