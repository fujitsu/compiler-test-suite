...
subroutine init
  ...
  p(:,:,:) = 0.0d0
  write(24,'(a,"0x",z16.16)') 'init: loc(p) = ', loc(p)
  grid%idum=>t1(::2)
   grid%idum=(/21,22,23/)
  return
end subroutine init
...
subroutine foo(a,idum,k,aa,jdum)
  ...
  write(24,'(a,"0x",z16.16,1x,"0x",z16.16)') ' foo: loc(a) = ', loc(a),loc(aa)
  select case(k)
  case(1)
    m=0
  case(2)
...
subroutine bar(a,b,aa,bb)
  ...
  write(24,'(a,"0x",z16.16)') ' bar: loc(a) = ', loc(a)
  write(24,'(a,"0x",z16.16)') ' bar: loc(aa) = ', loc(aa)
  return
end subroutine bar
...
program main
  ...
  call foo(p,max(idum1,idum2,grid%idum),3,p,max(idum1,idum2,grid%idum))
  call foo(p,max(grid%idum,idum2,idum1),3,p,max(grid%idum,idum2,idum1))
  call foo(p,max(grid%idum(n:3),idum2,idum1),3,p,max(idum1,grid%idum(n:3),idum2))  
  call foo(p,max(grid%idum(n:3:n),idum2(:),idum1),3,p,max(idum1,grid%idum(n:3:n),idum2(:)))  
  call foo3(p,max(grid%idum(n),idum1,idum2),3,p,max(idum1,grid%idum(n),idum2)) 
