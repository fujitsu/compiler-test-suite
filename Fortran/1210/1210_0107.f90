...
subroutine s1(w)
  ...
  class(e)::w(:)
    w%z1=11 
    allocate(w(1)%e1(2),source=c1) 
    allocate(w(2)%e1(2),source=c1) 
   ...
   select type(p=>name)
    ...
    write(2001,'(z16.16)') loc(p(1)   %e1)
      if (any(p(1)     %e1/=c1) ) print *,2829
      p(1)     %e1=c2 
      if (any(p(2)     %e1/=c1) ) print *,2829
      p(2)     %e1=c2 
      k=1
   end select
   if (k/=1) print *,1228
       if (w(1)%z1/=11) print *,3828
       if (any(w(1)%e1/=c1) ) print *,3829
       deallocate(w(1)%e1 )
       if (w(2)%z1/=11) print *,3828
       if (any(w(2)%e1/=c1) ) print *,3829
       deallocate(w(2)%e1 )
end
end
