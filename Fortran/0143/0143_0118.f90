...
forall (n2=2:k3)
  w(i2,i2,i2)%z(2)%a(:,:, :, n2,:,:,: )%v=w(i2,i2,i2)%z(1)%a(:,:  , : , n2-1,:,:,: )%v+dble(n2)
end forall
end subroutine
...
do      n2=2,k3
  c(:,: ,: , n2,:,:,:)=w(i2,i2,i2)%z(1)%b(:,:,:, n2-1,:,:,:  )%v+dble(n2)
end do
do      n2=2,k3
  w(i2,i2,i2)%z(i2)%b(:,: , :, n2,:,:,:)%v=c(:,:, : , n2,:,:,: )
end do
end subroutine
end
...
allocate( w(i2,i2,i2)%z(1)%a(k1,k1,k2,k3,k1,k1,k1))
allocate( w(i2,i2,i2)%z(1)%b(k1,k1,k2,k3,k1,k1,k1))
w(i2,i2,i2)%z(i2)%a%v=reshape([(n,n=1,20*4*4*4*4*4)],[4,4,4,4,4,4,4])
w(i2,i2,i2)%z(i2)%b%v=reshape([(n,n=1,20*4*4*4*4*4)],[4,4,4,4,4,4,4])
w(i2,i2,i2)%z(1)%a%v=reshape([(n,n=1,20*4*4*4*4*4)],[4,4,4,4,4,4,4])
w(i2,i2,i2)%z(1)%b%v=reshape([(n,n=1,20*4*4*4*4*4)],[4,4,4,4,4,4,4])
call s1(k1,k2,k3)
call s2(k1,k2,k3)
