...
i2fun(k)=k+2-k-2+jfun(k-1)+nf(k,(/(kk,kk=1,5)/))
ifun(k)=i2fun(jfun(k+1))+i2fun(jfun(k)+1)-jfun(i2fun(k+1))
call ss(p,ifun(i+1),ifun((i*2)+2-i))
end subroutine
end
use m
...
if (k<1) then
 ...
 if (kk(1)/=1)print *,'error-y1'
 if (kk(k)/=k)print *,'error-yk'
endif
nf=0
end
