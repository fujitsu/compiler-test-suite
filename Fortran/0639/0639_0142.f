     ...
     5         S1(50),T1(50),U1(50)
       REAL*8  AA(50),BB(50),CC(50),DD(50),EE(50),FF(50),GG(50),
     1         HH(50),KK(50),LL(50),MM(50),NN(50),OO(50),PP(50),
     2         QQ(50),RR(50),SS(50),TT(50),UU(50),
     3         A2(50),B2(50),C2(50),D2(50),E2(50),F2(50),G2(50),
     4         H2(50),K2(50),L2(50),M2(50),N2(50),O2(50),P2(50),
     5         Q2(50),R2(50),S2(50),T2(50),U2(50)
       ...
       DO 20 I = 1 , 50
         A(I)=(V/CT(1)*B(I))+AY(5)+(W/CT(2)*C(I))+CT(5)
         D(I)=(AY(4)/CT(3)*E(I))+X+(AY(3)/CT(2)*F(I))+Y
         G(I)=(CT(5)/CT(5)*H(I))+V+(CT(3)/CT(4)*K(I))+W
         L(I)=(X/CT(1)*M(I))+AY(3)+(Y/CT(3)*N(I))+AY(1)
         O(I)=(AY(3)/CT(4)*P(I))+CT(1)+(AY(1)/CT(4)*Q(I))+Z
         R(I)=(Z/CT(2)*S(I))+AY(2)+(X/CT(1)*T(I))+AY(1)
         U(I)=(V/CT(2)*A(I))+AY(5)+(W/CT(4)*D(I))+CT(3)
         A1(I)=(V/CT(1)*C1(I))+AY(5)+(W/CT(2)*B1(I))+CT(5)
         D1(I)=(AY(4)/CT(3)*F1(I))+X+(AY(3)/CT(2)*E1(I))+Y
         G1(I)=(CT(5)/CT(5)*K1(I))+V+(CT(3)/CT(4)*H1(I))+W
         L1(I)=(X/CT(1)*N1(I))+AY(3)+(Y/CT(3)*M1(I))+AY(1)
         O1(I)=(AY(3)/CT(4)*Q1(I))+CT(1)+(AY(1)/CT(4)*P1(I))+Z
         R1(I)=(Z/CT(2)*T1(I))+AY(2)+(X/CT(1)*S1(I))+AY(1)
         U1(I)=(V/CT(2)*A1(I))+AY(5)+(W/CT(4)*D1(I))+CT(3)

         DO 10 J = 1 , 50
           AA(J)=AY(5)*(X/CT(5)*BB(J))*AY(4)*(Y/CT(4)*CC(J))
           DD(J)=V*(AY(3)/CT(3)*EE(J))*W*(AY(2)/CT(2)*FF(I))
           GG(J)=AY(5)/(X/CT(1)*HH(J))/AY(4)/(Y/CT(2)*KK(J))
           LL(J)=V/(AY(3)/CT(3)*MM(J))/W/(AY(2)/CT(4)*NN(J))
           OO(J)=AY(1)*(Z/CT(5)*PP(J))*AY(2)*(W/CT(4)*QQ(J))
           RR(J)=Z*(Y/CT(3)*SS(J))*V*(W/CT(4)*TT(J))
           UU(J)=Z/(AY(3)/CT(3)*AA(J))*V*(AY(4)/CT(2)*DD(J))
           A2(J)=AY(5)*(X/CT(5)*B2(J))*AY(4)*(Y/CT(4)*C2(J))
           D2(J)=V*(AY(3)/CT(3)*E2(J))*W*(AY(2)/CT(2)*F2(I))
           G2(J)=AY(5)/(X/CT(1)*H2(J))/AY(4)/(Y/CT(2)*K2(J))
           L2(J)=V/(AY(3)/CT(3)*M2(J))/W/(AY(2)/CT(4)*N2(J))
           O2(J)=AY(1)*(Z/CT(5)*P2(J))*AY(2)*(W/CT(4)*Q2(J))
           R2(J)=Z*(Y/CT(3)*S2(J))*V*(W/CT(4)*T2(J))
           U2(J)=Z/(AY(3)/CT(3)*A2(J))*V*(AY(4)/CT(2)*D2(J))
 10      CONTINUE
 20    CONTINUE

       WRITE(6,100) A , D , G , L
       WRITE(6,200) O , R , U
       WRITE(6,100) A1 , D1 , G1 , L1
       WRITE(6,200) O1 , R1 , U1
       WRITE(6,100) AA , DD , GG , LL
       WRITE(6,200) OO , RR , UU
       WRITE(6,100) A2 , D2 , G2 , L2
       WRITE(6,200) O2 , R2 , U2
