      DOUBLE PRECISION RPT,APT,RPT1,RPT2,APT1,APT2
 ...
 1000 ITEM = ITEM+1
  ...
  100 FORMAT(1H1/1H0,5X,36H* TEST OF EXPRESSION ( COMPLEX*16 = ,
     *                  38HCOMPLEX*16 , COMPLEX*16 ) ERROR LIST *//1H0,
     *             25X,15HCONSTANT VALUE.,27X,13HRESULT VALUE./1H0,
     *              5X,7HITEM.NO,2(11X,9HREAL PART,6X,
     *                  14HIMAGINARY PART))
  101 FORMAT(1H1//1H0,33HERROR IS NOT DETECTED BY TEST0170)
  102 FORMAT(1H0,5X,37H*** ERROR IS DETECTED BY TEST0170 ***)
  103 FORMAT(1H0,7X,I5,4(D20.10))
   ...
   45 CANS = ((C3*C3)**(IG-2))**(-IG+6)
      RPT  =  0.64D 02
      APT  =  0.0D 00
 1100 RPT1 = RPT-DABS(RPT*0.1D-05)
      RPT2 = RPT+DABS(RPT*0.1D-05)
      APT1 = APT-DABS(APT*0.1D-05)
      APT2 = APT+DABS(APT*0.1D-05)
      IF (DREAL(CANS).GE.RPT1.AND.DREAL(CANS).LE.RPT2.AND.
     *    DIMAG(CANS).GE.APT1.AND.DIMAG(CANS).LE.APT2) GO TO 1000
