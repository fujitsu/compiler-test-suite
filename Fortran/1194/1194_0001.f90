...
!         end select
   ...
   select type(fm)
    type is(ext)
     if(fm(2)%b1/=1)print *,9001
     if(fm(2)%e1/=2)print *,9002
     if(fm(2)%e2%x1/=3)print *,9003
     if(fm(2)%e2%x2/=4)print *,9004
     if(fm(2)%e3%x1/=5)print *,9005
     select type(p=>fm(2)%e3)
       type is(x_ext)
...
use m1
  ...
  type (ext),allocatable,target ::fm2(:)
call s1(fm)
allocate(fm2(2))
call s1(fm2)
print *,'sngg413p : pass'
