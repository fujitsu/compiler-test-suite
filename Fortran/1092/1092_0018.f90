...
associate( k=> n)
  block
   character(k-1),dimension(p+int(k)) ::ary,cry(2+k)*(k),bry(2+2)*(k),ary2,bry2(2+2)*(k),cry2(2+k)*(k)
...
associate( k=> n)
  block
   character(k-1),dimension(p+2) ::ary,cry(2+k)*(k),bry(2+2)*(k),ary2,bry2(2+2)*(k),cry2(2+k)*(k)
    if (len (ary)/=1) print *,1001
