...
subroutine s01
 use m13,only:gen,assignment(=),operator=>operator_
 use m0,only:xx
 type(xx)::z
 if (operator/=111)print *,2101
 if (gen(xx(1),2)/=13)print *,22105
 z=2
...
subroutine s02
 use m11,only:operator(.o.)=>operator(.opa.),&
              ...
              operator 
 type(xx)::z
 if (gen(xx(1),2)/=13)print *,221051
 z=2
 if (z%x1/=2)print *,221061
 if ((xx(1).o.xx(2))/=3)print *,22101
 if (operator/=104)print *,501
end
subroutine s03
 use m11,only:xx,&
