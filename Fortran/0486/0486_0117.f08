MODULE m1
  ...
  END TYPE
CONTAINS
  FUNCTION fx( d,dd ) RESULT( r )
    TYPE(x), INTENT(IN) :: d(:)
    TYPE(x), allocatable :: dd(:)
    TYPE(x) :: r(size(d))
   r=d
   do k=1,size(r)
   r(k)%z0=d(k)%z0+3
...
allocate(v(3))
do k=1,3
allocate(v(k)% z0    ,source=-1)
allocate(v(k)% z1(10),source=-1)
allocate(v(k)% z2(10),source=-1)
allocate(v(k)% z3(10),source=-1)
allocate(v(k)% z4(10),source=-1)
allocate(v(k)% z5(10),source=-1)
end do
k=3
v(:k)= v(:k)
