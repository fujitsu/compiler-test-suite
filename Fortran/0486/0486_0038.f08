...
w%yy= w%yv(:)
if (any(ubound(w%yy)/=3)) print *,801, ubound(w%yy)
if (any(lbound(w%yy)/=1)) print *,811, lbound(w%yy)
w%zz= w%yz(:)
if (any(ubound(w%zz)/=3)) print *,701, ubound(w%zz)
if (any(lbound(w%zz)/=1)) print *,711, lbound(w%zz)
end
call s1
print *,'pass'
end
