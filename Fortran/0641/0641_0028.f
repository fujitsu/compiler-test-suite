   ...
   10 CONTINUE
      ...
      SUBROUTINE SUB ( I1, I2, ID1, ID2 )
       ...
       COMMON /BL1/ RA, RB, RC, RE
       COMMON /BL2/ CEA, CEB, CEC
       COMMON /BL3/ IL
       DATA SA/'FFFFF'/ 
       DATA LBA/1000*.TRUE./ 
       REAL RR2, RR1
       IF (SA .EQ. 'FALSE') THEN
        RR1 = RC(I1) + RA(ID2,ID2,I1) + RE(ID2,ID2,I2)
        DO J1=1,6,5
         RB(J1,J1) = RR1
         RB(J1+1,J1+1) = RR1
         RB(J1+2,J1+2) = RR1
         RB(J1+3,J1+3) = RR1
         RB(J1+4,J1+4) = RR1
         CEA(ID1,ID2,J1) = CEC(J1)
         CEA(ID1,ID2,J1+1) = CEC(J1+1)
         CEA(ID1,ID2,J1+2) = CEC(J1+2)
         CEA(ID1,ID2,J1+3) = CEC(J1+3)
         CEA(ID1,ID2,J1+4) = CEC(J1+4)
        END DO
        DO J1=1,10
         WRITE (6, *) 'LBA=', LBA
        END DO
       ELSE
        RR2 = RC(I1) + RA(ID2,ID2,I1) + RE(ID2,ID2,I2)
        DO J1=1,6,5
         RB(J1,J1) = RR2
         RB(J1+1,J1+1) = RR2
         RB(J1+2,J1+2) = RR2
