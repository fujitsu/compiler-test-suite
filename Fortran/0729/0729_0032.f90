 ...
 write(1,*)maxloc(u,dim=2)
 uu=maxloc(u,dim=2)
 write(1,*)uu
 call sub(maxloc(u,dim=2))
 write(1,*)minloc(u,dim=2)
 uu=minloc(u,dim=2)
 write(1,*)uu
 call sub(minloc(u,dim=2))
 u=reshape((/20,1,19,2,18,3,17,4,16,5,15,6,14,7,13,8,12,9,11,10/),(/1,4,5,1,1/))
 write(1,*)maxloc(u,dim=3)
 uu(:,:4,:,:)=maxloc(u,dim=3)
 write(1,*)uu(:,:4,:,:)
 call subx(maxloc(u,dim=3))
 write(1,*)minloc(u,dim=3)
 uu(:,:4,:,:)=minloc(u,dim=3)
 write(1,*)uu(:,:4,:,:)
 call subx(minloc(u,dim=3))
 rewind 1
 read(1,*) xu(2,:);if (any(xu(2,:)/=(/4,1,4,1,4/)))write(6,*) "NG"
 read(1,*) xu(2,:);if (any(xu(2,:)/=(/4,1,4,1,4/)))write(6,*) "NG"
 read(1,*) xu(2,:);if (any(xu(2,:)/=(/4,1,4,1,4/)))write(6,*) "NG"
 read(1,*) xu(2,:);if (any(xu(2,:)/=(/1,4,1,4,1/)))write(6,*) "NG"
 read(1,*) xu(2,:);if (any(xu(2,:)/=(/1,4,1,4,1/)))write(6,*) "NG"
 read(1,*) xu(2,:);if (any(xu(2,:)/=(/1,4,1,4,1/)))write(6,*) "NG"
 read(1,*) xu(2,:4);if (any(xu(2,:4)/=(/1,5,1,5/)))write(6,*) "NG"
 read(1,*) xu(2,:4);if (any(xu(2,:4)/=(/1,5,1,5/)))write(6,*) "NG"
 read(1,*) xu(2,:4);if (any(xu(2,:4)/=(/1,5,1,5/)))write(6,*) "NG"
 read(1,*) xu(2,:4);if (any(xu(2,:4)/=(/5,1,5,1/)))write(6,*) "NG"
 read(1,*) xu(2,:4);if (any(xu(2,:4)/=(/5,1,5,1/)))write(6,*) "NG"
 read(1,*) xu(2,:4);if (any(xu(2,:4)/=(/5,1,5,1/)))write(6,*) "NG"
 contains
  subroutine sub(n)
