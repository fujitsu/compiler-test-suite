subroutine s(i,k)
 ...
 type (x),pointer:: v(:)
 integer,target::t1(2,2,3)
 integer,target::t2(2,2,4)
 pointer(n1,d1)
 pointer(n2,d2)
 t1=reshape((/(m,m=1,12)/),(/2,2,3/))
 t2=reshape((/(m,m=1,16)/),(/2,2,4/))
 allocate(v(k))
 select case(i)
  case (1)
...
select case(i)
 case (1)
   ...
   if (loc(i1)/=n1)write(25,*) i,'i1 copied'
   if (loc(i2)==n2)write(25,*) i,'i2 no copied'
 case (2)
   if (any(i1/=(/(m,m=111,118)/)))write(6,*) "NG"
   if (any(i2/=(/(m,m=113,120)/)))write(6,*) "NG"
   if (loc(i1)/=n1)write(25,*) i,'i1 copied'
   if (loc(i2)/=n2)write(25,*) i,'i2 copied'
 case (3)
   if (any(i1/=(/5,6,7,8,13,14,15,16/)))write(6,*) "NG"
   if (any(i2/=(/(m,m=211,218)/)))write(6,*) "NG"
   if (loc(i1)==n1)write(25,*) i,'i1 no copied'
   if (loc(i2)/=n2)write(25,*) i,'i2 copied'
 case (4)
