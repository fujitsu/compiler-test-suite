subroutine s(i,k)
 type x
   integer::p1,p2
 end type
 type (x):: v(2)
 integer,target::t1(3)=(/1,2,3/)
 integer,target::t2(4)=(/4,5,6,7/)
 pointer(n1,d1)
 pointer(n2,d2)
 select case(i)
  case (1)
   v%p1=(/11,12/)
   v%p2=t1(1:3:2)
  case (2)
   v%p1=(/111,112/)
   v%p2=(/113,114/)
  case (3)
   v%p2=(/211,212/)
   v%p1=t2(2:4:2)
  case (4)
   v%p1=t1(1:3:2)
   v%p2=t2(2:4:2)
end select
n1=0;n2=0
j=jfoo(v%p1,v%p2,n1,n2,i)+jfoo(v%p1,v%p2,n1,n2,i)
if (j/=i*2)write(6,*) "NG"
end
call s(1,2)
call s(2,2)
call s(3,2)
call s(4,2)
print *,'pass'
end
integer function jfoo(i1,i2,n1,n2,i)
integer i1(2),i2(2)
 pointer(n1,d1)
 pointer(n2,d2)
select case(i)
 case (1)
   if (i1(1)/=11)write(6,*) "NG"
   if (i1(2)/=12)write(6,*) "NG"
   if (i2(1)/=1)write(6,*) "NG"
   if (i2(2)/=3)write(6,*) "NG"
   if (loc(i1)/=n1)write(61,*) i,'i1 copied'
   if (loc(i2)==n2)write(61,*) i,'i2 no copied'
 case (2)
   if (i1(1)/=111)write(6,*) "NG"
   if (i1(2)/=112)write(6,*) "NG"
   if (i2(1)/=113)write(6,*) "NG"
   if (i2(2)/=114)write(6,*) "NG"
   if (loc(i1)/=n1)write(61,*) i,'i1 copied'
   if (loc(i2)/=n2)write(61,*) i,'i2 copied'
 case (3)
   if (i1(1)/=5)write(6,*) "NG"
   if (i1(2)/=7)write(6,*) "NG"
   if (i2(1)/=211)write(6,*) "NG"
   if (i2(2)/=212)write(6,*) "NG"
   if (loc(i1)==n1)write(61,*) i,'i1 no copied'
   if (loc(i2)/=n2)write(61,*) i,'i2 copied'
 case (4)
   if (i1(1)/=1)write(6,*) "NG"
   if (i1(2)/=3)write(6,*) "NG"
   if (i2(1)/=5)write(6,*) "NG"
   if (i2(2)/=7)write(6,*) "NG"
   if (loc(i1)==n1)write(61,*) i,'i1 no copied'
   if (loc(i2)==n2)write(61,*) i,'i2 no copied'
end select
jfoo=i
end
