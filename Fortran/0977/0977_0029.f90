subroutine s(i,k)
 ...
 type (x),pointer:: v(:)
 integer,target::t1(3,2,2)
 integer,target::t2(4,2,2)
 pointer(n1,d1)
 pointer(n2,d2)
 t1=reshape((/(m,m=1,12)/),(/3,2,2/))
 t2=reshape((/(m,m=1,16)/),(/4,2,2/))
 allocate(v(k))
 select case(i)
  case (1)
...
n1=loc(v(2)%p1)
n2=loc(v(2)%p2)
call jfoo(v(k)%p1,v(k)%p2,n1,n2,i)
if (any(v(2)%p1/=-1))write(6,*) "NG"
if (any(v(2)%p2/=-2))write(6,*) "NG"
deallocate(v)
end
call s(1,2)
call s(2,2)
...
select case(i)
 case (1)
   ...
   if (loc(i1)/=n1)write(21,*) i,'i1 copied'
   if (loc(i2)==n2)write(21,*) i,'i2 no copied'
 case (2)
   if (any(i1/=(/(m,m=111,118)/)))write(6,*) "NG"
   if (any(i2/=(/(m,m=113,120)/)))write(6,*) "NG"
   if (loc(i1)/=n1)write(21,*) i,'i1 copied'
   if (loc(i2)/=n2)write(21,*) i,'i2 copied'
 case (3)
   if (any(i1/=(/2,4,6,8,10,12,14,16/)))write(6,*) "NG"
   if (any(i2/=(/(m,m=211,218)/)))write(6,*) "NG"
   if (loc(i1)==n1)write(21,*) i,'i1 no copied'
   if (loc(i2)/=n2)write(21,*) i,'i2 copied'
 case (4)
