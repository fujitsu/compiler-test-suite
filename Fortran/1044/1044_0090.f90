...
else if (ksize==4) then
 select case(kv)
  case(1)
    if (any(a(1:4)/=(/1,2,3,4/)))then
        write(6,*) 'ERROR item&subcode:',item, subcode ,' Value: ',a(:4)
    endif
  case(4)
   if (item==9 .or. item==10) then
    if (any(a(1:4)/=(/1,2,4,5/)))then
        write(6,*) 'ERROR item&subcode:',item, subcode ,' Value: ',a(:4)
    endif
   else 
    if (any(a(1:4)/=(/1,2,3,4/)))then
  ...
  case(5)
    if (any(a(1:4)/=(/7,8,10,11/)))then
        write(6,*) 'ERROR item&subcode:',item, subcode ,' Value: ',a(:4)
    else 
        write(1,*) 'OK    :',item
    endif
  case(2)
    if (any(a(1:4)/=(/7,8,9,10/)))then
        write(6,*) 'ERROR item&subcode:',item, subcode ,' Value: ',a(:4)
    else 
        write(1,*) 'OK    :',item
    endif
  case(3)
   if (item==9 .or. item==10) then
    ...
    if (any(a(1:4)/=(/13,14,15,16/)))then
        write(6,*) 'ERROR item&subcode:',item, subcode ,' Value: ',a(:4)
    endif
   endif
  case(6)
  if (any(a(1:4)/=(/13,14,16,17/)))then
      write(6,*) 'ERROR item&subcode:',item, subcode ,' Value: ',a(:4)
  else 
      write(1,*) 'OK    :',item
  endif
