...
integer(1),allocatable,dimension(:)::sv1,sv2
  ...
  if (.not.allocated(a(1)%ya(2)%zc)) write(6,*) "NG"
  if (     allocated(a(1)%ya(3)%zc)) write(6,*) "NG"
  if (.not.allocated(a(1)%ya(1)%zr)) write(6,*) "NG"
  if (.not.allocated(a(1)%ya(2)%zr)) write(6,*) "NG"
  if (.not.allocated(a(1)%ya(3)%zr)) write(6,*) "NG"
  if (any(a(1)%ya(2)%zi/=(/1,2,3/))) write(6,*) "NG"
  if (any(a(1)%ya(3)%zi/=(/4,5,6/))) write(6,*) "NG"
  if (any(a(1)%ya(1)%zi/=(/7,8,9/))) write(6,*) "NG"
  if (any(a(1)%ya(2)%zc/=(/'ABC','abc'/))) write(6,*) "NG"
  if (any(a(1)%ya(1)%zc/=(/'GHI','ghi'/))) write(6,*) "NG"
  if (any(a(1)%ya(2)%zr/=(/1.0,2.0/))) write(6,*) "NG"
  if (any(a(1)%ya(3)%zr/=(/3.0,4.0/))) write(6,*) "NG"
