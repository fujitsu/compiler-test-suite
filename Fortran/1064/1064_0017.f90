 ...
 subroutine ss1
   ...
   logical:: f=.false.
   if (maxval(a)/=3)print *,'fail'
   if (any(maxloc(a,back=f)/=2))print *,'fail'
   call chk1(maxval(a),3)
   call chk2(maxloc(a,back=f),2)
   if (minval(a)/=1)print *,'fail'
   if (any(minloc(a,back=f)/=3))print *,'fail'
   call chk1(minval(a),1)
   call chk2(minloc(a,back=f),3)
   if (maxval(b)/=5)print *,'fail'
   if (any(maxloc(b,back=f)/=(/1,2/)))print *,'fail'
   call chk1(maxval(b),5)
   call chk3(maxloc(b,back=f),(/1,2/))
   if (minval(b)/=0)print *,'fail'
   if (any(minloc(b,back=f)/=(/2,2/)))print *,'fail'
...
logical::f=.false.
   ...
   call chk3(maxloc(b,back=f),(/1,2/))
   if (minval(b)/=0)print *,'fail'
   if (any(minloc(b)/=(/2,2/)))print *,'fail'
   call chk1(minval(b),0)
   call chk3(minloc(b,back=f),(/2,2/))
 contains
 subroutine chk1(i,j)
