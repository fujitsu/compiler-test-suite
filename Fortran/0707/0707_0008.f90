 ...
 module m1
  ...
  subroutine i4_4(a)
   ...
   ax(:,:,2::j2,:,:)=a
   ap=>ax(:,:,2::j2,:,:)
   call ss2(reshape(a, (/2*j3*4*5/)))
   call ss2(reshape(ap,(/2*j3*4*5/)))
   call ss2(reshape(ax(:,:,2::j2,:,:),(/2*j3*4*5/)))
   call ss2(reshape(ax(:,:,v3,:,:),(/2*j3*4*5/)))
   call ss2(reshape((/(cmplx(i,i,16),i=11,11+2*j3*4*5-1)/),(/2*j3*4*5/)))
   call ss3(reshape(a, (/2*j3*4*5/)))
   call ss3(reshape(ap,(/2*j3*4*5/)))
   call ss3(reshape(ax(:,:,2::j2,:,:),(/2*j3*4*5/)))
   call ss3(reshape(ax(:,:,v3,:,:),(/2*j3*4*5/)))
   call ss3(reshape((/(cmplx(i,i,16),i=11,11+2*j3*4*5-1)/),(/2*j3*4*5/)))
   write(iu,*) reshape(a,(/2*j3*4*5/))
   write(iu,*) reshape(ap,(/2*j3*4*5/))
   write(iu,*) reshape(ax(:,:,2::j2,:,:),(/2*j3*4*5/))
   write(iu,*) reshape(ax(:,:,v3,:,:),(/2*j3*4*5/))
   write(iu,*) reshape(ax(:,:,v3,:,:),(/2*j3*4*5/))
   call ss4
  end subroutine
  subroutine i4_3(a)
  ...
  subroutine i4_1(a)
   ...
   call ss2(reshape(ap,(/2*j3*4*5/)))
   call ss2(reshape(ax(2::j2),(/2*j3*4*5/)))
   call ss2(reshape(ax(v5),(/2*j3*4*5/)))
   call ss2(reshape((/(cmplx(i,i,16),i=11,11+2*j3*4*5-1)/),(/2*j3*4*5/)))
   call ss3(reshape(a, (/2*j3*4*5/)))
   call ss3(reshape(ap,(/2*j3*4*5/)))
   call ss3(reshape(ax(2::j2),(/2*j3*4*5/)))
   call ss3(reshape(ax(v5),(/2*j3*4*5/)))
   call ss3(reshape((/(cmplx(i,i,16),i=11,11+2*j3*4*5-1)/),(/2*j3*4*5/)))
   write(iu,*) reshape(a,(/2*j3*4*5/))
   write(iu,*) reshape(ap,(/2*j3*4*5/))
   write(iu,*) reshape(ax(2::j2),(/2*j3*4*5/))
   write(iu,*) reshape(ax(v5),(/2*j3*4*5/))
   write(iu,*) reshape(ax(v5),(/2*j3*4*5/))
   call ss4
  end subroutine
  subroutine ss2(a)
     character(n),dimension(2*j3*4*5)::a
     if (any(a/= reshape((/(cmplx(i,i,16),i=11,11+2*j3*4*5-1)/),(/2*j3*4*5/))))print *,'fail'
  end subroutine
  subroutine ss3(a)
     character(n),dimension(  :    )::a
     if (any(a/= reshape((/(cmplx(i,i,16),i=11,11+2*j3*4*5-1)/),(/2*j3*4*5/))))print *,'fail'
     if (size(a)/=2*j3*4*5)print *,'fail'
     if (ubound(a,1)/=2*j3*4*5)print *,'fail'
  end subroutine
  subroutine ss4()
     character(n),dimension(2*j3*4*5)::a; rewind iu
     do iz=1,5; read(iu,*) a
     if (any(a/= reshape((/(cmplx(i,i,16),i=11,11+2*j3*4*5-1)/),(/2*j3*4*5/))))print *,'fail'
     enddo
  end subroutine
