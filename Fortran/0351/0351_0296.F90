program main
  ...
  do i=1,64
     if(i .gt. 64/2) then
          if(((abs(real(cmplx(real(i,kind=8),real(i,kind=8),kind=8)) - real( res(i)))/abs(real(cmplx(real(i,kind=8),real(i,kind=8),kind=8)))) .gt. 1.0D-13 .or. (abs(imag(cmplx(real(i,kind=8),real(i,kind=8),kind=8)) - imag( res(i)))/abs(imag(cmplx(real(i,kind=8),real(i,kind=8),kind=8)))) .gt. 1.0D-13)) then
     	     print *, "NG"
          endif
     else
