program main
  ...
  enddo
  do i=1,64
     if(((abs(real(dest(i)) - real( cmplx(real(i,kind=4),real(i,kind=4),kind=4) + cmplx(1._4, 2._4, kind=4)))/abs(real(dest(i)))) .gt. 1.0E-6 .or. (abs(imag(dest(i)) - imag( cmplx(real(i,kind=4),real(i,kind=4),kind=4) + cmplx(1._4, 2._4, kind=4)))/abs(imag(dest(i)))) .gt. 1.0E-6)) then
        print *, "NG"
     endif
  enddo
  print *, "OK"
