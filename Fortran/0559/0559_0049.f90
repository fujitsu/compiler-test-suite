...
CONTAINS
  SUBROUTINE SUB(dmy)
    ...
    IF(LBOUND(dmy%cmp_ty1%cmp_ty%ch,1) /= 2) PRINT*,'104',LBOUND(dmy%cmp_ty1%cmp_ty%ch,1)
    IF(UBOUND(dmy%cmp_ty1%cmp_ty%ch,1) /= 5) PRINT*,'105',LBOUND(dmy%cmp_ty1%cmp_ty%ch,1)
    PRINT*,'PASS'
  END SUBROUTINE
END PROGRAM
