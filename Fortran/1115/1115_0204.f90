...
contains
  ...
  subroutine check(end_i, end_j, end_k)
    ...
    do i=1,100
       do j=1,100
          do k=1,100
             if (output(i,k,j) .ne. input(i,k,j)  + 3) then
                print *,'NG=', input(i,k,j), output(i,k,j)
             endif
          end do
       end do
    end do
    if (last_i .ne. i) print *,'end_i=',100+0,'calc_i=',last_i,'opt_i=',i+0
