...
integer(8),allocatable,dimension(:,:)::P1,P2,P3,P4,P5,P6,P7,P8,P9,PA,PB,PC,PD,PE,PF,PG,PH
...
P8(1:N2,1:N3) = TRANSPOSE( int(reshape((/(i,i=1,6)/),(/3,2/)),8))
P9(1:N2,1:N3) = TRANSPOSE( reshape((/1_8,2_8,3_8,4_8,5_8,6_8/),(/3,2/)))
PA(1:N2,1:N3) = TRANSPOSE( reshape((/POP(1:N3,1:N2)/),(/3,2/)))
PB(1:N2,1:N3) = TRANSPOSE( reshape((/POP(1:N3,1:N2)+POP(VN3,VN2)-POP(:,:)/),(/3,2/)))
PC(1:N2,1:N3) = TRANSPOSE( int(reshape((/(i,i=1,6)/),(/N3,N2/)),8))
PD(1:N2,1:N3) = TRANSPOSE( reshape((/1_8,2_8,3_8,4_8,5_8,6_8/),(/N3,N2/)))
...
call s(TRANSPOSE( int(reshape((/(i,i=1,6)/),(/3,2/)),8)))
call s(TRANSPOSE( reshape((/1_8,2_8,3_8,4_8,5_8,6_8/),(/3,2/))))
call s( TRANSPOSE( reshape((/POP(1:N3,1:N2)/),(/3,2/))))
call s(TRANSPOSE( reshape((/POP(1:N3,1:N2)+POP(VN3,VN2)-POP(:,:)/),(/3,2/))))
call s( TRANSPOSE( int(reshape((/(i,i=1,6)/),(/N3,N2/)),8)))
call s( TRANSPOSE( reshape((/1_8,2_8,3_8,4_8,5_8,6_8/),(/N3,N2/))))
...
if (any(abs(P2-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(P3-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(P4-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(P5-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(P6-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(P7-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(P8-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(P9-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(PA-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(PB-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(PC-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(PD-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(PE-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(PF-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(PG-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
if (any(abs(PH-reshape((/1,4,2,5,3,6/),(/2,3/)))>0))write(6,*) "NG"
print *,'pass'
contains
subroutine s(x)
...
integer(8),dimension(:,:)::x
if (any(shape(x)/=(/2,3/)))write(6,*) "NG"
if (any(abs(x-cos(0.+reshape((/1,4,2,5,3,6/),(/2,3/))))>0))write(6,*) "NG"
end subroutine
end
