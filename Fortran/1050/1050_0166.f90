...
program main
  ...
  d=0
  !$omp parallel
  !$omp sections private(a1,b1,now) firstprivate(a2,b2) lastprivate(a3,b3) reduction(+:a4,b4)
  !$omp section
  do i=1,n
  ...
  do i=1,n
     ...
     if (a1(1,1,1,1,1,1,i).ne.b1(1,1,1,1,1,1,i)) d(i,now)=d(i,now)+1
     if (a2(1,1,1,1,1,1,i).ne.b2(1,1,1,1,1,1,i)) d(i,now)=d(i,now)+1
     if (a3(1,1,1,1,1,1,i).ne.b3(1,1,1,1,1,1,i)) d(i,now)=d(i,now)+1
     if (a4(1,1,1,1,1,1,i).ne.b4(1,1,1,1,1,1,i)) d(i,now)=d(i,now)+1
  end do
  !$omp end sections
  !$omp end parallel
  ...
  do i=1,n
   do j=1,max
     if (c(i,j).ne.0.or.d(i,j).ne.0) err=err+1
   end do
  end do
  if (err.eq.0) then
     print *,"OK"
