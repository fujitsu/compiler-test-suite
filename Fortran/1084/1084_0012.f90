...
allocate(base(4)%ii2(2))
allocate(ty::base(4)%lhs(2))
...
select type (a=>base(4)%lhs)
 type is (ty)
  select type (b=>base(4)%lhs)
   type is (ty)
    deallocate(base(4)%lhs)
    allocate(base(4)%lhs, mold = rhs0)
    if(allocated(base(4)%lhs) .AND. size(base(4)%lhs) == 3) then
     deallocate(base(4)%lhs)
     allocate(base(4)%lhs, source = rhs0)
    endif
...
select type (a=>base(4)%lhs)
 type is (ty)
  select type (b=>base(4)%lhs)
   type is (ty)
    deallocate(base(4)%lhs)
    allocate(base(4)%lhs, source = rhs1(2:3))
...
select type (a=>base(4)%lhs)
 type is (ty)
  select type (b=>base(4)%lhs)
   type is (ty)
    deallocate(base(4)%lhs)
    allocate(base(4)%lhs, source = objTT(2)%base1)
...
select type (a=>base(4)%lhs)
 type is (ty)
  select type (b=>base(4)%lhs)
   type is (ty)
    deallocate(base(4)%lhs)
    allocate(base(4)%lhs, source = objTT(2)%base2)
