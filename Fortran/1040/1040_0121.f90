...
subroutine s01(d)
integer::d(:)
  if (any(f01(d(2:3))/=(/1,1001/)+1000))write(6,*) "NG"
  if (size(f01(d(2:3)))/=2)write(6,*) "NG"
  if (any(lbound(f01(d(2:3)))/=1))write(6,*) "NG"
  if (any(ubound(f01(d(2:3)))/=2))write(6,*) "NG"
end subroutine
subroutine s02(d)
integer::d(:,:)
  if (any(f02(d(2:3,2:4))/=1100+(/ 1,101,1001,1101,2001,2101/)))write(6,*) "NG"
  if (size(f02(d(2:3,2:4)))/=6)write(6,*) "NG"
  if (any(lbound(f02(d(2:3,2:4)))/=1))write(6,*) "NG"
  if (any(ubound(f02(d(2:3,2:4)))/=6))write(6,*) "NG"
end subroutine
subroutine s03(d)
integer::d(:,:,:)
  if (any(f03(d(2:3,2:4,2:5))/=1110+(/&
1,11,101,111,201,211,1001,1011,1101,1111,1201,1211,2001,2011,2101,2111,2201,2211,3001,3011,3101,3111,3201,3211&
/)))write(6,*) "NG"
  if (size(f03(d(2:3,2:4,2:5)))/=24)write(6,*) "NG"
  if (any(lbound(f03(d(2:3,2:4,2:5)))/=1))write(6,*) "NG"
  if (any(ubound(f03(d(2:3,2:4,2:5)))/=24))write(6,*) "NG"
end subroutine
subroutine s04(d)
integer::d(:,:,:,:)
  if (any(f04(d(2:3,2:4,2:5,2:6))/=1111+(/&
...
/)))write(6,*) "NG"
  if (size(f04(d(2:3,2:4,2:5,2:6)))/=24*5)write(6,*) "NG"
  if (any(lbound(f04(d(2:3,2:4,2:5,2:6)))/=1))write(6,*) "NG"
  if (any(ubound(f04(d(2:3,2:4,2:5,2:6)))/=24*5))write(6,*) "NG"
end subroutine
end
use m2
