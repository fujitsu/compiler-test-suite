...
subroutine s01(d)
integer::d(:)
  if (any(f01(d(1:2))/=(/1,1001/)))write(6,*) "NG"
  if (size(f01(d(1:2)))/=2)write(6,*) "NG"
  if (any(lbound(f01(d(1:2)))/=1))write(6,*) "NG"
  if (any(ubound(f01(d(1:2)))/=2))write(6,*) "NG"
end subroutine
subroutine s02(d)
integer::d(:,:)
  if (any(f02(d(:2,:3))/=(/ 1,101,1001,1101,2001,2101/)))write(6,*) "NG"
  if (size(f02(d(:2,:3)))/=6)write(6,*) "NG"
  if (any(lbound(f02(d(:2,:3)))/=1))write(6,*) "NG"
  if (any(ubound(f02(d(:2,:3)))/=6))write(6,*) "NG"
end subroutine
subroutine s03(d)
integer::d(:,:,:)
  if (any(f03(d(:2,:3,:4))/=(/&
...
/)))write(6,*) "NG"
  if (size(f03(d(:2,:3,:4)))/=24)write(6,*) "NG"
  if (any(lbound(f03(d(:2,:3,:4)))/=1))write(6,*) "NG"
  if (any(ubound(f03(d(:2,:3,:4)))/=24))write(6,*) "NG"
end subroutine
subroutine s04(d)
integer::d(:,:,:,:)
  if (any(f04(d(:2,:3,:4,:5))/=(/&
...
/)))write(6,*) "NG"
  if (size(f04(d(:2,:3,:4,:5)))/=24*5)write(6,*) "NG"
  if (any(lbound(f04(d(:2,:3,:4,:5)))/=1))write(6,*) "NG"
  if (any(ubound(f04(d(:2,:3,:4,:5)))/=24*5))write(6,*) "NG"
end subroutine
end
use m2
