...
do j=1,n
  do i=1,n
    if (mod(m1(i,j),max(m2(i,j),1)) .gt. 0) then
      if (mod(m1(i,j),max(m3(i,j)*m4(i,j),1)) .gt. 4) then
         a(i,j) = b(i,j) + c(i,j)
      else if (mod(m2(i,j),max(m3(i,j)*m4(i,j),1)) .gt. 2) then
         a(i,j) = b(i,j) - d(i,j)
      else
