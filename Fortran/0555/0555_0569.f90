...
CHARACTER(LEN=7)::str_chk(2)
INTEGER :: var1=3,var2=5,var3=5

ALLOCATE(ty::obj1)
obj2%int1=50
obj2%int2=100
ALLOCATE(CHARACTER(LEN=7)::mystr(2))
...
SELECT TYPE(obj1)
  ...
  TYPE IS (ty1)
    ...
    str_chk=mystr
    IF(str_chk(1) .NE. 'INDIA') PRINT*,502
END SELECT
...
SELECT TYPE(obj1)
  TYPE IS(ty1)
    print*,401
  TYPE IS(ty2) 
    var3=obj1%prc2(obj2,mystr)
    str_chk=mystr
    IF(str_chk(2) .NE. 'JAPAN') PRINT*,503
END SELECT
