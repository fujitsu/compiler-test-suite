...
subroutine tune_src(x,y,z,a1,b1,c,d,e1,f1,a2,b2,e2,f2)
  integer(kind=4)::i
  DATA_TYPE,dimension(N)::x,y,z
  DATA_TYPE,dimension(N)::a1,b1,c,d,e1,f1
  DATA_TYPE,dimension(N)::a2,b2,e2,f2,tmp
  do i=1,N
     x(i) = ( a1(i) * b1(i) * c(i) + d(i) ) * e1(i) - f1(i)
     tmp(i) = c(i) + d(i)
  end do
  do i=1,N
     y(i) = ( a2(i) + b2(i) ) * tmp(i) + ( e2(i) + f2(i) ) * tmp(i)
     z(i) = x(i) + y(i)
  end do
end subroutine tune_src

subroutine test(x,y,z,a1,b1,c,d,e1,f1,a2,b2,e2,f2)
  integer(kind=4)::i
  DATA_TYPE,dimension(N)::x,y,z
  DATA_TYPE,dimension(N)::a1,b1,c,d,e1,f1
  DATA_TYPE,dimension(N)::a2,b2,e2,f2
  do i=1,N
     x(i) = ( a1(i) * b1(i) * c(i) + d(i) ) * e1(i) - f1(i)
     y(i) = ( a2(i) + b2(i) ) * ( c(i) + d(i) ) + ( e2(i) + f2(i) ) * ( c(i) + d(i) )
     z(i) = x(i) + y(i)
  end do
end subroutine test
...
program main
  ...
  y2 = y1
  z2 = z1
  call tune_src(x1,y1,z1,a1,b1,c1,d1,e1,f1,a2,b2,e2,f2)

  call test(x2,y2,z2,a1,b1,c1,d1,e1,f1,a2,b2,e2,f2)
