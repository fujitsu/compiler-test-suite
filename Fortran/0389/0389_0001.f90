module mod
  ...
  contains
  subroutine str(ai,ar,aco,al,ach,axd,asd)
  ...
  subroutine sequ(ai,ar,aco,al,ach,asd)
    integer,allocatable,intent(out)::ai(:)
    real,allocatable,intent(out)::ar(:)
    complex,allocatable,intent(out)::aco(:)
    logical,allocatable,intent(out)::al(:)
...
call str(xx%ai,xx%ar,xx%aco,xx%al,xx%ach,xx%axd,xx%asd)
call sequ(se%ai,se%ar,se%aco,se%al,se%ach,se%asd)
print *,'pass'
end
