 ...
 module m1
   ...
   subroutine chk_bs(bs,bso)
    ...
    do ix=1,size(bs%b3)
      if (any(bs%b3(ix)%a2/=reshape((/2,3/),(/1,1,1,2/))))write(6,*) "NG"
      if (associated(bs%b3(ix)%a3))write(6,*) "NG"
    end do
   ...
   subroutine clr_bs(bs,bso)
    ...
    allocate(bs%b2(1,1))
    do ix=1,size(bs%b3)
      bs%b3(ix)%a2=reshape((/1112,1113/),(/1,1,1,2/))
      bs%b3(ix)%a3=>a_target
    end do
    if(any(bs%b4==reshape((/(i,i=121,126)/),(/2,3,1/))))write(6,*) "NG"
  end subroutine
