subroutine fptrans_r4toi1(a,b,x,n)
real(4),dimension(1:n),volatile :: a,b
real(4),volatile :: x
do i=1,n
  b(i) = a(i)-int(a(i)/x,kind=1)*x
enddo
end subroutine

subroutine fptrans_r4toi2(a,b,x,n)
real(4),dimension(1:n),volatile :: a,b
real(4),volatile :: x
do i=1,n
  b(i) = a(i)-int(a(i)/x,kind=2)*x
enddo
end subroutine

subroutine fptrans_r4toi4(a,b,x,n)
real(4),dimension(1:n),volatile :: a,b
real(4),volatile :: x
do i=1,n
  b(i) = a(i)-int(a(i)/x,kind=4)*x
enddo
end subroutine

subroutine fptrans_r4toi8(a,b,x,n)
real(4),dimension(1:n),volatile :: a,b
real(4),volatile :: x
do i=1,n
  b(i) = a(i)-int(a(i)/x,kind=8)*x
enddo
end subroutine

subroutine fptrans_r8toi1(a,b,x,n)
real(8),dimension(1:n),volatile :: a,b
real(8),volatile :: x
do i=1,n
  b(i) = a(i)-int(a(i)/x,kind=1)*x
enddo
end subroutine

subroutine fptrans_r8toi2(a,b,x,n)
real(8),dimension(1:n),volatile :: a,b
real(8),volatile :: x
do i=1,n
  b(i) = a(i)-int(a(i)/x,kind=2)*x
enddo
end subroutine

subroutine fptrans_r8toi4(a,b,x,n)
real(8),dimension(1:n),volatile :: a,b
real(8),volatile :: x
do i=1,n
  b(i) = a(i)-int(a(i)/x,kind=4)*x
enddo
end subroutine

subroutine fptrans_r8toi8(a,b,x,n)
real(8),dimension(1:n),volatile :: a,b
real(8),volatile :: x
do i=1,n
  b(i) = a(i)-int(a(i)/x,kind=8)*x
enddo
end subroutine

subroutine fptrans_r4()
interface
  subroutine fptrans_r4toi1(a,b,x,n)
  real(4),dimension(1:n),volatile :: a,b
  real(4),volatile :: x
  end subroutine

  subroutine fptrans_r4toi2(a,b,x,n)
  real(4),dimension(1:n),volatile :: a,b
  real(4),volatile :: x
  end subroutine

  subroutine fptrans_r4toi4(a,b,x,n)
  real(4),dimension(1:n),volatile :: a,b
  real(4),volatile :: x
  end subroutine

  subroutine fptrans_r4toi8(a,b,x,n)
  real(4),dimension(1:n),volatile :: a,b
  real(4),volatile :: x
  end subroutine
end interface

integer,parameter :: n=100
real(4),dimension(1:n),volatile :: r4a,r4b
real(4),parameter :: r4zero=50.0_4, r4error=0.0_4
real(4),volatile :: r4x
r4a = (/(real(i,kind=4),i=1,n)/)
r4x = 2.
call fptrans_r4toi1(r4a,r4b,r4x,n)
if (abs(sum(r4b) - r4zero) .le. r4error) then
  print *,"ok"
else
  print *,"ng",sum(r4b) - r4zero
endif
call fptrans_r4toi2(r4a,r4b,r4x,n)
if (abs(sum(r4b) - r4zero) .le. r4error) then
  print *,"ok"
else
  print *,"ng",sum(r4b) - r4zero
endif
call fptrans_r4toi4(r4a,r4b,r4x,n)
if (abs(sum(r4b) - r4zero) .le. r4error) then
  print *,"ok"
else
  print *,"ng",sum(r4b) - r4zero
endif
call fptrans_r4toi8(r4a,r4b,r4x,n)
if (abs(sum(r4b) - r4zero) .le. r4error) then
  print *,"ok"
else
  print *,"ng",sum(r4b) - r4zero
endif
end subroutine

subroutine fptrans_r8()
interface
  subroutine fptrans_r8toi1(a,b,x,n)
  real(8),dimension(1:n),volatile :: a,b
  real(8),volatile :: x
  end subroutine

  subroutine fptrans_r8toi2(a,b,x,n)
  real(8),dimension(1:n),volatile :: a,b
  real(8),volatile :: x
  end subroutine

  subroutine fptrans_r8toi4(a,b,x,n)
  real(8),dimension(1:n),volatile :: a,b
  real(8),volatile :: x
  end subroutine

  subroutine fptrans_r8toi8(a,b,x,n)
  real(8),dimension(1:n),volatile :: a,b
  real(8),volatile :: x
  end subroutine
end interface

integer,parameter :: n=100
real(8),dimension(1:n),volatile :: r8a,r8b
real(8),parameter :: r8zero=50.0_8, r8error=0.0_8
real(8),volatile :: r8x
r8a = (/(real(i,kind=8),i=1,n)/)
r8x = 2.
call fptrans_r8toi1(r8a,r8b,r8x,n)
if (abs(sum(r8b) - r8zero) .le. r8error) then
  print *,"ok"
else
  print *,"ng",sum(r8b) - r8zero
endif
call fptrans_r8toi2(r8a,r8b,r8x,n)
if (abs(sum(r8b) - r8zero) .le. r8error) then
  print *,"ok"
else
  print *,"ng",sum(r8b) - r8zero
endif
call fptrans_r8toi4(r8a,r8b,r8x,n)
if (abs(sum(r8b) - r8zero) .le. r8error) then
  print *,"ok"
else
  print *,"ng",sum(r8b) - r8zero
endif
call fptrans_r8toi8(r8a,r8b,r8x,n)
if (abs(sum(r8b) - r8zero) .le. r8error) then
  print *,"ok"
else
  print *,"ng",sum(r8b) - r8zero
endif
end subroutine

program main
call fptrans_r4()
call fptrans_r8()
end program
