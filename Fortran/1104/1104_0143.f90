     ...
     allocate(e::value%cv)
     allocate(value%cv%d(3),source=2_8)
     ...
     if (k1==k2) print *,2232
     if (k/=1) print *,1442
     if (any(value%cv%d/=2)) print *,821
     value%bv%d=-2
     if (any(var  %cv%d/=2)) print *,822
     k=0
     select type(p=>var%cv)
         type is(e)
           if (any(p     %n(:)/='xy')) print *,1259
           p  %n='qq'
           k1=loc(p  %n)
           if (any(p  %n/='qq')) print *,80201
           k=1
     end select
     if (k/=1) print *,1252
     k=0
