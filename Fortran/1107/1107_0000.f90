     module m1
       type :: b
         ...
         class (d),allocatable:: bd(:)
         class (e),allocatable:: be(:)
       end type
       type,extends(b)::be
         integer(8):: b2(3)
...
select type(p=>d)
 type is(be)
  ...
  call chk_c(p(2)%bc,kk)
           kn=0
   select type(u=> p(2)%bec)
     type is(ce)
           if (u(2)%ced(2)%da(2)/='12')print *,14903,u(2)%ced(2)%da(2)
           print "('2:',z16.16)",loc(u(2)%ced(2)%da(2))
           kn=1
   end select
   if (kn/=1) print *,29291
   stop
...
!  call set_d(p(2)%cd,kk)
  ...
  allocate(p(2)%ced(2)%da(2),source='12')
         !  print "('8:',z16.16)",loc(p(2)%ced(2)%da(2))
...
select type(w=>p(2)%bec)
 type is(ce)
  allocate(de::w(2)%ced(2))
         !  print "('77:',z16.16)",loc(w(2)%ced(2))
  allocate(w(2)%ced(2)%da(2),source='12')
         !  print "('7:',z16.16)",loc(w(2)%ced(2)%da(2))
end select
...
!   select type(u=> p(2)%bec)
!     type is(ce)
!       allocate(de::u(2)%ced(2))
!       allocate(u(2)%ced(2)%da(2),source='12')
!           print "('9:',z16.16)",loc(u(2)%ced(2)%da(2))
!   end select
! end select
  kk=1
...
select type(p=>v2)
  type is(be)
   select type(u=> p(2)%bec)
     type is(ce)
           if (u(2)%ced(2)%da(2)/='12')print *,14903,u(2)%ced(2)%da(2)
        !   print "('aa:',z16.16)",loc(u(2)%ced(2))
        !   print "('a:',z16.16)",loc(u(2)%ced(2)%da(2))
           kn=1
   end select
 end select
...
select type(p=>v1)
  type is(be)
   select type(u=> p(2)%bec)
     type is(ce)
           if (u(2)%ced(2)%da(2)/='12')print *,14903,u(2)%ced(2)%da(2)
        !   print "('bb:',z16.16)",loc(u(2)%ced(2))
        !   print "('b:',z16.16)",loc(u(2)%ced(2)%da(2))
           kn=1
   end select
 end select
end
