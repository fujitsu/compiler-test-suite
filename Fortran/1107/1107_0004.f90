...
select type(w=>p%bec)
 type is(ce)
  ...
  allocate(w%ced%da(2),source='12')
    !!     print "('7:',z16.16)",loc(w%ced%da(2))
end select
  !!!!call set_c(p%bec)
  k=1
 class default
...
!     type is(ce)
!       allocate(de::u%ced)
!       allocate(u%ced%da(2),source='12')
!           print "('9:',z16.16)",loc(u%ced%da(2))
!   end select
! end select
...
select type(p=>v2)
  type is(be)
   select type(u=> p%bec)
     type is(ce)
           if (u%ced%da(2)/='12')print *,14903,u%ced%da(2)
      !    print "('aa:',z16.16)",loc(u%ced)
      !    print "('a:',z16.16)",loc(u%ced%da(2))
   end select
 end select
