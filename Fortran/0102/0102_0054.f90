integer*1,parameter::a=128_1
integer*1,parameter::b=127_1
integer*1,parameter::c=-127_1
integer*1,parameter::d=-128_1
integer*2,parameter::e=32766_2
integer*2,parameter::f=32767_2
integer*2,parameter::g=-32767_2
integer*2,parameter::h=-32768_2
integer*4,parameter::i=2147483646_4
integer*4,parameter::j=2147483647_4
integer*4,parameter::k=-2147483646_4
integer*4,parameter::l=-2147483647_4
integer*8,parameter::w=9223372036854775806_8
integer*8,parameter::x=9223372036854775807_8
integer*8,parameter::y=-9223372036854775807_8
integer*8,parameter::z=-9223372036854775808_8
logical,parameter::m=BLE(a,b)
logical,parameter::n=BLE(c,d)
logical,parameter::o=BLE(e,f)
logical,parameter::p=BLE(g,h)
logical,parameter::q=BLE(I=i,J=j)
logical,parameter::r=BLE(j=l,i=k)
logical,parameter::s=BLE(w,j = x)
logical,parameter::t=BLE(i=y,j=z)
logical::m1,n1,o1,p1,q1,r1,s1,t1
m1=BLE(a,b)
n1=BLE(c,d)
o1=BLE(e,f)
p1=BLE(g,h)
q1=BLE(i,j)
r1=BLE(k,l)
s1=BLE(w,x)
t1=BLE(y,z)
if((m .neqv. m1) .and. (m .neqv. .false.))print*,"100"
if((n .neqv. n1) .and. (n .neqv. .false.))print*,"101"
if((o .neqv. o1) .and. (o .neqv. .true.))print*,"102"
if((p .neqv. p1) .and. (p .neqv. .false.))print*,"103"
if((q .neqv. q1) .and. (q .neqv. .true.))print*,"104"
if((r .neqv. r1) .and. (r .neqv. .false.))print*,"105"
if((s .neqv. s1) .and. (s .neqv. .true.))print*,"106"
if((t .neqv. t1) .and. (t .neqv. .false.))print*,"107"
print*,"PASS"
end
