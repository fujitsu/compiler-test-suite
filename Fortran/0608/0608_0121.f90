...
PROGRAM MAIN
    ...
    CONTAINS
        SUBROUTINE Foo( m_dmy, n_dmy )
            ...
            TYPE(ty1) :: ty1_obj(m_dmy - 1, n_dmy)
            ty1_obj(1, 1)%ty_cmp%ii = 100
            ty1_obj(2, 1)%ty_cmp%ii = 200
            ty1_obj(1, 2)%ty_cmp%ii = 300
            ty1_obj(2, 2)%ty_cmp%ii = 400
            CALL Foo1(ty1_obj)
        END SUBROUTINE
        SUBROUTINE Foo1( ty1_dmy1 )
            TYPE(ty1), INTENT(OUT):: ty1_dmy1(:, :)
            ty1_dmy1(2, 2)%ty_cmp%ii = 500
            CALL Foo2( ty1_dmy1(2, 2)%ty_cmp )
            ty1_dmy1(1, 1)%ty_cmp%ii = 600
            ty1_dmy1(2, 1)%ty_cmp%ii = 700
            ty1_dmy1(1, 2)%ty_cmp%ii = 800
        END SUBROUTINE
        SUBROUTINE Foo2( ty_dmy2)
            TYPE(ty), INTENT(OUT):: ty_dmy2(1, 2)
            ty_dmy2(1,1)%ii = 900
            ty_dmy2(1,2)%ii = 900
        END SUBROUTINE
END PROGRAM
