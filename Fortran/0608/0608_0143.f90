...
CONTAINS
    SUBROUTINE final_ty(dmy)
        ...
        IF ( dmy(1)%ii .EQ. (100*counter) ) THEN
           PRINT*, counter,' 1st-destructor call PASS ', dmy(1)%ii
        ELSE
           PRINT*, counter,' 1st-destructor call FAIL ', dmy(1)%ii
        END IF
    END SUBROUTINE
...
PROGRAM main
 ...
 INTERFACE
    ...
    END SUBROUTINE
 END INTERFACE
 ALLOCATE(ty3_obj%cmp_ty1(1)%cmp_ty(1)%ii)
 ALLOCATE(ty3_obj%cmp_ty1(1)%cmp_ty(2)%ii)
 ALLOCATE(ty3_obj%cmp_ty1(2)%cmp_ty(1)%ii)
 ALLOCATE(ty3_obj%cmp_ty1(2)%cmp_ty(2)%ii)
 ty3_obj%ll = 100
 ty3_obj%cmp_ty1%jj = 200
 ty3_obj%cmp_ty1(1)%cmp_ty(1)%ii = 300
 ty3_obj%cmp_ty1(2)%cmp_ty(2)%ii = 300
 ty3_obj%cmp_ty1(2)%cmp_ty(1)%ii = 400
 ty3_obj%cmp_ty1(2)%cmp_ty(2)%ii = 400
 ty3_obj%kk = 500
