...
!
 ...
 do i2=1,mm1; do i1=1,mm2
  ...
  if (ij/=ik)print *,'fail'
!
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b1*k1,mask=a2.and.a2))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b2*k1,mask=a2.and.a2))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b3,mask=a2.and.a2))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b1(:,:,:,:,:,:)*k1,mask=a2(:,:,:,:,:,:).and.a2(:,:,:,:,:,:)))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b2(:,:,:,:,:,:)*k1,mask=a2(:,:,:,:,:,:).and.a2(:,:,:,:,:,:)))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b3(:,:,:,:,:,:)*k1,mask=a2(:,:,:,:,:,:).and.a2(:,:,:,:,:,:)))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b1(:,:,:,:,v1,v2)*k1,mask=a2(:,:,:,:,v1,v2).and.a2(:,:,:,:,v1,v2)))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b2(:,:,:,:,v1,v2)*k1,mask=a2(:,:,:,:,v1,v2).and.a2(:,:,:,:,v1,v2)))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b3(:,:,:,:,v1,v2)*k1,mask=a2(:,:,:,:,v1,v2).and.a2(:,:,:,:,v1,v2)))
 end do; end do
...
!
 ...
 do i2=1,mm1; do i1=1,mm2
  ...
  if (ij/=ik)print *,'fail'
!
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b1        ))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b2        ))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b3        ))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b1        (:,:,:,:,:,:)))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b2        (:,:,:,:,:,:)))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b3        (:,:,:,:,:,:)))
   ...
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b1(:,:,:,:,v1,v2)))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b2(:,:,:,:,v1,v2)))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(b3(:,:,:,:,v1,v2)))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(reshape((/b1(:,:,:,:,v1,v2)/),(/1,1,1,1,2,3/))))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(reshape((/b2(:,:,:,:,v1,v2)/),(/1,1,1,1,2,3/))))
   call zz(ij,reshape((/(i,i=1,ik)/),(/k1,1,1,ik/))+maxval(reshape((/b3(:,:,:,:,v1,v2)/),(/1,1,1,1,2,3/))))
 end do; end do
 if (6/=count(a2))print *,'fail'
!
 end do
