  ...
  type ty1
     integer(kind=1),pointer :: pia(:,:) => ia(1:4:2,1:2)
     integer(kind=2),pointer :: pib(:,:) => ib(3:4,1:2:2)
     integer(kind=4),pointer :: pic(:,:) => ic(1:4:3,2:1:-1)
     integer(kind=8),pointer :: pid(:,:) => id(2:4:1,1:2:1)
     real(kind=4),pointer  :: pra(:,:) => ra(4:1:-1,1:2:1)
     real(kind=8),pointer  :: prb(:)   => rb(2,2:1:-1)
     real(kind=16),pointer :: prc(:)   => rc(1:4:2,2)
     complex(kind=4),pointer  :: pxa(:,:) => xa(2:3:2,1:2)
     complex(kind=8),pointer  :: pxb(:,:) => xb(3:2:-1,2:1:-1)
     complex(kind=16),pointer :: pxc(:)   => xc(3,1:2:2)
     logical(kind=1),pointer :: pla(:,:) => la(1:3:2,3:1:-2)
     logical(kind=2),pointer :: plb(:,:) => lb(2:3:1,3:1:-1)
     logical(kind=4),pointer :: plc(:,:) => lc(1:3:2,1:2:1)
     logical(kind=8),pointer :: pld(:)   => ld(2:3:2,1)
     character(kind=1),pointer :: pca(:,:) => ca(2:5:2,2:1:-1)
     character(kind=1,len=2),pointer :: psa(:,:) => sa(1:2,1:4:2)
  end type ty1
  type (ty1) :: str
