      ...
      subroutine s1
      integer*8 ip21,ip22,ip23,ip24,ip25,ip26
      ip21=malloc(8)
      ip22=malloc(8*10)
      ip23=malloc(8*10+8)
      ip24=malloc(8*8)
...
3     continue
      do 4 j=1,10
         ...
         write(15,'(4a)')i,ia(3),ia(3)//ia(3),ib(ic(2,2)+m)
4     continue
      rewind 15
      do 5 j=1,10
         write(xx,'(i4)') j+1
         read (15,'(4a)')ii,jj,ia(4),ib(ic(2,2)+3)
...
7     continue
      do 8 j=1,10
         ...
         ib(ic(2,2)+ic(2,2)-2)='3333'
         if (len(ib(12)).ne.4)print *,"fail"
         if (j.le.4) then
           if (len(i(:j)).ne.j)print *,"fail"
           if (len(ia(2)(:j)).ne.j)print *,"fail"
           if (len(ib(ic(2,2)+ic(2,2)-2)(:j)).ne.j)print *,"fail"
         endif
         if (j.le.3) then
           if (len(i(:j+1)).ne.j+1)print *,"fail"
           if (len(ia(2)(:j+1)).ne.j+1)print *,"fail"
           if (len(ib(ic(2,2)+ic(2,2)-2)(:j+1)).ne.j+1)print *,"fail"
         endif
8     continue
      do 9 j1=1,2
      do 9 j2=1,2
...
9     continue
      write(16,'(a)') id
      write(16,'(a)') (((id(j1,j2,j3),j1=1,2),j2=1,2),j3=1,2)
      do10 j3=1,2
      do10 j2=1,2
      do10 j1=1,2
10      if (id(j1,j2,j3).ne.'yyyy')print *,"fail"
      rewind 16
      read (16,'(a)') (((ie(j1,j2,j3),j1=1,2),j2=1,2),j3=1,2)
      do11 j3=1,len(i)-2
      do11 j2=1,len(i)/2
      do11 j1=1,2
