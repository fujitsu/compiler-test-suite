     ...
     +           chp=(/'A'/) )
      ...
      parameter(i1a=reshape(SOURCE=i1s,SHAPE=i1sh,PAD=i1p))
      parameter(i8a=reshape(SHAPE=i8sh,SOURCE=i8s,PAD=i8p))
      parameter(r8a=reshape(r8s,PAD=r8p,SHAPE=i2sh))
      parameter(c8a=reshape(c8s,i4sh,c8p))
      parameter(cha=reshape(chs,i2sh,chp))
      parameter(ta =reshape(ts ,i4sh2,tp))

      i1b=reshape(SOURCE=i1s,SHAPE=i1sh,PAD=i1p)
      i8b=reshape(SHAPE=i8sh,SOURCE=i8s,PAD=i8p)
      r8b=reshape(r8s,PAD=r8p,SHAPE=i2sh)
      c8b=reshape(c8s,i4sh,c8p)
      chb=reshape(chs,i2sh,chp)
      tb =reshape(ts ,i4sh2,tp)
