  ...
  use mcc,only:ri
   contains
     function ba(x1,x2,i,x3) result(ar)
      dimension i(:)
      integer(8),dimension(size(i),1+size(i)):: ar
      character*(*) x1(:),x2(:),x3(:)
      ar=ri
      call xx(x1,x2,x3)
 ...
 subroutine s1
  ...
  if (any(shape(aa(xx,xx,y(k3,4,4:k3+2)%za,xx))/=(/2,3/)))print *,'fail'
  ...
  if (any(shape(aax(y(k3,4,4:k3+2)%za,xx,xx,xx))/=(/2,3/)))print *,'fail'
  ...
  if (any(shape(aaax(xx,y(k3,4,4:k3+2)%za,xx,xx))/=(/2,3/)))print *,'fail'
  ...
  if (any(shape(aaaax(xx,xx,y(k3,4,4:k3+2)%za,xx))/=(/2,3/)))print *,'fail'
  ...
  if (any(shape(aaaaax(xx,xx,xx,y(k3,4,4:k3+2)%za))/=(/2,3/)))print *,'fail'
