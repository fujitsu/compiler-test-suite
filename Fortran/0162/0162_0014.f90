...
print *,'pass'
 ...
 subroutine alloc_mystr
   use mymod
   allocate (x, mystr%ptrs(1) )
   write(17,'(a,z14)') ' loc(mystr%ptrs)=',loc(mystr%ptrs)
   allocate ( mystr%ptrs(1)%dat )
   call init_ptr(1)
   write(17,'(a,z14)') ' loc(mystr%ptrs(1)%dat)=',loc(mystr%ptrs(1)%dat)
