 ...
 contains
   ...
   integer,intent(in)::i
   deallocate ( mystr%ptrs(iii((/i/)))%dat )
   end subroutine
   subroutine alloc_ptr(j)
   integer,intent(in)::j
   allocate ( mystr%ptrs(iii((/j/)))%dat )
   end subroutine
   function ifun(ii)
   ifun=ii
   end function
...
print *,'pass'
 ...
 subroutine alloc_mystr
   ...
   call alloc_ptr(1)
   write(11,'(a,z14)') ' loc(mystr%ptrs)=',loc(mystr%ptrs)
   allocate ( mystr%ptrs(1)%dat )
   call init_ptr(1)
   write(11,'(a,z14)') ' loc(mystr%ptrs(1)%dat)=',loc(mystr%ptrs(1)%dat)
