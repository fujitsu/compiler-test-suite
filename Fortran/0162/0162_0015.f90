 module mymod
   ...
   type(mystr_t)::mystr,x
 contains
   subroutine init_ptr(i)
   integer,intent(in)::i
   k=i
   deallocate ( x%ptrs(i)%dat,mystr%ptrs(k)%dat )
   end subroutine
 end module mymod
...
print *,'pass'
 ...
 subroutine alloc_mystr
   ...
   allocate ( x%ptrs(1) )
   write(18,'(a,z14)') ' loc(mystr%ptrs)=',loc(mystr%ptrs)
   allocate ( mystr%ptrs(1)%dat )
   allocate ( x%ptrs(1)%dat )
   call init_ptr(1)
   write(18,'(a,z14)') ' loc(mystr%ptrs(1)%dat)=',loc(mystr%ptrs(1)%dat)
