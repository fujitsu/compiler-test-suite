  ...
  if ( ( a(1)%ii(1,1).eq.11 .and. a(1)%ii(1,2).eq.12 .and. &
         ...
         .not.any(a(1)%ii/=reshape((/11,21,12,22/),(/2,2/))) ) then
    n=n+1
  else
    write(6,*) "NG"
  endif
  ...
  if ( ( a(1)%ch(1,1).eq.'ab' .and. a(1)%ch(2,1).eq.'cd' .and. &
        ...
        all(a(1)%ch==reshape((/'ab','cd','ef','gh'/),(/2,2/))) ) then
    n=n+1
  else
    write(6,*) "NG"
  endif
  ...
  if ( ( a(1)%rr(1,1).eq.11.0 .and. a(1)%rr(1,2).eq.12.0 .and. &
        ...
        all(a(1)%rr==reshape((/11,21,12,22/),(/2,2/))) ) then
    n=n+1
  else
    write(6,*) "NG"
  endif
  ...
  if ( ( a(1)%cc(1,1).eq.cmplx(1) .and. a(1)%cc(2,1).eq.cmplx(2) .and. &
        ...
        all(a(1)%cc==reshape(cmplx,(/2,2/))) )  then
    n=n+1
  else
    write(6,*) "NG"
  endif
  ...
  if ( ( a(1)%ii(1,1).eq.11 .and. a(1)%ii(1,2).eq.12 .and. &
         a(1)%ii(2,1).eq.21 .and. a(1)%ii(2,2).eq.22 )           .and. &
         .not.any(a(1)%ii/=reshape((/11,21,12,22/),(/2,2/)))             .and. &
       ( a(1)%ch(1,1).eq.'ab' .and. a(1)%ch(2,1).eq.'cd' .and. &
         a(1)%ch(1,2).eq.'ef' .and. a(1)%ch(2,2).eq.'gh' )       .and. &
        all(a(1)%ch==reshape((/'ab','cd','ef','gh'/),(/2,2/)))           .and. &
       ( a(1)%rr(1,1).eq.11.0 .and. a(1)%rr(1,2).eq.12.0 .and. &
         a(1)%rr(2,1).eq.21.0 .and. a(1)%rr(2,2).eq.22.0 )        .and. &
        all(a(1)%rr==reshape((/11,21,12,22/),(/2,2/)))                   .and. &
       ( a(1)%cc(1,1).eq.cmplx(1) .and. a(1)%cc(2,1).eq.cmplx(2) .and. &
         a(1)%cc(1,2).eq.cmplx(3) .and. a(1)%cc(2,2).eq.cmplx(4) ).and. &
        all(a(1)%cc==reshape(cmplx,(/2,2/)))                             .and. &
       ( a(1)%ll(1,1) .and. a(1)%ll(1,2) .and. &
         a(1)%ll(2,1) .and. a(1)%ll(2,2) )    .and. &
        all(a(1)%ll) ) goto 1000
  write(6,*) "NG"
1000 continue
