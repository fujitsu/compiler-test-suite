...
!$omp single
      ...
      if (any(a(:,:)(:i5-i1*3)/=b(:,:)(1:2)))print *,"fail"
      if (any(aa(:,:)(:i5-i1*3)/=b(:,:)(1:2)))print *,"fail"
      if (any(c(:,:)(:i5-i1*3)/=b(:,:)(1:2)))print *,"fail"
      if (any(cc(:,:)(:i5-i1*3)/=b(:,:)(1:2)))print *,"fail"
!$omp end single
!$omp barrier
!$omp end parallel
      end subroutine
...
!$omp end parallel
 ...
 subroutine ss3(i3,i5,mm,a,aa,b,c,cc)
      ...
      character*(*) a(:,:),aa(:,:)
      character*(*) c(:,:),cc(:,:)
!$omp parallel default(private) shared(mm,a,aa,b,c,cc,i1,i3,i5)
!$omp single
      b=reshape((/(mm(ii),ii=1,15)/),(/5,3/))
      i1=1
