...
subroutine test()
  ...
  do k=1,2
    if (.not.allocated(a(k)%ya)) write(6,*) "NG"
    ...
    if (.not.allocated(a(k)%ya(1,1)%za)) write(6,*) "NG"
    if (.not.allocated(a(k)%ya(1,2)%za)) write(6,*) "NG"
    if (.not.allocated(a(k)%ya(2,1)%za)) write(6,*) "NG"
    if (.not.allocated(a(k)%ya(2,2)%za)) write(6,*) "NG"
