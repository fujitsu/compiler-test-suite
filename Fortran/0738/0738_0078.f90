...
type z
  ...
  integer(4),allocatable,dimension(:)::zb
end type
type y
...
type(s)     ,allocatable,dimension(:)::a,b
  ...
  allocate(a(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya(k)%zb(3))

  a(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya(k)%za = (/1,2,3,4,5/)
  a(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya(k)%zb = (/6,7,8/)

  b(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya =           &
                                a(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya

  deallocate(a(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya(k)%zb)
  ...
  if (size(b(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya(k)%zb)/=3) write(6,*) "NG"

  if (any(b(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya(k)%za/=(/1,2,3,4,5/))) &
  write(6,*) "NG"
  if (any(b(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya(k)%zb/=(/6,7,8/))) &
  write(6,*) "NG"
