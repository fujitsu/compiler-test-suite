...
type(s)     ,allocatable,dimension(:)::a,b
  ...
  allocate(a(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(1))
  allocate(a(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(1)%ya(k)%za(5))
  ...
  if (any(b(k)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(1)%ya(k)%za/=(/1,2,3,4,5/))) &
  write(6,*) "NG"
