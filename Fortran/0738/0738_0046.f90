...
type(s)     ,allocatable,dimension(:)::a,b
  ...
  if (size(b(j)%sa)/=k) write(6,*) "NG"
  if (size(b(j)%sa(k)%ta)/=k) write(6,*) "NG"
  if (size(b(j)%sa(k)%ta(k)%ua)/=k) write(6,*) "NG"
  if (size(b(j)%sa(k)%ta(k)%ua(k)%va)/=k) write(6,*) "NG"
  if (size(b(j)%sa(k)%ta(k)%ua(k)%va(k)%wa)/=k) write(6,*) "NG"
  if (size(b(j)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa)/=k) write(6,*) "NG"
  if (size(b(j)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya)/=k) write(6,*) "NG"
  if (size(b(j)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya(k)%za)/=5) write(6,*) "NG"
  if (size(b(j)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya(k)%zi)/=4) write(6,*) "NG"

  if (any(b(j)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya(k)%za/=(/1,2,3,4,5/))) &
  write(6,*) "NG"
  if (any(b(j)%sa(k)%ta(k)%ua(k)%va(k)%wa(k)%xa(k)%ya(k)%zi/=(/6,7,8,9/))) &
  write(6,*) "NG"
