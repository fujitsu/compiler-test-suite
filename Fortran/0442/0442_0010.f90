...
type zz
type(z),allocatable:: zv
end type
type(zz),allocatable::zv
allocate(zv)
allocate(zv%zv)
allocate(zv%zv%lll,source=tttx)
if (tttx%p(2)/=3) print *,101
if (zv%zv%lll%p(2)/=3) print *,'err-1'
k=0
select type(tttx)
type is(t2)
...
type zz
class(z),allocatable:: zv
end type
type(zz),allocatable::zv
allocate(zv)
allocate(zv%zv)
allocate(zv%zv%lll,source=tttx)
if (tttx%p(2)/=3) print *,5101
k=0
select type(w=>zv%zv)
type is(z)
select type(r=>w%lll)
