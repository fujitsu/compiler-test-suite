MODULE M
IMPLICIT NONE

TYPE T
 INTEGER::A=5
 CONTAINS
  PROCEDURE::UNFMTWRITE
  PROCEDURE::UNFMTREAD
  GENERIC::WRITE(UNFORMATTED)=>UNFMTWRITE
  GENERIC::READ(UNFORMATTED)=>UNFMTREAD
END TYPE T

TYPE T2
 TYPE(T),POINTER::OBJ1
 INTEGER::AA
 END TYPE T2
CONTAINS
     SUBROUTINE UNFMTWRITE (DTV, UNIT, IOSTAT, IOMSG)
         CLASS (T), INTENT(IN)    :: DTV
         INTEGER,                  INTENT(IN)    :: UNIT
         INTEGER,                  INTENT(OUT)   :: IOSTAT
         CHARACTER(LEN=*),         INTENT(INOUT) :: IOMSG

        WRITE (UNIT=UNIT, IOSTAT=IOSTAT, IOMSG=IOMSG) DTV%A+1
     END SUBROUTINE UNFMTWRITE


     SUBROUTINE UNFMTREAD(DTV, UNIT, IOSTAT, IOMSG)
         CLASS (T), INTENT(INOUT)  :: DTV
         INTEGER,                  INTENT(IN)     :: UNIT
         INTEGER,                  INTENT(OUT)    :: IOSTAT
         CHARACTER (LEN=*),        INTENT(INOUT)  :: IOMSG

        READ (UNIT=UNIT, IOSTAT=IOSTAT, IOMSG=IOMSG) DTV%A
DTV%A=DTV%A+1        
     END SUBROUTINE UNFMTREAD
END MODULE M



PROGRAM MAIN
USE M

IMPLICIT NONE


TYPE(T2)::OBJW,OBJR
TYPE(T),TARGET::OBJ11,OBJ22

OBJW%OBJ1=>OBJ11
OBJR%OBJ1=>OBJ22


 OPEN (UNIT=71, FILE='fort.82', FORM='UNFORMATTED')
     WRITE(UNIT=71)OBJW%OBJ1
  
     CLOSE (UNIT=71)


 OPEN (UNIT=77, FILE='fort.82', FORM='UNFORMATTED')
   READ (UNIT=77)OBJR%OBJ1
     CLOSE (UNIT=77,status='delete')


IF( OBJR%OBJ1%A.EQ. 7)THEN
 PRINT*,'PASS'
ELSE
 PRINT*,'ERROR'
END IF
 
END PROGRAM

