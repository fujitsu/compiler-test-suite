...
program main
  USE m
   TYPE (ty) :: t1(7),t2(7)
   open(unit=71, file='fort.28',form='FORMATTED')     
  write(unit=71,*),t1([4,3,5,2])
        close(unit=71)

        open(unit=77, file='fort.28',form='FORMATTED')
   read(unit=77,*)t2([4,3,5,2])
close(unit=77,status='delete')

        if(any(t2([4,3,5,2])%x .ne.11))print*,101
        print*,'pass'
End program
