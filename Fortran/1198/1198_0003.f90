     ...
     contains
       subroutine s2(var,war,xar)
         ...
         class (b), intent(out) :: war
         class (b), intent(out) :: xar(:)
...
subroutine ss
     ...
     type is(ext)
       ...
       write(11,"(z16.16)") loc(p%name)
       write(12,"(z16.16)") loc(p%na)
       k=1
     end select
     if (k/=1) print *,1002
     k=0
     ...
     type is(ext)
       allocate(p  %name,source= a)
       allocate(p  %na,source= a)
       write(13,"(z16.16)") loc(p  %name)
       write(14,"(z16.16)") loc(p  %na)
       k=1
     end select
     if (k/=1) print *,1002
     end select
    ...
    if (.not.allocated(v1)) print *,8292
     k=0
      select type(v1)
       type is(e)
         if (v1%dk/=1) print *,656
         if (allocated(v1%ma)) print *,64521
         if (allocated(v1%mame)) print *,64531
         if (allocated(v1%qa  )) print *,64541
       k=1
     end select
     if (k/=1) print *,1022
