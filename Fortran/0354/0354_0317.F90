...
#define N (16)
#define DATA_TYPE complex(kind=4)
...
subroutine sub_init(ans,res,r,s) 
  ...
  do i=1, N
     do j=1, N
        r(i,j) = cmplx(sin(1.1*(i+j)),cos(0.3*i))
        s(i,j) = cmplx(cos(1.1*(i-j)),cos(0.3*j))
     enddo
  end do
  ans = cmplx(-1,-1)
  res = ans
...
subroutine sub_cmp(ans, res)
  ...
  do j=1,N
     do i=1,N
        if ((.not.is_equal_r4(real(ans(i,j)), real(res(i,j)))).or. &
             (.not.is_equal_r4(imag(ans(i,j)), imag(res(i,j))))) then
