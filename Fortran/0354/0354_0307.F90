#define PERMISSIBLE_ERROR_R4 (1.0E-6)
#define PERMISSIBLE_ERROR_R8 (1.0E-13)

#define N (16)
#define DATA_TYPE real(kind=4)

subroutine sub_test(ans,r,s,list)
  integer(kind=4)::i,j
  integer::list(N)
  DATA_TYPE,dimension(N,N)::ans,r,s
  do j=1,N
     do i=1,N
        r(list(i),j) = r(list(i),j) + s(i,j)
     end do
  end do
  ans = r
end subroutine sub_test

subroutine sub(ans,r,s,list)
  integer(kind=4)::i,j
  integer::list(N)
  DATA_TYPE,dimension(N,N)::ans,r,s
  do j=1,N
     do i=1,N
        r(list(i),j) = r(list(i),j) + s(i,j)
     end do
  end do
  ans = r
end subroutine sub

subroutine sub_init(ans,res) 
  DATA_TYPE,dimension(N,N)::ans,res
  ans = -1
  res = ans
end subroutine sub_init
subroutine sub_init2(r,s) 
  integer(kind=4)::i,j
  DATA_TYPE,dimension(N,N)::r,s
  do i=1, N
     do j=1, N
        r(i,j) = sin(1.1*(i+j))
        s(i,j) = cos(1.1*(i-j))
     enddo
  end do
end subroutine sub_init2

subroutine sub_cmp(ans, res)
  integer(kind=4)::i,j
  DATA_TYPE,dimension(N,N)::ans,res
  do j=1,N
     do i=1,N
        if (.not.is_equal_r4(ans(i,j), res(i,j))) then
           print *, "NG:", j,i, ans(i,j), res(i,j)
           exit
        end if
     end do
  end do
contains
  logical function is_equal_r4(a, b)
    real(kind=4)::a,b
    if ( a.eq.b ) then
       is_equal_r4 = .true.
    else if ( a.eq.0 ) then
       is_equal_r4 = ( b .lt. PERMISSIBLE_ERROR_R4) 
    else 
       is_equal_r4 = ( abs(a-b)/abs(a) .lt. PERMISSIBLE_ERROR_R4 )
    end if
  end function is_equal_r4

  logical function is_equal_r8(a, b)
    real(kind=8)::a,b
    if ( a.eq.b ) then
       is_equal_r8 = .true.
    else if ( a.eq.0 ) then
       is_equal_r8 = ( b .lt. PERMISSIBLE_ERROR_R8) 
    else 
       is_equal_r8 = ( abs(a-b)/abs(a) .lt. PERMISSIBLE_ERROR_R8 )
    end if
  end function is_equal_r8
end subroutine sub_cmp


program main
  DATA_TYPE,dimension(N,N)::ans,res,r,s
  integer::list(N)
  data list/16,8,1,9,2,10,3,11,4,12,5,13,6,14,7,15/
  call sub_init(ans,res)
  call sub_init2(r,s)
  call sub_test(ans, r,s,list)
  call sub_init2(r,s)
  call sub(res, r,s,list)
  call sub_cmp(ans, res)
  print *, "OK"
end program main
