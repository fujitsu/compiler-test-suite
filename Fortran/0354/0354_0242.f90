...
#define NJJ (16)
#define NKK (24)
#define PERMISSIBLE_ERROR (1.0E-14)
...
subroutine sub_test(r,s,II,JJ,KK)
  ...
  real(kind=8),dimension(NK,NJ,NI)::r,s

  do i =1,II
     do j=1,JJ
        do k=1,KK
           r(k,j,i) = s(k,j,i) * i + j * k /1.1_8
           s(k,j,i) = s(k,j,i) - j + k * 1.1_8
        end do
     end do
  end do
...
subroutine sub_init(r,s) 
  ...
  do i =1,NI
     do j=1,NJ
        do k=1,NK
           r(k,j,i) = 0.0_8
           s(k,j,i) = real(i)/NI*1.1_8 + real(j)/NJ * real(k)/NK - 1.0_8
        end do
     end do
  end do
...
subroutine sub_cmp(p,q,is_ok,II,JJ,KK)
  integer(kind=4)::II,JJ,KK
  real(kind=8),dimension(NK,NJ,NI)::p,q
...
program main
  ...
  real(kind=8),dimension(NK,NJ,NI)::r_ans,r,s
  logical::is_ok
  is_ok = .true.
