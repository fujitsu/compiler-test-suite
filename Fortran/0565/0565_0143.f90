...
Allocate(objMain%obj0(3),objMain%obj0P(3))

do k = 1,3
Allocate(objMain%obj0(k)%obj1(2),objMain%obj0(k)%obj2(2))
Allocate(objMain%obj0P(k)%obj1(2),objMain%obj0P(k)%obj2(2))
 do m=1,2
Allocate(objMain%obj0(k)%obj1(m)%ii1(2,2))
Allocate(objMain%obj0(k)%obj1(m)%cmp1(2,2))
Allocate(objMain%obj0(k)%obj1(m)%ll1(2,2))
...
call s1(objMain%obj0(1:3:2),objMain%obj0P(1:3:2))
...
do k=1,2
  do m=1,2
    ...
    if(.NOT. Allocated(dmy(k)%obj1(m)%ch1)) print *,103,k,m
    ...
    dmy2(k)%obj1(m)%ii1 = 60
    dmy2(k)%obj2(m)%ii2 = 60
    dmy2(k)%obj1(m)%ch1 = '60'
    dmy2(k)%obj2(m)%ch2 = '60'
    dmy2(k)%obj1(m)%cmp1 = (60.0,60.0)
    dmy2(k)%obj2(m)%cmp2 = (60.0,60.0)
    dmy2(k)%obj1(m)%ll1 = .FALSE.
    dmy2(k)%obj2(m)%ll2 = .FALSE.
