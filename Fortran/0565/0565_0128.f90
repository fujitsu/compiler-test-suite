...
call s1(obj1(1:3:2))
if((.NOT. Allocated(obj1(3)%ii)) .or. (.NOT. Allocated(obj1(3)%rr))  .or. (.NOT. Allocated(obj1(3)%ch)) .or. (.NOT. Allocated(obj1(3)%cmp) ) .or. (.NOT. Allocated(obj1(3)%ll))) then
print *,201
else
 if(ANY(obj1(1)%ii .ne. 21)) print *,202, obj1(1)%ii
 if(ANY(obj1(3)%ii .ne. 23)) print *,203, obj1(3)%ii
 if(ANY(obj1(1)%rr .ne. 21.0)) print *,204, obj1(1)%rr
 if(ANY(obj1(3)%rr .ne. 23.0)) print *,205, obj1(3)%rr
 if(ANY(obj1(1)%ch .ne. '21')) print *,206, obj1(1)%ch
 if(ANY(obj1(3)%ch .ne. '23')) print *,207, obj1(3)%ch
 if(ANY(obj1(1)%cmp .ne. (21.0,21.0))) print *,208, obj1(1)%cmp
 if(ANY(obj1(3)%cmp .ne. (23.0,23.0))) print *,209, obj1(3)%cmp
 if(ANY(obj1(1)%ll .neqv. .TRUE.)) print *,210, obj1(1)%ll
 if(ANY(obj1(3)%ll .neqv. .FALSE.)) print *,211, obj1(3)%ll
end if

if((.NOT. Allocated(obj1(1)%ch1)) .or. (.NOT. Allocated(obj1(3)%ch1))) then
print *,301
else
