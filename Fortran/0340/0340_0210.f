     ...
     &        a30(10,10)
      ...
      data    result/(710000.000,409800.000)/
      ...
      do j=1,10
       do i=1,10
         a00(i,j)=x0(i,j)*x1(i,j)
     &         +x2(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a01(i,j)=x3(i,j)*x4(i,j)
     &         +x5(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a02(i,j)=y0(i,j)*y1(i,j)
     &         +y2(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a03(i,j)=y3(i,j)*y4(i,j)
     &         +y5(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a04(i,j)=z0(i,j)*z1(i,j)
     &         +z2(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a05(i,j)=z3(i,j)*z4(i,j)+
     &         z5(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a06(i,j)=x1(i,j)*x2(i,j)
     &         +x3(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a07(i,j)=x4(i,j)*x5(i,j)
     &         +x0(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a08(i,j)=y1(i,j)*y2(i,j)
     &         +y3(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a09(i,j)=y4(i,j)*y5(i,j)
     &         +y0(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a10(i,j)=z1(i,j)*z2(i,j)
     &         +z3(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a11(i,j)=z4(i,j)*z5(i,j)
     &         +z0(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a12(i,j)=x2(i,j)*x3(i,j)
     &         +x4(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a13(i,j)=x5(i,j)*x0(i,j)
     &         +x1(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a14(i,j)=y2(i,j)*y3(i,j)
     &         +y4(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a15(i,j)=y5(i,j)*y0(i,j)
     &         +y1(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a16(i,j)=z2(i,j)*z3(i,j)
     &         +z4(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a17(i,j)=z5(i,j)*z0(i,j)
     &         +z1(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a18(i,j)=x3(i,j)*x4(i,j)
     &         +x5(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a19(i,j)=x0(i,j)*x1(i,j)
     &         +x2(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a20(i,j)=y3(i,j)*y4(i,j)
     &         +y5(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a21(i,j)=y0(i,j)*y1(i,j)
     &         +y2(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a22(i,j)=z3(i,j)*z4(i,j)
     &         +z5(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a23(i,j)=z0(i,j)*z1(i,j)
     &         +z2(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a24(i,j)=x4(i,j)*x5(i,j)
     &         +x0(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a25(i,j)=x1(i,j)*x2(i,j)
     &         +x3(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a26(i,j)=y4(i,j)*y5(i,j)
     &         +y0(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a27(i,j)=y1(i,j)*y2(i,j)
     &         +y3(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a28(i,j)=z4(i,j)*z5(i,j)
     &         +z0(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a29(i,j)=z1(i,j)*z2(i,j)
     &         +z3(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
         a30(i,j)=x5(i,j)*y5(i,j)
     &         +z5(i,j)*(x1(i,j)+x2(i,j)+x3(i,j))
       enddo
      enddo
      res=0
