...
PROGRAM main
 ...
 INTERFACE
     SUBROUTINE FOO(dmy1, dmy2)
        IMPORT ty, ty3
        CLASS(ty3), INTENT(OUT) :: dmy1(2,2)
        CLASS(ty), INTENT(OUT) :: dmy2(:,:)
     END SUBROUTINE
     SUBROUTINE FOO1(dmy1)
        IMPORT ty3
        CLASS(ty3), INTENT(OUT) :: dmy1(2,2)
     END SUBROUTINE
 END INTERFACE
 ...
 ty3_obj(1, 1)%cmp_ty1(2)%cmp_ty%ii = 300
 ty3_obj(1, 1)%kk = 400

 ty3_obj(2, 1)%cmp_ty1%jj = 500
 ty3_obj(2, 1)%cmp_ty1(1)%cmp_ty%ii = 600
 ty3_obj(2, 1)%cmp_ty1(2)%cmp_ty%ii = 700
 ty3_obj(2, 1)%kk = 800

 ty3_obj(1, 2)%cmp_ty1%jj = 900
 ty3_obj(1, 2)%cmp_ty1(1)%cmp_ty%ii = 1000
 ty3_obj(1, 2)%cmp_ty1(2)%cmp_ty%ii = 1100
 ty3_obj(1, 2)%kk = 1200

 ty3_obj(2, 2)%cmp_ty1%jj = 1300
 ty3_obj(2, 2)%cmp_ty1(1)%cmp_ty%ii = 1400
 ty3_obj(2, 2)%cmp_ty1(2)%cmp_ty%ii = 1500
 ty3_obj(2, 2)%kk = 1600
 CALL FOO1(ty3_obj)
END PROGRAM
