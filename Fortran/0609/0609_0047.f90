...
CONTAINS
    ...
    SUBROUTINE final_ty3(dmy)
        ...
        IF ( dmy(1)%ll .EQ. (100*counter) ) THEN
           PRINT*, counter,' 4th-destructor call PASS ', dmy(1)%ll
        ELSE
           PRINT*, counter,' 4th-destructor call FAIL ', dmy(1)%ll
        END IF
    END SUBROUTINE
END MODULE
...
PROGRAM main
 ...
 ALLOCATE(ty_obj)
 ty3_obj(2, 5)%ll = 100
 ty3_obj(2, 5)%cmp_ty1%jj = 200
 ty3_obj(2, 5)%cmp_ty1(1)%cmp_ty%ii = 300
 ty3_obj(2, 5)%cmp_ty1(2)%cmp_ty%ii = 400
 ty3_obj(2, 5)%kk = 500

 ty3_obj(4, 5)%cmp_ty1%jj = 600
 ty3_obj(4, 5)%cmp_ty1(1)%cmp_ty%ii = 700
 ty3_obj(4, 5)%cmp_ty1(2)%cmp_ty%ii = 800
 ty3_obj(4, 5)%kk = 900
 ty_obj%ii = 1000
 CALL FOO(ty3_obj(2:4:2, 5:5:1), ty_obj)
END PROGRAM
