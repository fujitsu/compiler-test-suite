     ...
     contains
      ...
      select type(var)
       type is(e)
         ...
         if (same_type_as(var%qa,base_x)) print *,7452
         if (same_type_as(var%mame,ext_x)) print *,7453
         k=1
       end select
       if (k/=1) print *,3628
       end subroutine s2
     ...
     class (b   ), allocatable :: v
      ...
      select type(v)
       type is(e)
         allocate(ext::v%ma(2))
         allocate(ext::v%qa(2))
         allocate(ext::v%mame(2))
     k=0
     select type(p=>v%ma(2))
     type is(ext)
     ...
     k=0
      select type(v)
       type is(e)
         ...
         if (allocated(v%mame)) print *,64531
         if (allocated(v%qa  )) print *,64541
       k=1
     end select
     if (k/=1) print *,1022
end 
