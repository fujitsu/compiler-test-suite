module mod
interface 
  module subroutine s11
  end
  module subroutine s12
  end
  module subroutine s13
  end
  module subroutine s14
  end
end interface
type :: ty
integer :: k
end type
  class(ty),pointer :: p
  class(ty),pointer :: p2
contains
subroutine s21
  allocate(p)
  allocate(ty::p2)
  if (.not.extends_type_of(p2,p)) print *,1011
  if (.not.same_type_as(p2,p)) print *,101211
  m=0
  select type(p)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10131
  m=0
  select type(p2)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10141
end 
end

submodule (mod) smod
  type,extends(ty)::ty2
    integer::k1
  end type
  class(ty2),pointer :: q
  class(ty2),pointer :: q2
contains
subroutine s22
  allocate(q)
  allocate(ty2::q2)
  if (.not.extends_type_of(q2,q)) print *,1001
  if (.not.same_type_as(q2,q)) print *,1002
  m=0
  select type(q)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,1113
  m=0
  select type(q2)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,1014
  allocate(p)
  allocate(ty::p2)
  if (.not.extends_type_of(p2,p)) print *,1011
  if (.not.same_type_as(p2,p)) print *,1012
  m=0
  select type(p)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10131
  m=0
  select type(p2)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10141
  allocate(p)
  allocate(ty2::p2)
  if (.not.extends_type_of(p2,p)) print *,1011
  if (     same_type_as(p2,p)) print *,101212
  m=0
  select type(p)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10131
  m=0
  select type(p2)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,10141
end 
end 

 
submodule (mod:smod) smod2
  type,extends(ty2)::ty3
    integer::k2
  end type
  class(ty3),pointer :: r
  class(ty3),pointer :: r2
contains
subroutine s23
  allocate(r)
  allocate(ty3::r2)
  if (.not.extends_type_of(r2,r)) print *,1001
  if (.not.same_type_as(r2,r)) print *,1002
  m=0
  select type(r)
   type is(ty3)
     m=1
  end select
  if (m/=1) print *,1113
  m=0
  select type(r2)
   type is(ty3)
     m=1
  end select
  if (m/=1) print *,1014
  allocate(q)
  allocate(ty2::q2)
  if (.not.extends_type_of(q2,q)) print *,1001
  if (.not.same_type_as(q2,q)) print *,1002
  m=0
  select type(q)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,1113
  m=0
  select type(q2)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,1014
  allocate(p)
  allocate(ty::p2)
  if (.not.extends_type_of(p2,p)) print *,1011
  if (.not.same_type_as(p2,p)) print *,1012
  m=0
  select type(p)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10131
  m=0
  select type(p2)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,12131

  allocate(p)
  allocate(ty2::p2)
  if (.not.extends_type_of(p2,p)) print *,1011
  if (     same_type_as(p2,p)) print *,101256
  m=0
  select type(p)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10131
  m=0
  select type(p2)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,12131

  allocate(q)
  allocate(ty3::q2)
  if (.not.extends_type_of(q2,q)) print *,1001
  if (     same_type_as(q2,q)) print *,10023
  m=0
  select type(q)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,1113
  m=0
  select type(q2)
   type is(ty3)
     m=1
  end select
  if (m/=1) print *,1014
  allocate(p)
  allocate(ty3::p2)
  if (.not.extends_type_of(p2,p)) print *,1011
  if (     same_type_as(p2,p)) print *,10124
  m=0
  select type(p)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10131
  m=0
  select type(p2)
   type is(ty3)
     m=1
  end select
  if (m/=1) print *,12131
end
end
 
submodule (mod:smod2) smod3
  type,extends(ty3)::ty4
    integer::k3
  end type
  class(ty4),pointer :: u
  class(ty4),pointer :: u2
contains
subroutine s24
  allocate(u)
  allocate(ty4::u2)
  if (.not.extends_type_of(u2,u)) print *,1001
  if (.not.same_type_as(u2,u)) print *,1002
  m=0
  select type(u)
   type is(ty4)
     m=1
  end select
  if (m/=1) print *,1113
  m=0
  select type(u2)
   type is(ty4)
     m=1
  end select
  if (m/=1) print *,1014
  allocate(r)
  allocate(ty3::r2)
  if (.not.extends_type_of(r2,r)) print *,1001
  if (.not.same_type_as(r2,r)) print *,1002
  m=0
  select type(r)
   type is(ty3)
     m=1
  end select
  if (m/=1) print *,1113
  m=0
  select type(r2)
   type is(ty3)
     m=1
  end select
  if (m/=1) print *,1014
  allocate(q)
  allocate(ty2::q2)
  if (.not.extends_type_of(q2,q)) print *,1001
  if (.not.same_type_as(q2,q)) print *,1002
  m=0
  select type(q)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,1113
  m=0
  select type(q2)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,1014
  allocate(p)
  allocate(ty::p2)
  if (.not.extends_type_of(p2,p)) print *,1011
  if (.not.same_type_as(p2,p)) print *,1012
  m=0
  select type(p)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10131
  m=0
  select type(p2)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,1213121

  allocate(p)
  allocate(ty2::p2)
  if (.not.extends_type_of(p2,p)) print *,1011
  if (     same_type_as(p2,p)) print *,101256
  m=0
  select type(p)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10131
  m=0
  select type(p2)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,12131

  allocate(q)
  allocate(ty3::q2)
  if (.not.extends_type_of(q2,q)) print *,1001
  if (     same_type_as(q2,q)) print *,10023
  m=0
  select type(q)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,1113
  m=0
  select type(q2)
   type is(ty3)
     m=1
  end select
  if (m/=1) print *,1014
  allocate(p)
  allocate(ty3::p2)
  if (.not.extends_type_of(p2,p)) print *,1011
  if (     same_type_as(p2,p)) print *,10124
  m=0
  select type(p)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10131
  m=0
  select type(p2)
   type is(ty3)
     m=1
  end select
  if (m/=1) print *,12131

  allocate(p)
  allocate(ty4::p2)
  if (.not.extends_type_of(p2,p)) print *,1011
  if (     same_type_as(p2,p)) print *,101256
  m=0
  select type(p)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,10131
  m=0
  select type(p2)
   type is(ty4)
     m=1
  end select
  if (m/=1) print *,12131

  allocate(q)
  allocate(ty4::q2)
  if (.not.extends_type_of(q2,q)) print *,1001
  if (     same_type_as(q2,q)) print *,10023
  m=0
  select type(q)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,1113
  m=0
  select type(q2)
   type is(ty4)
     m=1
  end select
  if (m/=1) print *,1014
  allocate(r)
  allocate(ty4::r2)
  if (.not.extends_type_of(r2,r)) print *,1001
  if (     same_type_as(r2,r)) print *,10029
  m=0
  select type(r)
   type is(ty3)
     m=1
  end select
  if (m/=1) print *,1113
  m=0
  select type(r2)
   type is(ty4)
     m=1
  end select
  if (m/=1) print *,1014
end
  module subroutine s11
  allocate(p)
  allocate(ty4::p2)
  if (.not.extends_type_of(p2,p)) print *,1011
  if (same_type_as(p2,p)) print *,101234
  m=0
  select type(p)
   type is(ty)
     m=1
  end select
  if (m/=1) print *,1013
  m=0
  select type(p2)
   type is(ty4)
     m=1
  end select
  if (m/=1) print *,1014
end
  module subroutine s12
  allocate(q)
  allocate(ty4::q2)
  if (.not.extends_type_of(q2,q)) print *,1001
  if (same_type_as(q2,q)) print *,1002
  m=0
  select type(q)
   type is(ty2)
     m=1
  end select
  if (m/=1) print *,1113
  m=0
  select type(q2)
   type is(ty4)
     m=1
  end select
  if (m/=1) print *,1014
end
  module subroutine s13
  allocate(r)
  allocate(ty4::r2)
  if (.not.extends_type_of(r2,r)) print *,1021
  if (same_type_as(r2,r)) print *,1022
  m=0
  select type(r)
   type is(ty3)
     m=1
  end select
  if (m/=1) print *,1213
  m=0
  select type(r2)
   type is(ty4)
     m=1
  end select
  if (m/=1) print *,1214
end
  module subroutine s14
  allocate(u)
  allocate(ty4::u2)
  if (.not.extends_type_of(u2,u)) print *,1021
  if (.not.same_type_as(u2,u)) print *,1022
  m=0
  select type(u)
   type is(ty4)
     m=1
  end select
  if (m/=1) print *,1313
  m=0
  select type(u2)
   type is(ty4)
     m=1
  end select
  if (m/=1) print *,1314
call s21
call s22
call s23
call s24
end
end 

use mod
call s11
call s12
call s13
call s14
call s21
print*,"sngg258o : pass"
end

