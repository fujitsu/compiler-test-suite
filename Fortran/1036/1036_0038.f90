...
call sub(t%w1%ww1%w(:,:),vp%w1%ww1%w(:,:),i,f)
call sub2(vp%w1%ww1%w(:,:),i,f,t%w1%ww1%w(:,:))
call suba(t%w1%ww1%w(:,2),vp%w1%ww1%w,i,f)
call subb(vp%w1%ww1%w(:,2),i,f,t%w1%ww1%w)
end subroutine
...
subroutine suba(y,w,i,f)
integer w(*),f(*),y(*)
  if (any((/f(1:25)/)/=(/(k,k=1,25)/)))print *,'error-4',i
  if (any((/w(1:25)/)/=(/(k,k=1,25)/)))print *,'error-3',i
if (i==1) then
  if (any((/y(1:5)/)/=(/(k,k=6,10)/)))print *,'error-9',i
else 
  if (any((/y(1:3)/)/=(/6,8,10/)))print *,'error-d',i
endif
end subroutine
subroutine subb(w,i,f,y)
integer w(*),f(*),y(*)
  if (any((/f(1:25)/)/=(/(k,k=1,25)/)))print *,'error-4',i
