 ...
 type(x)::z
  forall(j=j1:k1)
   forall(i=1:3,z%a(i)>j.and.z%b(i)>j)
     z%a(j+i:1+i)=z%a(i:i)
     z%b(j+i:1+i)=z%b(i:i)
     z%aa(j+i:1+i)=z%aa(i:i)
     z%ba(j+i:1+i)=z%ba(i:i)
   end forall
   end forall
 ...
 type(x)::z
  forall(j=j1:k1)
   forall(i=1:3,z%d1(i).and.z%d2(i))
     z%d1(i+j:1+i)=.not.z%d1(i+1:1+i)
     z%d2(i+j:1+i)=.not.z%d2(i+1:1+i)
     z%d3(i+j:1+i)=.not.z%d3(i+1:1+i)
     z%d4(i+j:1+i)=.not.z%d4(i+1:1+i)
   end forall
   end forall
...
use m1
 type(x)::z
  ...
  forall(j=j1:k1)
     ...
     z%e2(i+1:j+i)=.not.z%e2(i+1:1+i)
     z%e3(i+1:j+i)=.not.z%e3(i+1:1+i)
   end forall
...
use m1
 type(x)::z
   ...
   forall(i=1:3,z%c3(i).and.z%c2(i))
     z%c2(i+1:1+i)=.not.z%c2(i+1:1+i)
   end forall
   forall(i=1:3,z%c3(i).and.z%c4(i))
     z%c3(i+1:1+i)=.not.z%c3(i+1:1+i)
   end forall
   forall(i=1:3,z%c4(i).and.z%c4(i))
