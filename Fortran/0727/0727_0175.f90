...
use m1
 type(x)::z
   ...
   forall(m=j1:j2,&
     ...
     where(z%a(i:i,j:k,k:j,m:m)>1.and.z%b(1+i-1:-1+i+1,k:j,j:k,m:m)>1)
...
use m1
 type(x)::z
   ...
   forall(m=j1:j2,&
           z%a(i,j,k,m)>1.and.z%b(i,j,k,m)>1)
     where(z%d1(i:i,j:k,k:j,m:m).and.z%d2(1+i-1:-1+i+1,k:j,j:k,m:m))
...
use m1
 type(x)::z
   forall(ix=k1:j2,jx=j1:k1,kx=j1:k1,mx=j1:j2,&
           z%f1(ix,jx,kx,mx).and.z%f2(ix,jx,kx,mx))
   forall(i=k1:j2)
   forall(j=j1:k1)
   forall(k=j1:k1)
   forall(m=j1:j2,&
