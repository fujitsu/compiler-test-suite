 ...
 module m1
  integer,pointer::a11,a21(:,:),a12,a22(:,:)
  type x
  integer,pointer::a11,a21(:,:),a12,a22(:,:)
  end type
  contains
  subroutine ss1(p11,p21,p12,p22)
  integer,pointer::p11,p21(:,:),p12,p22(:,:)
 ...
 write(24,*)p21(:,:)
 write(24,*)p22(:,:)
 write(24,*)((p21(i1,i2),i1=1,2),i2=1,3)
 write(24,*)((p22(i3,i4),i3=1,2),i4=1,3)
  call chkio(p22,p11)
  contains
  subroutine chkio(a22,a11)
  integer,pointer::a22(:,:),a11
  rewind 24
  read(24,*) a22(:,1)
  if (a22(2,1)/=20)write(6,*) "NG"
  if (a22(2,1)/=20)write(6,*) "NG"
do i=1,6
  read(24,*) a22(:,:)
  if (any(abs((/a22/)- (/reshape((/1,2,3,4,5,6/),(/2,3/))/))>0.0001))write(6,*) "NG"
end do
  rewind 24
  read(24,*) a21
  if (a11/=20)write(6,*) "NG"
 end subroutine
  subroutine sy1(xx)
  type(x)::xx
  ...
  integer,pointer::ps1,pa1(:,:),ps2,pa2(:,:)
  if (ps1/=10)write(6,*) "NG"
  if (ps2/=10)write(6,*) "NG"
  ...
  integer::ps1,pa1(2,3),ps2,pa2(4,6)
  if (ps1/=10)write(6,*) "NG"
  if (ps2/=10)write(6,*) "NG"
  if (any(pa1/=reshape((/1,2,3,4,5,6/),(/2,3/))))write(6,*) "NG"
  if (any(pa2(::2,::2)/=reshape((/1,2,3,4,5,6/),(/2,3/))))write(6,*) "NG"
