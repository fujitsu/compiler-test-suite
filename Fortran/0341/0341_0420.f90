  ...
  do i = 1, m
    do j = 1,n-2
      do L = 4,k
        ...
        answer2 = R2(L,j) * mul
        answer3 = R3(L,j) * mul
        RR1(L-3,j+2)=RR1(L-3,j+2) + A(j,L)*answer1
        RR2(L,j+1)=RR2(L,j+1) + B(j,L)+answer2
        RR3(L-2,j)=RR3(L-2,j) + answer3
      enddo
    enddo
  enddo
  do i = 1, 10
    ...
    enddo
  enddo
  if (RR1_answer .ne. 705700.000) then
     write(6,*) "NG"
  else if (RR2_answer .ne. 105760.000) then
     write(6,*) "NG"
