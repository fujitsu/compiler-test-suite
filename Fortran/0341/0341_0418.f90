subroutine sub1(A,B,R1,R2,R3,m,n,k)
  ...
  do i = 1, m
    do j = 1,n
      do L = 1,k
        answer1(3,5) = R1(L,j) * mul
        answer2(2,7) = R2(L,j) * mul
        answer3(4,10) = R3(L,j) * mul
        RR1(L,j)=RR1(L,j) + A(j,L)*answer1(3,5)
        RR2(L,j)=RR2(L,j) + B(j,L)+answer2(2,7)
        RR3(L,j)=RR3(L,j) + answer3(4,10)
      enddo
    enddo
  enddo
  do i = 1, 10
