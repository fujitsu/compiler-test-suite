...
integer(1),parameter::n1=2,n2=2,n3=2
  ...
  forall (i=1:n3,j=1:n2,k=n1:1:-1)
    where(mask(k,j,:))
      a(k,j,:)=z((/'abc'/),reshape((/1,2,3,4/),(/2,2/)), &
                 (/(1.0,2.0),(-1.0,-2.0)/))
    end where
  end forall
  ...
  if (any(abs(b(1,1,1)%za3-(/(1.0,2.0),(-1.0,-2.0)/)).gt.1.0e-8)) &
     write(6,*) "NG"
  ...
  if (any(abs(b(2,2,1)%za3-(/(1.0,2.0),(-1.0,-2.0)/)).gt.1.0e-8)) &
     write(6,*) "NG"
  ...
  if (any(abs(b(2,1,2)%za3-(/(1.0,2.0),(-1.0,-2.0)/)).gt.1.0e-8)) &
     write(6,*) "NG"
  ...
  if (any(abs(b(1,2,2)%za3-(/(1.0,2.0),(-1.0,-2.0)/)).gt.1.0e-8)) &
     write(6,*) "NG"
