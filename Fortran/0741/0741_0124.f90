...
logical(1),dimension(:,:,:),allocatable::mask
  ...
  if (any(lbound(aa(1,2,1)%za2)/=(/11/))) write(6,*) "NG"
  if (any(ubound(aa(1,2,1)%za2)/=(/12/))) write(6,*) "NG"
  if (any(lbound(aa(1,3,1)%za2)/=(/12/))) write(6,*) "NG"
  if (any(ubound(aa(1,3,1)%za2)/=(/13/))) write(6,*) "NG"
  if (any(lbound(aa(1,4,1)%za2)/=(/14/))) write(6,*) "NG"
  if (any(ubound(aa(1,4,1)%za2)/=(/15/))) write(6,*) "NG"
  if (any(aa(1,1,1)%za2/=(/1,2/))) write(6,*) "NG"
  if (any(aa(1,2,1)%za2/=(/1,2/))) write(6,*) "NG"
  if (any(aa(1,3,1)%za2/=(/2,3/))) write(6,*) "NG"
  if (any(aa(1,4,1)%za2/=(/4,5/))) write(6,*) "NG"
