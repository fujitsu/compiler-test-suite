 ...
 contains
  ...
  subroutine sub()
   ...
   type(y),allocatable::a(:)
    ...
    if (any(a(1)%yy(1)%za/=(/2,3/))) write(6,*) "NG"
    if (any(a(1)%yy(2)%za/=(/2,3/))) write(6,*) "NG"
    if (any(a(2)%yy(1)%za/=(/4,5/))) write(6,*) "NG"
    if (any(a(2)%yy(2)%za/=(/4,5/))) write(6,*) "NG"
    if (any(a(2)%yy(3)%za/=(/4,5/))) write(6,*) "NG"
  end subroutine
