 ...
 contains
  ...
  subroutine sub()
   ...
   type(y),allocatable::a(:,:)
    ...
    if (any(a(1,1)%yy(1)%za/=(/3,4/))) write(6,*) "NG"
    if (any(a(1,1)%yy(2)%za/=(/3,4/))) write(6,*) "NG"
    if (any(a(2,2)%yy(1)%za/=(/3,4/))) write(6,*) "NG"
    if (any(a(2,2)%yy(2)%za/=(/3,4/))) write(6,*) "NG"
    if (any(a(2,2)%yy(3)%za/=(/3,4/))) write(6,*) "NG"
  end subroutine
