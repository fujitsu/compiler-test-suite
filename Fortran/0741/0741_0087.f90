 ...
 contains
  ...
  subroutine sub()
   ...
   logical(1),parameter::lo(2)=(/.true.,.true./)
    ...
    if (any(a(1)%yy(1)%za/=(/5,6/))) write(6,*) "NG"
    if (any(a(1)%yy(2)%za/=(/5,6/))) write(6,*) "NG"
    if (any(a(1)%yy(3)%za/=(/5,6/))) write(6,*) "NG"
    if (any(a(2)%yy(1)%za/=(/5,6/))) write(6,*) "NG"
    if (any(a(2)%yy(2)%za/=(/5,6/))) write(6,*) "NG"
    if (any(a(2)%yy(3)%za/=(/5,6/))) write(6,*) "NG"
  end subroutine
