...
do nm=2,nn
     allocate(x3::xv(nm))
             write (571,"(z16.16)") loc(xv )
     select type(y=>xv)
      type is(x3)
        allocate(y(nm)%xn(nm),source='a')
             write (572,"(z16.16)") loc(y(nm)%xn )
     end select
     allocate(x3::yv(nm))
             write (561,"(z16.16)") loc(yv )
     select type(y=>yv)
      type is(x3)
        allocate(y(nm)%xn(nm),source='a')
             write (562,"(z16.16)") loc(y(nm)%xn )
        allocate(y(nm)%xna(nm),source='a')
             write (563,"(z16.16)") loc(y(nm)%xna)
     end select
    k=0
     select type(xv)
      type is(x3)
    ...
    select type(p=>xv(nm)%v)
      type is(e)
        k=1
             write (511,"(z16.16)") loc(xv(nm)%v   )
          allocate(p(nm)%n(nm),source=a)
             write (512,"(z16.16)") loc(xv(nm)%v(nm)%n   )
          allocate(p(nm)   %na(nm),source=a)
             write (513,"(z16.16)") loc(p(nm)   %na  )
          allocate(p(nm)   %nam(nm),source=a)
             write (514,"(z16.16)") loc(p(nm)   %nam )
     allocate(b3::p(nm)   %b2v(nm))
    kk=0
      select type(q=>p(nm)%b2v)
      type is(b3)
        kk=1
         write (541,"(z16.16)") loc(q      )
        ...
        kkk=1
              ...
              write (542,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (543,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,862
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (544,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (545,"(z16.16)") loc(r(nm) %w02  )
           end select
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (546,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (547,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,8624
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (548,"(z16.16)") loc(r(nm)%w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (549,"(z16.16)") loc(r(nm)%w02  )
           end select
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (550,"(z16.16)") loc(r(nm)%w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (551,"(z16.16)") loc(r(nm)%w02  )
           end select
        if (kkk/=1) print *,8664
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (552,"(z16.16)") loc(r(nm)%w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (553,"(z16.16)") loc(r(nm)%w02  )
           end select
    ...
    k=0
     select type(yv)
      ...
      allocate(ext::yv(nm)%v(nm))
             write (521,"(z16.16)") loc(yv(nm)%v(nm)%d1  )
      yv(nm)%v(nm)%d1(1)=1
    select type(p=>yv(nm)%v)
      type is(ext)
        k=1
          allocate(p(nm)   %n(nm),source=b)
             write (522,"(z16.16)") loc(p(nm)   %n   )
          allocate(p(nm)   %na(nm),source=b)
             write (523,"(z16.16)") loc(p(nm)   %na  )
     allocate(b3::p(nm)   %b2v(nm))
    kk=0
      select type(q=>p(nm)%b2v)
      type is(b3)
     ...
     select type(p=>xv)
      type is(x3)
        k=1
          ...
          if (p(nm)   %xna(nm)/='a') print *,728291,p(nm)   %xna(nm)
    end select
    if (k/=1) print *,867381
    k=0
 select type(yv)
