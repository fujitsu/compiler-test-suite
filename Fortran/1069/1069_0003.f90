...
do nm=2,nn
     allocate(x::xv(nm),yv(nm))
             write (231,"(z16.16)") loc(xv )
             write (232,"(z16.16)") loc(yv )
    k=0
select type(xv)
  type is(x)
    ...
    select type(p=>xv(nm)%v)
      type is(e)
        k=1
             write (211,"(z16.16)") loc(xv(nm)%v   )
          allocate(p(nm)%n(nm),source=a)
             write (212,"(z16.16)") loc(p(nm)       %n   )
          allocate(p(nm)   %na(nm),source=a)
             write (213,"(z16.16)") loc(p(nm)   %na  )
          allocate(p(nm)   %nam(nm),source=a)
             write (214,"(z16.16)") loc(p(nm)   %nam )
     allocate(b3::p(nm)   %b2v(nm))
    kk=0
      select type(q=>p(nm)%b2v)
      type is(b3)
        kk=1
         write (241,"(z16.16)") loc(q      )
        ...
        kkk=1
              ...
              write (242,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (243,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,862
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (244,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (245,"(z16.16)") loc(r(nm) %w02  )
           end select
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (246,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (247,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,8624
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (248,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (249,"(z16.16)") loc(r(nm) %w02  )
           end select
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (250,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (251,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,8664
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (252,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (253,"(z16.16)") loc(r(nm) %w02  )
           end select
...
select type(yv)
  type is(x )
    ...
    select type(p=>yv(nm)%v)
      type is(ext)
             write (221,"(z16.16)") loc(p       (nm)%d1  )
      p       (nm)%d1(1)=1
        k=1
          allocate(p(nm)   %n(nm),source=b)
             write (222,"(z16.16)") loc(p(nm)   %n   )
          allocate(p(nm)   %na(nm),source=b)
             write (223,"(z16.16)") loc(p(nm)   %na  )
     allocate(b3::p(nm)   %b2v(nm))
    kk=0
      select type(q=>p(nm)%b2v)
      type is(b3)
