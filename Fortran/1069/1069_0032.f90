...
do nm=2,nn
     allocate(xv(nm),yv(nm))
             write (2331,"(z16.16)") loc(xv )
             write (2332,"(z16.16)") loc(yv )
      allocate(e ::xv(nm)%v(nm))
    k=0
    select type(p=>xv(nm)%v)
      type is(e)
        k=1
             write (2311,"(z16.16)") loc(xv(nm)%v   )
          allocate(p(nm)%n(nm),source=a)
             write (2312,"(z16.16)") loc(xv(nm)%v(nm)%n   )
          allocate(p(nm)   %na(nm),source=a)
             write (2313,"(z16.16)") loc(p(nm)   %na  )
          allocate(p(nm)   %nam(nm),source=a)
             write (2314,"(z16.16)") loc(p(nm)   %nam )
     allocate(b3::p(nm)   %b2v(nm))
    kk=0
      select type(q=>p(nm)%b2v)
      type is(b3)
        kk=1
         write (2341,"(z16.16)") loc(q      )
        ...
        kkk=1
              ...
              write (2342,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2343,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,862
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (2344,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2345,"(z16.16)") loc(r(nm) %w02  )
           end select
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (2346,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2347,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,8624
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (2348,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2349,"(z16.16)") loc(r(nm) %w02  )
           end select
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (2350,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2351,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,8664
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (2352,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2353,"(z16.16)") loc(r(nm) %w02  )
           end select
    ...
    if (kk/=1) print *,6733
      ...
      allocate(ext::yv(nm)%v(nm))
             write (2321,"(z16.16)") loc(yv(nm)%v(nm)%d1  )
      yv(nm)%v(nm)%d1(1)=1
    k=0
      ...
      type is(ext)
        k=1
          allocate(p(nm)   %n(nm),source=b)
             write (2322,"(z16.16)") loc(p(nm)   %n   )
          allocate(p(nm)   %na(nm),source=b)
             write (2323,"(z16.16)") loc(p(nm)   %na  )
     allocate(b3::p(nm)   %b2v(nm))
    kk=0
      select type(q=>p(nm)%b2v)
      type is(b3)
