...
contains
     ...
     allocate(xv,yv)
             write (831,"(z16.16)") loc(xv )
             write (832,"(z16.16)") loc(yv )
      allocate(e ::xv%v)
    k=0
    select type(p=>xv%v)
      type is(e)
        k=1
             write (811,"(z16.16)") loc(xv%v   )
          allocate(p%n,source=a)
             write (812,"(z16.16)") loc(xv%v%n   )
          allocate(p   %na,source=a)
             write (813,"(z16.16)") loc(p   %na  )
          allocate(p   %nam,source=a)
             write (814,"(z16.16)") loc(p   %nam )
       end select
    if (k/=1) print *,6733

      allocate(ext::yv%v)
             write (821,"(z16.16)") loc(yv%v%d1  )
      yv%v%d1(1)=1
    k=0
    select type(p=>yv%v)
      type is(ext)
        k=1
          allocate(p   %n,source=b)
             write (822,"(z16.16)") loc(p   %n   )
          allocate(p   %na,source=b)
             write (823,"(z16.16)") loc(p   %na  )
    end select
    if (k/=1) print *,6738
