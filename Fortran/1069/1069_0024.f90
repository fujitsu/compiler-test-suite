...
contains
    ...
    select type(p=>xv(nn)%v)
      type is(e)
        k=1
             write (1711,"(z16.16)") loc(xv(nn)%v   )
          allocate(p(nn)%n(nn),source=a)
             write (1712,"(z16.16)") loc(xv(nn)%v(nn)%n   )
          allocate(p(nn)   %na(nn),source=a)
             write (1713,"(z16.16)") loc(p(nn)   %na  )
          allocate(p(nn)   %nam(nn),source=a)
             write (1714,"(z16.16)") loc(p(nn)   %nam )
       end select
    if (k/=1) print *,6733

      allocate(ext::yv(nn)%v(nn))
             write (1721,"(z16.16)") loc(yv(nn)%v(nn)%d1  )
      yv(nn)%v(nn)%d1(1)=1
    k=0
      ...
      type is(ext)
        k=1
          allocate(p(nn)   %n(nn),source=b)
             write (1722,"(z16.16)") loc(p(nn)   %n(nn)   )
          allocate(p(nn)   %na(nn),source=b)
             write (1723,"(z16.16)") loc(p(nn)   %na(nn)  )
    end select
    if (k/=1) print *,6738
