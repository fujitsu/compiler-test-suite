...
do nm=2,nn
     allocate(x::xv(nm),yv(nm))
             write (331,"(z16.16)") loc(xv )
             write (332,"(z16.16)") loc(yv )
    k=0
select type(xv)
  type is(x)
    ...
    select type(p=>xv(nm)%v)
      type is(e)
        k=1
             write (311,"(z16.16)") loc(xv(nm)%v   )
          allocate(p(nm)%n(nm),source=a)
             write (312,"(z16.16)") loc(p(nm)       %n   )
          allocate(p(nm)   %na(nm),source=a)
             write (313,"(z16.16)") loc(p(nm)   %na  )
          allocate(p(nm)   %nam(nm),source=a)
             write (314,"(z16.16)") loc(p(nm)   %nam )
     allocate(b3::p(nm)   %b2v(nm))
    kk=0
      select type(q=>p(nm)%b2v)
      type is(b3)
        kk=1
         write (341,"(z16.16)") loc(q      )
        ...
        kkk=1
              ...
              write (342,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (343,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,862
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (344,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (345,"(z16.16)") loc(r(nm) %w02  )
           end select
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (346,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (347,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,8624
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (348,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (349,"(z16.16)") loc(r(nm) %w02  )
           end select
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (350,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (351,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,8664
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (352,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (353,"(z16.16)") loc(r(nm) %w02  )
           end select
...
select type(yv)
  type is(x )
    ...
    select type(p=>yv(nm)%v)
      type is(ext)
             write (321,"(z16.16)") loc(p       (nm)%d1  )
      p       (nm)%d1(1)=1
        k=1
          allocate(p(nm)   %n(nm),source=b)
             write (322,"(z16.16)") loc(p(nm)   %n   )
          allocate(p(nm)   %na(nm),source=b)
             write (323,"(z16.16)") loc(p(nm)   %na  )
     allocate(b3::p(nm)   %b2v(nm))
    kk=0
      select type(q=>p(nm)%b2v)
      type is(b3)
