...
do nm=2,nn
     allocate(xv(nm),yv(nm))
             write (2131,"(z16.16)") loc(xv )
             write (2132,"(z16.16)") loc(yv )
      allocate(e ::xv(nm)%v(nm))
    k=0
    select type(p=>xv(nm)%v)
      type is(e)
        k=1
             write (2111,"(z16.16)") loc(xv(nm)%v   )
          allocate(p(nm)%n(nm),source=a)
             write (2112,"(z16.16)") loc(xv(nm)%v(nm)%n   )
          allocate(p(nm)   %na(nm),source=a)
             write (2113,"(z16.16)") loc(p(nm)   %na  )
          allocate(p(nm)   %nam(nm),source=a)
             write (2114,"(z16.16)") loc(p(nm)   %nam )
       end select
    if (k/=1) print *,6733

      allocate(ext::yv(nm)%v(nm))
             write (2121,"(z16.16)") loc(yv(nm)%v(nm)%d1  )
      yv(nm)%v(nm)%d1(1)=1
    k=0
      ...
      type is(ext)
        k=1
          allocate(p(nm)   %n(nm),source=b)
             write (2122,"(z16.16)") loc(p(nm)   %n   )
          allocate(p(nm)   %na(nm),source=b)
             write (2123,"(z16.16)") loc(p(nm)   %na  )
    end select
    if (k/=1) print *,6738
