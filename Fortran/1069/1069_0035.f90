...
do nm=2,nn
     allocate(x2::xv(nm))
             write (2671,"(z16.16)") loc(xv )
     select type(y=>xv)
      type is(x2)
        allocate(y(nm)%xn(nm),source='a')
             write (2672,"(z16.16)") loc(y(nm)%xn )
     end select
     allocate(x3::yv(nm))
             write (2661,"(z16.16)") loc(yv )
     select type(y=>yv)
      type is(x3)
        allocate(y(nm)%xn(nm),source='a')
             write (2662,"(z16.16)") loc(y(nm)%xn )
        allocate(y(nm)%xna(nm),source='a')
             write (2663,"(z16.16)") loc(y(nm)%xna)
     end select
    ...
    select type(p=>xv(nm)%v)
      type is(e)
        k=1
             write (2611,"(z16.16)") loc(xv(nm)%v   )
          allocate(p(nm)%n(nm),source=a)
             write (2612,"(z16.16)") loc(xv(nm)%v(nm)%n   )
          allocate(p(nm)   %na(nm),source=a)
             write (2613,"(z16.16)") loc(p(nm)   %na  )
          allocate(p(nm)   %nam(nm),source=a)
             write (2614,"(z16.16)") loc(p(nm)   %nam )
     allocate(b3::p(nm)   %b2v(nm))
    kk=0
      select type(q=>p(nm)%b2v)
      type is(b3)
        kk=1
         write (2641,"(z16.16)") loc(q      )
        ...
        kkk=1
              ...
              write (2642,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2643,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,862
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (2644,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2645,"(z16.16)") loc(r(nm) %w02  )
           end select
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (2646,"(z16.16)") loc(r(nm) %w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2647,"(z16.16)") loc(r(nm) %w02  )
           end select
        if (kkk/=1) print *,8624
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (2648,"(z16.16)") loc(r(nm)%w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2649,"(z16.16)") loc(r(nm)%w02  )
           end select
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (2650,"(z16.16)") loc(r(nm)%w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2651,"(z16.16)") loc(r(nm)%w02  )
           end select
        if (kkk/=1) print *,8664
        ...
        kkk=1
              allocate(r(nm)%w01(nm),source=a)
              write (2652,"(z16.16)") loc(r(nm)%w01  )
              allocate(r(nm)%w02(nm),source=a)
              write (2653,"(z16.16)") loc(r(nm)%w02  )
           end select
    ...
    if (kk/=1) print *,6733
      ...
      allocate(ext::yv(nm)%v(nm))
             write (2621,"(z16.16)") loc(yv(nm)%v(nm)%d1  )
      yv(nm)%v(nm)%d1(1)=1
    k=0
      ...
      type is(ext)
        k=1
          allocate(p(nm)   %n(nm),source=b)
             write (2622,"(z16.16)") loc(p(nm)   %n   )
          allocate(p(nm)   %na(nm),source=b)
             write (2623,"(z16.16)") loc(p(nm)   %na  )
     allocate(b3::p(nm)   %b2v(nm))
    kk=0
      select type(q=>p(nm)%b2v)
      type is(b3)
