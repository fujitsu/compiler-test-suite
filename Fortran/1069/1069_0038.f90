...
contains
     ...
     allocate(xv,yv)
             write (2931,"(z16.16)") loc(xv )
             write (2932,"(z16.16)") loc(yv )
      allocate(e ::xv%v)
    k=0
    select type(p=>xv%v)
      type is(e)
        k=1
             write (2911,"(z16.16)") loc(xv%v   )
          allocate(p%n,source=a)
             write (2912,"(z16.16)") loc(p   %n   )
          allocate(p   %na,source=a)
             write (2913,"(z16.16)") loc(p   %na  )
          allocate(p   %nam,source=a)
             write (2914,"(z16.16)") loc(p   %nam )
       end select
    if (k/=1) print *,6733
      ...
      type is(ext)
        k=1
          allocate(p   %n,source=b)
             write (2922,"(z16.16)") loc(p   %n   )
          allocate(p   %na,source=b)
             write (2923,"(z16.16)") loc(p   %na  )
    end select
    if (k/=1) print *,6738
