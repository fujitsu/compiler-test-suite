...
program main
  ...
  allocate(value%cli_options%clasg(n1)%cla(n2)%progname,source="ab")
 ...
 select type (p=>var)
    ...
    do n2=1,size(p%cli_options%clasg(n1)%cla)
      p%cli_options%clasg(n1)%cla(n2)%progname   ='12'
 ...
 if (k/=1) print *,3241
    ...
    do n2=1,size(value%cli_options%clasg(n1)%cla)
     if(value%cli_options%clasg(n1)%cla(n2)%progname   /='ab') print *,1003
