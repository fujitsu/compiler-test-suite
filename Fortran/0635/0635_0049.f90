...
SUBROUTINE TEST_1(P1,P2,P3,D1,D2,D3,N,M)
  ...
  DO I=1,N
    D1(I)%F=D1(I)%F+P1(M)%F
  END DO
  DO I=1,N-1
    D2(I)%F=D2(I)%F+P2(M+1)%F
  END DO
  DO I=1,N
    D3(I)%F=D3(I)%F+P3(M+2)%F
  END DO
...
SUBROUTINE TEST_2(P1,P2,P3,D1,D2,D3,N)
  ...
  DO I=1,N
    D1(I)%F=D1(I)%F+P1(N+1)%F
  END DO
  DO I=1,N-1
    D2(I)%F=D2(I)%F+P2(N+1)%F
  END DO
  DO I=1,N
    D3(I)%F=D3(I)%F+P3(N+1)%F
  END DO
...
PROGRAM MAIN
  USE FT
  TYPE(FTYPE),DIMENSION(67),TARGET :: D1=FTYPE(1.0_8),D2=FTYPE(2.0_8),D3=FTYPE(3.0_8),D4,D5,D6
  TYPE(FTYPE),POINTER,DIMENSION(:) :: P1,P2,P3
  INTERFACE 
     SUBROUTINE TEST_1(P1,P2,P3,D1,D2,D3,N,M)
       USE FT
  ...
  IF (ANSWER_CHECK(D1,5.0_8).NE.0 .OR. &
      ANSWER_CHECK(D2,7.0_8).NE.0 .OR. &
      ANSWER_CHECK(D3,9.0_8).NE.0) THEN
     PRINT *, "NG"
  ELSE
     PRINT *, "OK"
  END IF  
  CALL TEST_2(P1,P2,P3,D1,D2,D3,64_4)
  IF (ANSWER_CHECK(D1,9.0_8).NE.0 .OR. &
      ANSWER_CHECK(D2,12.0_8).NE.0 .OR. &
      ANSWER_CHECK(D3,15.0_8).NE.0) THEN
     PRINT *, "NG"
  ELSE
     PRINT *, "OK"
  END IF  
  CALL TEST_3(D1,D2,D3,64_4)
  IF (ANSWER_CHECK(D1,10.0_8).NE.0 .OR. &
      ANSWER_CHECK(D2,14.0_8).NE.0 .OR. &
      ANSWER_CHECK(D3,18.0_8).NE.0) THEN
     PRINT *, "NG"
  ELSE
     PRINT *, "OK"
  END IF  
