...
CONTAINS
  ...
  SUBROUTINE calc_sub(data_array1,data_array2)
    REAL(num), DIMENSION(-n:,-k:), INTENT(OUT) :: data_array1
    REAL(num), DIMENSION(-n:,-k:), INTENT(OUT) :: data_array2
    data_array1=1.
    data_array2=1.
    if (any(lbound(data_array1)/=-6)) print *,701
    if (any(lbound(data_array2)/=-6)) print *,702
    if (any(ubound(data_array1)/=-6+1)) print *,711
    if (any(ubound(data_array2)/=-6+1)) print *,712
