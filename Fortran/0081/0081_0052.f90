...
CONTAINS
 ...
 USE module0 
    REAL(num), DIMENSION(1-n+1+1:,1-n+1+k:), INTENT(OUT) :: data_array1
    REAL(num), DIMENSION(1-n+1+1:,1-n+1+k:), INTENT(OUT) :: data_array2
    data_array1=1.
    data_array2=1.
    if (any(lbound(data_array1)/=-3)) print *,701
    if (any(lbound(data_array2)/=-3)) print *,702
    if (any(ubound(data_array1)/=-3+1)) print *,711
    if (any(ubound(data_array2)/=-3+1)) print *,712
