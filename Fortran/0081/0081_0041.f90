...
CONTAINS
  ...
  SUBROUTINE calc_sub(data_array1,data_array2)
    REAL(num), DIMENSION(1-n:,1-n:), INTENT(OUT) :: data_array1
    REAL(num), DIMENSION(1-n:,1-n:), INTENT(OUT) :: data_array2
    data_array1=1.
    data_array2=1.
    if (any(lbound(data_array1)/=-5)) print *,701
    if (any(lbound(data_array2)/=-5)) print *,702
    if (any(ubound(data_array1)/=-5+1)) print *,711
    if (any(ubound(data_array2)/=-5+1)) print *,712
