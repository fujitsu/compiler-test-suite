...
subroutine inlined(dest1,dest2,src1,src2)
  real(kind=8),dimension(1:N)::dest1, dest2, src1, src2
  integer::i
  do i=1,N
     dest2(i) = tan(src1(i) / src2(i)) + src2(i) - N + i + 1
     dest1(i) = dest2(i) + log(abs(src2(i) + dest2(i))) * i + 2
  end do
end subroutine inlined

subroutine target(dest1, dest2, src1, src2)
  real(kind=8),dimension(1:N)::dest1, dest2, src1, src2

  call inlined(dest1, dest2, src1, src2)
