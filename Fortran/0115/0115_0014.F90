...
subroutine target(dest1, dest2, src1, src2)
  integer(kind=8),dimension(1:N)::dest1, dest2, src1, src2
  integer::i

  forall(i=1:N)
     dest1(i) = src1(i) * src2(i) + src1(i) * src2(i)
     dest2(i) = src1(i) / (src2(i)*src2(i) + 1) + src2(i)
     dest2(i) = src2(i) - (src2(i) * dest1(i))
  end forall
...
subroutine init(src1, src2)
  integer(kind=8),dimension(1:N)::src1, src2
  integer::i
  do i=1,N
    src1(i) = int(i*1.0 + 0.1)
    src2(i) = int(i*0.5 + 0.3)
  end do
end subroutine init


subroutine check(dest1, dest2, dest1_res, dest2_res)  
  integer(kind=8),dimension(1:N)::dest1, dest2, dest1_res, dest2_res
  integer::i
  interface
  ...
  do i=1,N
     if ( (dest1(i)==dest1_res(i)) .and. (dest2(i)==dest2_res(i)) &
        .neqv. .true.)then
        print *, "NG"
     endif
  end do
...
program main
  integer(kind=8),dimension(1:N)::dest1, dest2, src1, src2
  integer(kind=8),dimension(1:N)::dest1_res, dest2_res, src1_res, src2_res
  call init(src1, src2);
