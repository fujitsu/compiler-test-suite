...
real(8),dimension(1:3) :: ans = (/2.699999823421243E-02_8,3.239999776929620E-02_8,3.779999635443100E-02_8/)
real(8),parameter :: error = 6.034969989021643E-9_8
integer :: test_no
do i=1,3
...
real(8),dimension(1:3,1:3) :: a
real(8),dimension(1:3) :: s,x
a = reshape((/(real(i,kind=8)*0.03_4,i=1,3*3)/),(/3,3/))
s = (/(real(i,kind=8)*0.03_4,i=1,3)/)
x = (/(real(i,kind=8)*0.03_4,i=4,6)/)
call sub1(a,s,x)
call check_x(x,1)
a = reshape((/(real(i,kind=8)*0.03_4,i=1,3*3)/),(/3,3/))
s = (/(real(i,kind=8)*0.03_4,i=1,3)/)
