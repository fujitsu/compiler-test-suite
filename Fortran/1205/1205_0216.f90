 ...
 if (n<loop) then
   allocate(tye::t(2,2)%node(2,2))
   call alc(t(2,2)%node,n)
 endif
   select type(t)
    type is(tye)
     allocate(t(2,2)%c1e(2,2),source=a1)
   end select
 end
 ...
 class (tt) :: t(:,:)
   ...
   n=0
   allocate(tye::t(2,2)%node1(2,2))
   call alc(t(2,2)%node1,n)
 if (n2<loop) then
   allocate(tte::t(2,2)%node2(2,2))
   call alc_tt(t(2,2)%node2,n2)
 endif
   select type(t)
    type is(tte)
     allocate(t(2,2)%c2e(2,2),source=a1)
   end select
 end
subroutine put_addr(t,unit)
 ...
 select type(t)
   type is(tte)
    write (unit+7,"(z16.16)") loc(t(2,2)%c2e)
    select type(p=>t(2,2)%node1)
      type is(tye)
        write (unit+8,"(z16.16)") loc(p      (2,2)%c1e )
        select type(q=>p(2,2)%node)
          type is(tye)
            write (unit+9,"(z16.16)") loc(q      (2,2)%c1e )
        end select
    end select
    select type(p=>t(2,2)%node2)
      type is(tte)
        write (unit+10,"(z16.16)") loc(p      (2,2)%c2e )
        select type(q=>p(2,2)%node1)
          type is(tye)
            write (unit+11,"(z16.16)") loc(q      (2,2)%c1e )
            select type(w=>q(2,2)%node)
              type is(tye)
              write (unit+12,"(z16.16)") loc(w      (2,2)%c1e )
            end select
        end select
    end select
 end select
...
subroutine chk_value(d)
 ...
 select type(t=>d)
   type is(tte)
    if (t(2,2)%c2e(2,2)/=a1) print *,70001
    select type(p=>t(2,2)%node1)
      type is(tye)
        if (p      (2,2)%c1e(2,2)/=a1) print *,70002
        select type(q=>p(2,2)%node)
          type is(tye)
            if (q      (2,2)%c1e (2,2)/=a1) print *,70003
            k=1
        end select
    end select
    if (k/=1) print *,70090
    ...
    select type(p=>t(2,2)%node2)
      type is(tte)
        if (p      (2,2)%c2e (2,2)/=a1) print *,70011
        select type(q=>p(2,2)%node1)
          type is(tye)
            if (q      (2,2)%c1e (2,2)/=a1) print *,70012
            select type(w=>q(2,2)%node)
              type is(tye)
              if (w      (2,2)%c1e (2,2)/=a1) print *,70013
            k=1
            end select
        end select
    end select
...
type(g)::h(2,2)
k=2
allocate(ttx::h(k,k)%y(2,2))
 n2=0
 call alc_tt(h(k,k)%y,n2)
 call put_addr(h(k,k)%y,1000)

allocate(tte::h(k,k)%t(2,2))
