 ...
 class (ty),allocatable :: t (:)
   n=n+1
   t(2)%value=n
   allocate(t(2)%c1(2),source=a1)
 if (n<loop) then
   allocate(tye::t(2)%node(2))
   call alc(t(2)%node,n)
 endif
   select type(t)
    type is(tye)
...
subroutine put_addr(t,unit)
 ...
 write (unit+2,"(z16.16)") loc(t(2)%node1(2)%c1  )
 write (unit+3,"(z16.16)") loc(t(2)%node1(2)%node(2)%c1)
 write (unit+4,"(z16.16)") loc(t(2)%node2(2)%c2)
 write (unit+5,"(z16.16)") loc(t(2)%node2(2)%node1(2)%c1)
 write (unit+6,"(z16.16)") loc(t(2)%node2(2)%node1(2)%node(2)%c1)
 select type(t)
   type is(tte)
...
subroutine set(t)
 ...
 t(2)%node2(2)%c2=b1
 t(2)%node2(2)%node1(2)%c1=b1
 t(2)%node2(2)%node1(2)%node(2)%c1=b1
...
subroutine chk_value(d)
 ...
 select type(t=>d)
   type is(tte)
    if (t(2)%c2e(2)/=a1) print *,70001
    select type(p=>t(2)%node1)
      type is(tye)
        if (p      (2)%c1e(2)/=a1) print *,70002
        select type(q=>p(2)%node)
          type is(tye)
            if (q      (2)%c1e (2)/=a1) print *,70003
            k=1
        end select
    end select
    if (k/=1) print *,70090
    ...
    select type(p=>t(2)%node2)
      type is(tte)
        if (p      (2)%c2e (2)/=a1) print *,70011
        select type(q=>p(2)%node1)
          type is(tye)
            if (q      (2)%c1e (2)/=a1) print *,70012
            select type(w=>q(2)%node)
              type is(tye)
              if (w      (2)%c1e (2)/=a1) print *,70013
            k=1
            end select
        end select
    end select
