...
program main
  ...
  complex(kind=4),dimension(1:64) :: c
...
subroutine init(a,b,c,res,x,y)
  ...
  do i=1,64
     ...
     if (i .gt. 64/2) then
        ...
        call set(c(i) ,real(2.11,kind=4), real(-1.111,kind=4))
     else
        call set(b(i) ,real(-i,kind=4), real(i*2,kind=4))
        call set(c(i) ,real(-4.01,kind=4), real(1.999,kind=4))
     endif
     res(i) = 0._4
     x(i) = real(i,kind=4)
     y(i) = 0._4
