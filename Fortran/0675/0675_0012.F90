...
subroutine init(a,b,res,x)
  ...
  do i=1,64
     a(i) = 0._4
     if (i .gt. 64/2) then
        b(i) = cmplx(real(i)/real(2*64),real(i)/real(64),kind=8)
        res(i) = asin(cmplx(real(i)/real(2*64),real(i)/real(64),kind=8))
     else
        b(i) = cmplx(real(i)/real(3*64),real(i)/real(2*64),kind=8)
        res(i) = asin(cmplx(real(i)/real(3*64),real(i)/real(2*64),kind=8))
     endif
