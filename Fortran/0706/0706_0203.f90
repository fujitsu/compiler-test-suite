 ...
 call    z20(reshape(cha,(/1,1,1,1,1,1,10/))    )
 end subroutine
...
1 end subroutine
 ...
 iu=12
 open(iu,delim='quote')
 write(iu,*) pack(chx(:,:,:,:,:,:,1:10:9),reshape(mask(1:2),sh),chy),'-1'
 write(iu,*) pack(chx(:,:,:,:,:,:,i1:i10:i9),reshape(mask(1:2),sh),chy),'-1'
 write(iu,*) pack(chx(:,:,:,:,:,:,v),reshape(mask(1:2),sh),chy),'-1'
 write(iu,*) pack(chx(:,:,:,:,:,:,(/1,10/)),reshape(mask(1:2),sh),chy),'-1'
 rewind iu
 do i=1,4
   read (iu,*) chw(1:3)
   if (any(chw(1:3)/=(/chz(1:2),'-1      '/)))print *,'fail'
 end do
 read (iu,*,end=1) chw
 print *,'fail'
1 end subroutine
