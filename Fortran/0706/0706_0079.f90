...
c
     ...
     1              reshape(input_check_value,x2_shape)))print *,'fail'
        ...
        function ww(arg,n) result(r)
        character(len=*,kind=1),dimension(:,:),optional,intent(in)::arg
        character(len=n,kind=1),pointer,dimension(:,:)::r
        r=>www(arg,size(arg,dim=1),size(arg,dim=2))
        contains
        function www(arg,n1,n2) result(r)
        character(len=*,kind=1),dimension(:,:),optional,intent(in)::arg
        character(len=n,kind=1),pointer,dimension(:,:)::r
        allocate (r(n1,n2))
        r=arg
      ...
      a1(:,2,:)=input_check_value
       call x1(vector_subscript_flag=logical(t,kind=1), 
     1    a=w(a1(v_s,2,v_s)))
c      if (any(a1(:,2,:)/=return_value))print *,'fail'
c     a1(:,2,:)=input_check_value
       call x2(vector_subscript_flag=logical(t,kind=1), 
     1    a=w(a1(v_s,2,v_s)))
c      if (any(a1(:,2,:)/=return_value))print *,'fail'
c     a1(:,2,:)=input_check_value
       call x3(vector_subscript_flag=logical(t,kind=1), 
     1    a=w(a1(v_s,2,v_s)))
c      if (any(a1(:,2,:)/=return_value))print *,'fail'
c     a1(:,2,:)=input_check_value
       call x4(vector_subscript_flag=logical(t,kind=1), 
     1    a=w(a1(v_s,2,v_s)))
c      if (any(a1(:,2,:)/=return_value))print *,'fail'
      end
      subroutine s12
