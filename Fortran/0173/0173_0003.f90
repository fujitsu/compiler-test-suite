...
mask(3,1,1,1)=.true.
where (mask(:,:,:,:))
ww(1)%w(:n,:,:,:)= f(  vv(1)%v(1:n,:,:,:) )
end where
if (ww(1)%w(1,1,1,1)%z(1)/=1 ) print *,101
if (allocated(ww(1)%w(2,1,1,1)%z)) print *,1021
if (ww(1)%w(3,1,1,1)%z(1)/=3 ) print *,103
where (mask(:,:,:,:))
vv(1)%v(2:m-2,:,:,:)= f(  vv(1)%v(1:n,:,:,:) )
end where
