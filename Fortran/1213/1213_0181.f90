...
subroutine s1(m)
 type(y)::va(m),vb(m)
  ...
  allocate(   va(m)%v,v,e(2),vb(m)%v,mold=w,stat=nn)
    ...
    k=0
      if (allocated(e(1)%na)) print *,2829
      k=1
      if (.not.same_type_as(q,e(1)%na)) print *,2828
      deallocate(e(1)%na,stat=n)
      if (n==0) print *,282
      if (allocated(e(2)%na)) print *,2829
      if (.not.same_type_as(q,e(2)%na)) print *,2828
      deallocate(e(2)%na,stat=n)
      if (n==0) print *,282
    if (k/=1) print *,5255
    k=0
    select type(v)
