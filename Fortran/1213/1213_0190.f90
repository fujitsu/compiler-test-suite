...
subroutine s1(m)
 type(y)::va(m),vb(m)
  ...
  allocate(te::e(2))
  allocate(   va(m)%v,v,vb(m)%v,e(2),mold=w,stat=nn)
   if (nn==0) print *,7652
   if (.not.allocated(va(m)%v)) print *,80001
   if (.not.allocated(v)) print *,80002
   if (.not.allocated(vb(m)%v)) print *,80003
    k=0
    select type(v)
     type is(te)
      ...
      if (n==0) print *,282
      if (allocated(v(2)%na)) print *,282901
      if (.not.same_type_as(q,v(2)%na)) print *,282801
    ...
    select type(v)
     type is(te)
      ...
      k=1
      if (.not.same_type_as(q,v(1)%name)) print *,282802
      deallocate(v(1)%name,stat=n)
      if (n==0) print *,282
      if (allocated(v(2)%name)) print *,2829
      if (.not.same_type_as(q,v(2)%name)) print *,282803
 ...
 k=0
    select type(p=>va(m)%v(2))
     type is(te)
      ...
      k=1
      if (.not.same_type_as(q,p      %name)) print *,282804
    ...
    select type(p=>vb(m)%v(2))
     type is(te)
      if (allocated(p%name)) print *,2829
      k=1
      if (.not.same_type_as(q,p      %name)) print *,282805
      deallocate(p      %name,stat=n)
      if (n==0) print *,282
    end select
    if (k/=1) print *,5255
 k=0
    select type(p=>va(m)%v(2))
     type is(te)
      ...
      k=1
      if (.not.same_type_as(q,p      %na)) print *,282806
    ...
    select type(p=>vb(m)%v(2))
     type is(te)
      if (allocated(p%na)) print *,2829
      k=1
      if (.not.same_type_as(q,p      %na)) print *,282807
      deallocate(p      %na,stat=n)
      if (n==0) print *,282
    end select
    if (k/=1) print *,5255
