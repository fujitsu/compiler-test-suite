...
subroutine s1(m)
 ...
 ! type (t),allocatable::e(:)
  ...
  allocate(   va(m)%v,v,     vb(m)%v,mold=w)
    ...
    k=0
!D      if (allocated(e(1)%na)) print *,2829
!D      k=1
!D      if (.not.same_type_as(q,e(1)%na)) print *,2828
!D      deallocate(e(1)%na,stat=n)
!D      if (n==0) print *,282
!D      if (allocated(e(2)%na)) print *,2829
!D      if (.not.same_type_as(q,e(2)%na)) print *,2828
!D      deallocate(e(2)%na,stat=n)
!D      if (n==0) print *,282
!D    if (k/=1) print *,5255
!D    k=0
    select type(v)
