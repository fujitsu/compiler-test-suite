  ...
  allocate(   va(m)%v,v,vb(m)%v,mold=w)
    ...
    select type(v)
     type is(te)
      ...
      k=1
      if (.not.same_type_as(q,v(1)%name)) print *,92828
      deallocate(v(1)%name,stat=n)
      if (n==0) print *,282
      if (allocated(v(2)%name)) print *,2829
      if (.not.same_type_as(q,v(2)%name)) print *,82828
   ...
   k=0
    select type(p=>va(m)%v(2))
     type is(te)
      ...
      k=1
      if (.not.same_type_as(q,p      %name)) print *,72828
    ...
    select type(p=>vb(m)%v(2))
     type is(te)
      if (allocated(p%name)) print *,2829
      k=1
      if (.not.same_type_as(q,p      %name)) print *,62828
      deallocate(p      %name,stat=n)
      if (n==0) print *,282
    end select
    if (k/=1) print *,5255
