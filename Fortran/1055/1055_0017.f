      ...
      integer*4 :: loop=50, thread=4, chunk=1
...
!$omp end parallel do
      ...
      ic=1
      id=1
      do while(ib(ic).ne.0 .and. ic.le.loop)
         do while(ib(ic).gt.ih(ic,k) .and. id.le.loop)
      ...
      enddo
      write(*,*) "----- test --"
      write(*,*) "      parallel do schedule(guided,chunk=1) -----"
      if(ier(1).eq.0 .or. ier(2).eq.0) then
         write(*,*) "OK"
      else
         write(*,*) "NG!  SCHEDULE(GUIDED) clause is incorrect!"
         write(*,*) "     loop=",loop," thread=",thread," chunk=",chunk
         write(*,*) "     ier=",ier
         write(*,*) "     ia=",ia
         write(*,*) "     ib=",ib
         write(*,*) "     ih=",ih
