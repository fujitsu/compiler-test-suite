...
#define LOOP_KERNEL       (r[i][j] += cos(i*1.1)/ 10.2 + I*sin(i*1.1*_CNT_ / NI) + _CNT_*2.01);
...
#define ERR_S 1.0E-6
#define EQUAL_CHECK(val1, val2) ((cabsf(val1) == 0 && cabsf(val2) <= ERR_S) || \
				 (cabsf(val1 - val2)/cabsf(val1) <= ERR_S))
...
void sub_init(DATA_TYPE r[NI][NI], int n) {
  ...
  for (j=0; j<NI; ++j) {
  for (i=0;i<NI;++i) {
    r[i][j] = sin(i+j*1.1) + I*cos(i-j*0.3);
  }
  }
}
...
void sub_cmp(DATA_TYPE p[NI][NI], DATA_TYPE q[NI][NI], int *is_ok) {
  ...
  for (i=0; i<NI; ++i) {
    if (EQUAL_CHECK(p[i][j], q[i][j]) == _FALSE_) {
      printf("%d %d %f %f %f %f\n", i,j, creal(p[i][j]),cimag(p[i][j]), 
	     creal(q[i][j]), cimag(q[i][j]));
      *is_ok = _FALSE_;
    }
  }
  }
