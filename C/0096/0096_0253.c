...
int test_loop_no_escape_pt1_##test_type(test_type flag1, test_type flag2, int lmax, test_type d_val) {\
  ...
  for(j=0; j<LOOP_CONST; j++) {\
    for(i=0; i<LOOP_CONST; i++) {\
      if (flag1 == flag2) {\
        d_data1[j][i] = (test_type)TRUE_RES;\
      }\
      d_data2[j][i] = (test_type)TRUE_RES;\
    }\
  }\
  for(j=0; j<LOOP_CONST; j++) {\
    for(i=0;i<LOOP_CONST;i++) {\
      if (d_data1[j][i] != d_val || d_data2[j][i] != (test_type)TRUE_RES) {\
        return FALSE_RES;\
      }\
    }\
  }\
...
int main() {
  CALL_TEST_LOOP_NO_ESCAPE(float, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(float, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(double, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(double, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LD, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LD, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(char, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(char, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(SI, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(SI, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(int, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(int, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LI, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LI, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LLI, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LLI, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UC, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UC, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(US, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(US, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UI, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UI, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UL, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UL, 2, 3, LOOP_CONST, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(ULL, 1, 1, LOOP_CONST, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(ULL, 2, 3, LOOP_CONST, FALSE_RES);

  CALL_TEST_LOOP_NO_ESCAPE(float, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(float, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(double, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(double, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LD, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LD, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(char, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(char, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(SI, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(SI, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(int, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(int, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LI, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LI, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LLI, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(LLI, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UC, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UC, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(US, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(US, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UI, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UI, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UL, 1, 1, -1, TRUE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(UL, 2, 3, -1, FALSE_RES);
  CALL_TEST_LOOP_NO_ESCAPE(ULL, 1, 1, -1, TRUE_RES);
