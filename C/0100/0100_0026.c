...
#define ANSWER -100.0f

void sub_004(int (*aa)[ELEMENT_NUM],
	      int (*bb)[ELEMENT_NUM], 
	      int pi,
	      int pj,
	      int pk,
	      ...
	      int pn)
{
  ...
  for (i=0; i < ROTATION_NUM; ++i) {
    for (j=0; j < ROTATION_NUM; ++j) {
      aa[10/2][j+pi+pl+pm+pn] = aa[i+0+pj*pn][j] - bb[i+pi+pl-pm+3-2-1/1+pk+pi+pj][j];
    }
  }
}
...
int main( void ) {
  ...
  sub_004(array2_a, array2_b, i, j, k, l, m, n);
