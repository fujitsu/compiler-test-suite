...
void init(unsigned int *a, unsigned int *b, int *mask) {
  int i;
...
void test(double _Complex * restrict dest, unsigned int * restrict a, unsigned int * restrict b, int * restrict mask) {
  int i;
#ifdef HAS_MASK
  for(i = 0;i < N;i++ ) {
    if (mask[i] == 1) {
