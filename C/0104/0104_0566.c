...
void init(unsigned long long int *a, unsigned long long int *b, int *mask) {
  int i;
...
void test(double _Complex * restrict dest, unsigned long long int * restrict a, unsigned long long int * restrict b, int * restrict mask) {
  int i;
#ifdef HAS_MASK
  for(i = 0;i < N;i++ ) {
    if (mask[i] == 1) {
