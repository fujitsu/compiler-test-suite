...
void test(double _Complex * restrict dest, double _Complex * restrict a, double _Complex * restrict b, int * restrict mask) {
  int i;
#ifdef HAS_MASK
  for(i = 0;i < N;i++ ) {
    if (mask[i] == 1) {
      dest[i] = a[i] == b[i] ? (0.0+0.0i) : (1.0+1.0i);
    }
  }
#else
  for(i = 0;i < N;i++ ) {
...
int MAINF() {
  double _Complex a[N],b[N],dest[N];
  int mask[N];
  int i;
  init (a,b,mask);
  for (i = 0;i < 2;i++ ) {
