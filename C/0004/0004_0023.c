...
static  void init_i8(signed long long int *i);
...
int main(void)
{
  ...
  /* test start */
  printf("test start\n\n");
  EQ(i2,u2,u4); /* test No.1-1 */
  EQ(i2,u2,u8); /* test No.1-2 */
  EQ(i2,u4,u2); /* test No.1-3 */
  EQ(i2,u8,u2); /* test No.1-4 */

  EQ(i4,u2,u4); /* test No.2-1 */
  EQ(i4,u2,u8); /* test No.2-2 */
  EQ(i4,u4,u2); /* test No.2-3 */
  EQ(i4,u8,u2); /* test No.2-4 */

  EQ(i8,u2,u4); /* test No.3-1 */
  EQ(i8,u2,u8); /* test No.3-2 */
  EQ(i8,u4,u2); /* test No.3-3 */
  EQ(i8,u8,u2); /* test No.3-4 */

  EQ(u1,u2,u4); /* test No.4-1 */
  EQ(u1,u2,u8); /* test No.4-2 */
  EQ(u1,u4,u2); /* test No.4-3 */
  EQ(u1,u8,u2); /* test No.4-4 */

  EQ(u2,u2,u4); /* test No.5-1 */
  EQ(u2,u2,u8); /* test No.5-2 */
  EQ(u2,u4,u2); /* test No.5-3 */
  EQ(u2,u8,u2); /* test No.5-4 */

  EQ(u4,u2,u4); /* test No.6-1 */
  EQ(u4,u2,u8); /* test No.6-2 */
  EQ(u4,u4,u2); /* test No.6-3 */
  EQ(u4,u8,u2); /* test No.6-4 */

  EQ(u8,u2,u4); /* test No.7-1 */
  EQ(u8,u2,u8); /* test No.7-2 */
  EQ(u8,u4,u2); /* test No.7-3 */
  EQ(u8,u8,u2); /* test No.7-4 */
  printf("test OK\n\n");
  printf("test end\n");
}
