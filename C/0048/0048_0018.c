...
m3edc139 (void)
{
   ...
   volatile int signed long    a5[2]={4,5}, b5=0;
...
m3edc173 (void)
{
   const struct f2stag1 stx={0,0}, *stxp=NULL;
   volatile struct f2stag2 {
         char  c;
         int   i;
   } f2st2 = {0 , 0};
   const enum entag2 {
         f2enx=-1, f2eny=0, f2enz=100
   } f2en1 = f2enz;
...
m3edc183 (void)
{
   ...
   volatile const f3inttp  a1=0;
   volatile const f3inttp  a2=0, b2=0;
...
{
    if ((now->forward->data)<(add->data) && now->forward!=0)
      {
        correlation (now->forward,add);
      }
    else
     {
        add->forward=now->forward;
        now->forward=add ;
     }
    return 0;
}
