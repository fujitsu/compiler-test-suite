#include <stdlib.h>


#include <stdio.h>
int main()
{
	long long int a[10]={0x7fffffffffffffffll,0x7ffffffffffffffell,
			     0x7ffffffffffffffdll,0x7ffffffffffffffcll,
			     0x7ffffffffffffffbll,0x7ffffffffffffffall,
			     0x7ffffffffffffff9ll,0x7ffffffffffffff8ll,
			     0x7ffffffffffffff7ll,0x7ffffffffffffff6ll};
	long long int c[10]={0x7ffffffffffffff0ll,0x7ffffffffffffff0ll,
			     0x7ffffffffffffff0ll,0x7ffffffffffffff0ll,
			     0x7ffffffffffffff0ll,0x7ffffffffffffff0ll,
			     0x7ffffffffffffff0ll,0x7ffffffffffffff0ll,
			     0x7ffffffffffffff0ll,0x7ffffffffffffff0ll};
	long long int b[10];
	long int i;

	for(i=0;i<10;i++) {
		if ( i > 5 )
		b[i] = a[i] ^ c[i];
	}
	
	for(i=0;i<10;i++)
		if ( i > 5 )
		if( b[i] == a[i]-0x7ffffffffffffff0ll ) {
			printf("***** Element %d OK *****\n",i);
		} else {
			printf("***** Element %d NG *****     ",i);
			printf("b[%d] = %x %x\n",i,*(long int *)&b[i],*(((long int *)&b[i])+1)) ;
		}
	
	exit(0);
}

