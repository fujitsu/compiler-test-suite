...
void check_opt(float const rval, float * restrict prdef, long * restrict pldef, int num) {
  int i;
  float rarg, rtmp;
  ...
  for(i=0;i<num;++i) {
    prdef[i] = roundf(rarg);
    pldef[i] = lroundf(rarg);
    rarg += rtmp;
  }
}

void check_special_value(float const rval, float * restrict prdef, long * restrict pldef) {
  *prdef = roundf(rval);
  *pldef = lroundf(rval);
}

void value_init(float *rval, long *ival) {
  *rval = 2.0f;
  *ival = 2L;
}
int main() {
