...
void check_opt(double const rval, double * restrict prdef, long long * restrict pldef, int num) {
  int i;
  double rarg, rtmp;
  ...
  for(i=0;i<num;++i) {
    prdef[i] = round(rarg);
    pldef[i] = llround(rarg);
    rarg += rtmp;
  }
}

void check_special_value(double const rval, double * restrict prdef, long long * restrict pldef) {
  *prdef = round(rval);
  *pldef = llround(rval);
}

void value_init(double *rval, long long *ival) {
  *rval = 2.0;
  *ival = 2L;
}
int main() {
