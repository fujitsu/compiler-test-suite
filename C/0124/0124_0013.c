...
#define VLR6C(i) t.vlr6c[i-1]
 ...
 double vlr5d[512];
 double vlr6d[1024];
 struct m {double vlr1c[32];
           double vlr2c[64];
           double vlr3c[128];
           double vlr4c[256];
           double vlr5c[512];} ;
 union {double vlr6c[1024];
        struct m s         ;} t;
int main()
  {
   long int i,j,I,J;
 ...
 LBL_8000:;
    ...
    for(i=1;i<=1024;i+=4){
      printf(" %g %g %g %lu ",VLR6A(i),VLR6B(i),VLR6D(i),L(i));
      printf(" %g %g %g %lu ",VLR6A(i+1),VLR6B(i+1),VLR6D(i+1),L(i+1));
    printf(" %g %g %g %lu ",VLR6A(i+2),VLR6B(i+2),VLR6D(i+2),L(i+2));
