...
int main()
  {
   ...
   for(JJ=1;JJ<=10;JJ++)
   {
      for(J=1;J<=10;J++)
      {
         ...
         DD4.real=pow(C4(J,1).real,C4(J,3).real)*2.0-
                  pow(C4(J,1).imag,C4(J,3).imag)*1.0;
         DD4.imag=pow(C4(J,1).real,C4(J,3).real)*1.0+
                  pow(C4(J,1).imag,C4(J,3).imag)*2.0;
         DD5.real=DD1.real+DD2.real+DD3.real+DD4.real+DD6.real+1.0;
         DD5.imag=DD1.imag+DD2.imag+DD3.imag+DD4.imag+DD6.imag+1.0;
         I=J;
         C4(J,2).real=DD5.real+C1(1,I).real+C2(2,I).real+C3(3,J).real+
                      C4(4,I).real+DD1.real+DD2.real+DD3.real;
         C4(J,2).imag=DD5.imag+C1(1,I).imag+C2(2,I).imag+C3(3,J).imag+
                      C4(4,I).imag+DD1.imag+DD2.imag+DD3.imag;
      }
   }
   ED5.dreal=1.0;
   ED5.dimag=0.5;
   ...
   for(JJ=1;JJ<=10;JJ++)
   {
      for(J=1;J<=10;J++)
      {
         ...
         ED5.dreal=ED1.dreal+ED2.dreal+ED3.dreal+ED4.dreal+
                   pow(ED6.dreal,CD1(1,J).dreal);
         ED5.dimag=ED1.dimag+ED2.dimag+ED3.dimag+ED4.dimag+ED6.dimag;
                   pow(ED6.dimag,CD1(1,J).dimag);
         I=J;
         CD4(J,2).dreal=ED6.dreal+CD1(1,I).dreal+CD2(2,I).dreal+CD3(3,J).dreal+
                        CD4(4,I).dreal+ED6.dreal+ED2.dreal+ED3.dreal;
         CD4(J,2).dimag=ED6.dimag+CD1(1,I).dimag+CD2(2,I).dimag+CD3(3,J).dimag+
                        CD4(4,I).dimag+ED6.dimag+ED2.dimag+ED3.dimag;
      }
   }
   FD5.qreal=0.0;
   FD5.qimag=0.0;
   ...
   for(J=1;J<=10;J++)
   {
      FD1.qreal=((CQ1(1,J).qreal*2.0-CQ1(1,J).qimag*1.0)*CQ1(2,J).qreal+
                 (CQ1(1,J).qreal*1.0+CQ1(1,J).qimag*2.0)*CQ1(2,J).qimag)/
                 (pow(CQ1(1,J).qreal,2)+pow(CQ1(1,J).qimag,2));
      FD1.qimag=((CQ1(1,J).qreal*1.0+CQ1(1,J).qimag*2.0)*CQ1(2,J).qreal-
                 (CQ1(1,J).qreal*2.0-CQ1(1,J).qimag*1.0)*CQ1(1,J).qimag)/
                 (pow(CQ1(1,J).qreal,2)+pow(CQ1(1,J).qimag,2));
      I=J;
      CQ4(1,J).qreal=CQ1(1,I).qreal+CQ2(2,I).qreal+CQ3(3,J).qreal+
                     CQ4(4,I).qreal+FD1.qreal+FD5.qreal+FD6.qreal;
      CQ4(1,J).qimag=CQ1(1,I).qimag+CQ2(2,I).qimag+CQ3(3,J).qimag+
                     CQ4(4,I).qimag+FD1.qimag+FD5.qimag+FD6.qimag;
   }
   printf(" *** PATTERN2007 ***\n");
   for(j=1;j<=10;j++){
    for(i=1;i<=10;i++){
         printf(" %g %g %g %g %g %g %g %g %g \n",
                  R4(i,j),D4(i,j),(double)Q4(i,j),
                  C4(i,j).real,C4(i,j).imag,CD4(i,j).dreal,CD4(i,j).dimag,
                  (double)CQ4(i,j).qreal,(double)CQ4(i,j).qimag);
    }
   }
exit (0);
  }
