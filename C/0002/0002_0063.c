...
#ifndef ASM_ONLY
void init(float * dest, float * src, float * res, float (*func)(float), float (*func2)(float)) {
  int i;
  for (i = 0; i < N; i++) {
    dest[i] = 0.0F;
    src[i] = (float)i;
    res[i] = func((float)i) + func2((float)i) ;
  }
}
#endif
