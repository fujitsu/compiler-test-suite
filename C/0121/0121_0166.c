...
LBL_10:
	...
	printf(" #####  #####\n");
	printf(" ----- TEST NO.1 ----- TRIPPLY NESTED         \n");
	printf("            VECTORIZABLE BY J (FUTURE ?)      \n");
	for(I=I;I<=L1;I++)
	{
		RC1(I)=-(float)(I);
LBL_41:
...
LBL_420:
	...
	printf("       SHOULD BE VECTORIZABLE BY J (FUTURE ?) \n");
	printf("       BUSY-ON-EXIT SSS NOT VECTORIZABLE ,    \n");
	printf("       WHEN THERE IS A DUMMY OUTER LOOP ?     \n");
	SSS=-15.0;
	for(I=1;I<=L1;I++)
	{
		for(J=1;J<=L2;J++)
...
LBL_520:
	...
	printf("       CONFIRMATION OF TEST NO.2              \n");
	printf("       SHOULD BE VECTORIZABLE BY J (FUTURE ?) \n");
	printf("       BUSY-ON-EXIT SSS NOT VECTORIZABLE ,    \n");
	printf("       WHEN THERE IS A DUMMY OUTER LOOP ?     \n");
	SSS=-15.0;
...
LBL_620:
	...
	printf(" SSS = %g\n",SSS);
	printf(" ----- TEST NO.4 ----- DOUBLY NESTED          \n");
	printf("       CONFIRMATION OF TEST NO.3              \n");
	printf("       SIMPLER FORM OF BUSY-ON-EXIT (AND ALSO \n");
	printf("       BUSY-ON-ENTRANCE ) DATA                \n");
	printf("       NOT VECTORIZABLE ?                     \n");
	SSS=-15.0;
	for(J=1;J<=L1;J++)
	{
		for(K=2;K<=L2;K++)
...
LBL_710:
	...
	printf(" SSS = %g\n",SSS);
	printf("\n");
	printf(" ----- TEST NO.5 ----- NO NEST                \n");
	printf("            CONFIRMATION OF TEST NO.4         \n");
	printf("            SIMPLEST POSSIBLE                 \n");
	printf("            FORM OF BUSY-ON-EXIT (AND ALSO    \n");
	printf("               BUSY-ON-ENTRANCE ) DATA        \n");
	printf("  WHY THIS IS VECTORIZABLE AND NOT TEST NO.4 ?\n");
	SSS=-15.0;
	for(J=1;J<=L1;J++)
	{
		if (RC1(J) > 0.0)
