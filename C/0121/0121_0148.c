...
int main()
{
	...
	for(J=1;J<=N1;J++)
	{
		for(I=1;I<=N2;I++)
		{
			...
			CQC(I,J).qimag=(long double)(1);
LBL_1:
			;
		}
	}
	printf(" *****  *****\n");
	printf(" ----- TEST NO.1 -----  OUTER = S  INNER = V & V\n");
	for(I=1;I<=N1;I++)
	{
		QA(I,1)=sqrt(fabs(QB(I,1)))*QC(I,1);
		for(J=1;J<=10;J++)
		{
			for(K=1;K<=10;K++)
			{
...
LBL_11:
		...
		QC(I,2)=sqrt(fabs(QB(I,2)))+QA(I,2);
		for(L=1;L<=10;L++)
		{
			for(M=1;M<=10;M++)
			{
				if (LB(L,M)==1) RA(L,M) = RB(L,M)*RC(L,M==1);
LBL_12:
				;
			}
		}
LBL_100:
	...
	printf(" ----- TEST NO.2 -----  OUT = S  IN = V(MI) & V(SI)\n");
	for(I=1;I<=N1;I++)
	{
		QA(I,1)=sqrt(fabs(QB(I,1)))*QC(I,1);
		for(J=1;J<=N2;J++)
...
LBL_200:
	...
	printf("  DS1 =  %g\n",DS1);
	printf(" ----- TEST NO.3 -----  OUT = S  IN = V(MI) & S\n");
	for(I=1;I<=N1;I++)
	{
		CQA(I,1).qreal=  sqrt(fabs(CQB(I,1).qreal))*CQC(I,1).qreal
		    - sqrt(fabs(CQB(I,1).qimag))*CQC(I,1).qimag;
		CQA(I,1).qimag=  sqrt(fabs(CQB(I,1).qreal))*CQC(I,1).qimag
		    + sqrt(fabs(CQB(I,1).qimag))*CQC(I,1).qreal;
		for(J=1;J<=10;J++)
		{
			for(K=1;K<=10;K++)
			{
				if (   CA(J,K).real != CB(J,K).real
				    || CA(J,K).imag != CB(J,K).imag)
				{
...
LBL_300:
	...
	printf(" ----- TEST NO.4 -----  OUT = S  IN = V(SI) & S\n");
	for(I=1;I<=N1;I++)
	{
		IS1=0;
		for(J=1;J<=N2;J++)
...
LBL_50:
		...
		for(L=1;L<=N2;L++)
		{
			for(M=1;M<=N3;M++)
			{
				if (LBA(L,M)==1) IHA(L,M) = IHB(L,M)*IHC(M,L);
LBL_51:
				;
			}
		}
LBL_500:
	...
	printf(" ----- TEST NO.6 -----  OUT = S  IN = V(MI) & N\n");
	for(I=1;I<=N1;I++)
	{
