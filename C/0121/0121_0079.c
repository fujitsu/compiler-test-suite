...
int JND249 (double a[], double b[], double c[], long int n);
int bdata (void);
int main (void)
{
	long int i;
...
int JND239 (long int n)
{
	...
	for(j=1;j<=2;j++)
	{
		...
		for(i=1;i<=nx;i++)
		{
			B(i).dreal=B(i).dreal-cy.dreal;
			B(i).dimag=B(i).dimag-cy.dimag;
			cy.dreal=((double)(C(i).dimag) < (double)(B(i).dreal)) ?
			    (double)(C(i).dimag):(double)(B(i).dreal);
			cy.dimag=((double)(C(i).dimag) < (double)(B(i).dreal)) ?
			    (double)(C(i).dimag):(double)(B(i).dreal);
			udata.cx.dreal=udata.cx.dreal+C(i).dreal;
			udata.cx.dimag=udata.cx.dimag+C(i).dimag;
		}
	}
	C(nx).dreal=cy.dreal;
	C(nx).dimag=cy.dimag;
	C(n).dreal=udata.cx.dreal;
	C(n).dimag=udata.cx.dimag;
	return 0;
}
