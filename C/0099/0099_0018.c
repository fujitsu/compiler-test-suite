...
double soft_max(double x,double y)
{
  if( x > y ) return x;
  return y;
}
...
int main()
{
  ...
  *(long long *)&snan = 0x7ff0000000000001LL;
  ...
  for(i=0;i<10;i++) {
    for(j=0;j<10;j++) {
      result1[i][j] = max( test_data[i] , test_data[j] ) ;
      result2[i][j] = max( test_data[j] , test_data[i] ) ;
      answer1[i][j] = soft_max( test_data[i] , test_data[j] ) ;
      answer2[i][j] = soft_max( test_data[j] , test_data[i] ) ;
    }
  }
  for(i=0;i<10;i++) {
    for(j=0;j<10;j++) {
