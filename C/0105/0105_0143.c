...
void sub_abc(double r[NI][NK], double s[NI][NK], volatile int II,  int KK) {
  int i,k;
  for (i=0; i<II; ++i) {
    for (k=0; k<KK; ++k) {
      r[i][k] = s[i][k] * i * k;
...
void sub(double r[NI][NK], double s[NI][NK], volatile int II,  int KK) {
  int i,k;
  for (i=0; i<II; ++i) {
    for (k=0; k<KK; ++k) {
      r[i][k] = s[i][k] * i * k;
...
int sub_cmp(double p[NI][NK], double q[NI][NK], volatile int II,  int KK) {
  int i,k;
  for (i=0; i<II; ++i) {
    for (k=0; k<KK; ++k) {
