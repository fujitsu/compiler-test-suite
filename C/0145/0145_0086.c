...
#pragma omp for collapse(3) private(i,j,k,l,m)
    for (i = 0; i < n1 ; ++i ){
      for (j = 0; j <= n2 ; ++j ){
	for (k = 0; k <= n3 ; ++k ){
	  for (l = 0; l <= n4 ; ++l ){
...
#pragma omp for collapse(3) private(j,k,l,m)
      for (j = 0; j <= n2 ; ++j ){
	for (k = 0; k <= n3 ; ++k ){
	  for (l = 0; l <= n4 ; ++l ){
	    for (m = 0; m < n5 ; ++m ){
...
#pragma omp parallel
      {
#pragma omp for collapse(3) private(k,l,m)
	for (k = 0; k <= n3 ; ++k ){
	  for (l = 0; l <= n4 ; ++l ){
	    for (m = 0; m < n5 ; ++m ){
  ...
  for (i = 0; i < n1 ; ++i ){
	...
	for (l = 0; l <= n4 ; ++l ){
	  for (m = 0; m < n5 ; ++m ){
	    ...
	    if(aa[i][j][k][l][m] != bb[i][j][k][l][m] ||
	       bb[i][j][k][l][m] != cc[i][j][k][l][m] ||
	       cc[i][j][k][l][m] != dd[i][j][k][l][m] )
	      {
		flag = 1;
		goto check_end;
	      }
