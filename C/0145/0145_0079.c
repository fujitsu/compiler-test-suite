...
void work(double (*aa)[MAX_ELM2][MAX_ELM3][MAX_ELM4],unsigned int idx1,unsigned int idx2,unsigned int idx3,unsigned int idx4);
...
#pragma omp parallel num_threads(2)
  {
#pragma omp for collapse(4) ordered private(i,j,k,l) schedule(static,3)
    for(l=1; l<5; l=l+1){
      for(k=0; k<=1; k=k+1){
	for(j=0; j<=2; ++j){
	  for(i=0; i<4; ++i){
