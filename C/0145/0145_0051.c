	 ...
	 int init4 ,int end4, int step4)
{
  ...
  int i,j,k,l,m,n;
  unsigned int j_u,l_u,n_u;
  long int i_i8,k_i8,m_i8;
  unsigned long int i_u8,j_u8,k_u8,l_u8,m_u8,n_u8;
  double res=0;
  unsigned long int flag =0;

  int linit1=ELM_MAX-1;
  
  int lstep1=1;

  int linit2=ELM_MAX-1;
  int lend2 =1;
  int lstep2=1;

  int linit3=ELM_MAX-1;
  int lend3 =1;
  int lstep3=2;

  int linit4=ELM_MAX-1;
  int lend4 =1;
  int lstep4=1;
...
#pragma omp for collapse(4) private(n_u)
      for(j_u  = linit1; j_u  > gend1; j_u  = j_u  - step1){
      for(k_i8 = ginit1; k_i8 >  end1; k_i8 = k_i8 - 2){
      for(l_u8 =  init1; l_u8 >     8; l_u8 = l_u8 - lstep1){
      for(m    =      9; m    >  end1; m    = m    - gstep1){
      for(n_u  =      1; n_u  >     0; n_u  = n_u  - 1){
 ...
 test_end1:;
  ...
  for(    i_i8 =      1; i_i8 >    0; --i_i8){
#pragma omp parallel
    {
#pragma omp for collapse(4)  private(n_u8)
      for(j_u8 =  init2; j_u8 >=lend2; j_u8 = j_u8  - gstep2){ 
      for(k    =      9; k    >=gend2; k    = k     - step2){
      for(l_u  = linit2; l_u  >= end2; l_u  = l_u   - 3){
      for(m_i8 = ginit2; m_i8 >=    1; m_i8 = m_i8  - lstep2){
      for(n_u8 =      1; n_u8 >=    1; n_u8--){
 ...
 test_end2 :;
  ...
  for(     i_u8 =1     ; i_u8 >=    1; i_u8 = i_u8 -1){
#pragma omp parallel
    {
#pragma omp for collapse(4) private(n_u)
      for( j_u  = ginit3; j_u  > lend3; j_u  = j_u   -  step3){
      for( k_i8 = linit3; k_i8  >= end3; k_i8 = k_i8 -  2){
      for( l    =  init3; l    >    1; l    =  l     - gstep3){
      for( m_u8 =      9; m_u8 > gend3; m_u8 = m_u8  - lstep3){
      for( n_u  = 1     ; n_u  >=     1; n_u = n_u   - 1){
...
#pragma omp for collapse(4) private(n)
      ...
      for( k_u8 =      9; k_u8 > lend4; k_u8 = k_u8 - step4){
      for( l_u  = ginit4; l_u  >  end4; l_u  = l_u  - 1){
      for( m_i8 = linit4; m_i8 >=    2; m_i8 = m_i8 - gstep4){
      for( n    =      1; n    >     0; --n){
...
int main(void) {
  ...
  int init1=ELM_MAX-1, end1=1,   step1=2;
  int init2=ELM_MAX-1, end2=1, step2=1;
  int init3=ELM_MAX-1, end3=1, step3=2;
  int init4=ELM_MAX-1, end4=1, step4=1;
