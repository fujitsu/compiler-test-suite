...
void test_extended_abs_r4(float * restrict a, float * restrict b, 
			  float * restrict c, float * restrict d, int n) {
  int i;
  
  for(i=0; i<n; i+=2) {
    d[i] = a[i] * c[i];
    d[i+1] = fabs(a[i+1]) * c[i+1];
  }
  
  return;
}

void test_extended_abs_r8(double * restrict a, double * restrict b, 
...
int main(void) {
  ...
  test_extended_abs_r4(fa, fb, fc, fd, 10);
  test_extended_abs_r8(da, db, dc, dd, 10);
