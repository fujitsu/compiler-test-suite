...
void target_i1(double *restrict dest1, double *restrict dest2,
  ...
  for(i=0;i<N;i+=STRIDE) {
    src1[indx[i]]  = src1[indx[i]];
    src2[indx[i]]  = src2[indx[i]] * src1[indx[i]] + src1[indx[i]];
    dest1[indx[i]] = src1[indx[i]] + src2[indx[i]];
    dest2[indx[i]] = src1[indx[i]] * src2[indx[i]] + dest1[indx[i]];
  }
  for(i=1;i<N;i+=STRIDE) {
    src1[indx[i]]  = src1[i-1];
    dest1[indx[i]] = src1[indx[i]]  + src2[i-1];
    src2[i-1]  = src2[indx[i]] * src1[i-1];
    dest2[indx[i]] = src1[indx[i]]  * src2[indx[i]] + dest1[i-1];
  }
}
