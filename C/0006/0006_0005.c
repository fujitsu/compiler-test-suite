...
void builtin_swpl(svbool_t p1, svbool_t p2,
                  long long int  *restrict x,
                  long long int  *restrict y) {
  int i;
  svbool_t p0;
  p0 = svptrue_b64();

  for(i=0;i<N;i+=VLEN) {
    svint64_t xi = svld1_s64(p0, (const long *)&(x[i]));
    svint64_t yi = svld1_s64(p0, (const long *)&(y[i]));
    p1 = svcmpgt_s64(p0,xi,yi);
    p2 = svorr_b_z(p0, p1, p2);
  }

  for(i=0;i<N;i+=VLEN) {
    svint64_t xi = svld1_s64(p0, (const long *)&(x[i]));
    svst1_s64(p2, (long *)&(c[i]), xi);
  }
}
