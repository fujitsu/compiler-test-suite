...
#define TEST_TYPE double _Complex
struct mem_str {
  unsigned long ar[4];
} sval = {0, 1, 2, 3};
...
int main() {
  int ret = 0;
  ret += do_test_reg0(sval);
  ret += do_test_reg1(sval);
  ret += do_test_reg2(sval);
  ret += do_test_reg3(sval);
  ret += do_test_reg4(sval);
  ret += do_test_reg5(sval);
  if (ret == 0) {
    puts("OK");
  } else {
    printf("NG : %d\n", ret);
