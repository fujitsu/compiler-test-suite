...
#define ALIGNMENT_VALUE 64
#define TEST_TYPE unsigned long long int
...
int main() {
  int ret = 0;
  ret += do_test_reg0(20, sval, 10);
  ret += do_test_reg1(20, sval, 10);
  ret += do_test_reg2(20, sval, 10);
  ret += do_test_reg3(20, sval, 10);
  ret += do_test_reg4(20, sval, 10);
  ret += do_test_reg5(20, sval, 10);
  if (ret == 0) {
    puts("OK");
  } else {
    printf("NG : %d\n", ret);
