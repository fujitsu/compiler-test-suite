	...
	for(JJJ=1;JJJ<=N;JJJ++)
	{
		...
		for(J=1;J<=10;J++)
		{
			...
			for(I=1;I<=10;I++)
			{
				if (I >  J)
				{
					DD1.real=sin(DD2.real)+I;
					DD1.imag=sin(DD2.imag)+I;
				}
		...
		for(J=1;J<=10;J++)
		{
			...
			if (6 >  J)
			{
				ED6.dreal=sin(CD4(4,J).dreal);
				ED6.dimag=sin(CD4(4,J).dimag);
			}
			for(I=1;I<=10;I++)
			{
				ED6.dreal=CD1(1,I).dreal+CD2(2,I).dreal+CD3(3,J).dreal+
				    CD4(4,I).dreal+ED1.dreal+ED2.dreal+ED3.dreal;
				ED6.dimag=CD1(1,I).dimag+CD2(2,I).dimag+CD3(3,J).dimag+
				    CD4(4,I).dimag+ED1.dimag+ED2.dimag+ED3.dimag;
			}
		}
		FD5.qreal=0.0;
		FD5.qimag=0.0;
		...
		for(J=1;J<=10;J++)
		{
			...
			{
				if (6 >  J) printf(" %ld \n",I);
				FD6.qreal=CQ1(1,I).qreal+CQ2(2,I).qreal+CQ3(3,J).qreal+
				    CQ4(4,I).qreal+FD1.qreal+FD2.qreal+FD3.qreal;
				FD6.qimag=CQ1(1,I).qimag+CQ2(2,I).qimag+CQ3(3,J).qimag+
				    CQ4(4,I).qimag+FD1.qimag+FD2.qimag+FD3.qimag;
				for(II=1;II<=10;II++)
				{
					AA1=R1(I,II)+R2(J,J)+pow(((float)((int)(R3(II,J)))),N)+pow(I,N);
LBL_60:
					;
				}
			}
