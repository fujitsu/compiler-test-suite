...
void sub(float _Complex a[], float c)
{
  ...
  if ((crealf(d[0]) == 0.0f && cimagf(d[0]) == 3.0f) &&
      (crealf(d[1]) == 2.0f && cimagf(d[1]) == 3.0f) &&
      (crealf(d[2]) == 4.0f && cimagf(d[2]) == 3.0f) &&
      (crealf(d[3]) == 6.0f && cimagf(d[3]) == 3.0f) &&
      (crealf(d[4]) == 8.0f && cimagf(d[4]) == 3.0f)) {
    printf("OK\n");
  }else{
