...
void __test()
{
    ...
    T t1 = T(1);
    T t2 = T(2);
    assert(std::atomic_compare_exchange_weak_explicit(&at, &t1, T(3),
           std::memory_order_seq_cst, std::memory_order_seq_cst) == true);
    assert(std::atomic_compare_exchange_weak_explicit(&atv, &t2, T(4),
           std::memory_order_seq_cst, std::memory_order_seq_cst) == true);
    assert(t1 == T(1));
    assert(t2 == T(2));
    assert(at == T(3));
    ...
    assert(std::atomic_compare_exchange_weak_explicit(&at, &t1, T(5),
           std::memory_order_seq_cst, std::memory_order_seq_cst) == false);
    assert(std::atomic_compare_exchange_weak_explicit(&atv, &t2, T(6),
           std::memory_order_seq_cst, std::memory_order_seq_cst) == false);
    assert(t1 == T(3));
    assert(t2 == T(4));
    assert(at == T(3));
...
void __test<bool>()
{
    ...
    bool b1 = true;
    bool b2 = true;
    assert(std::atomic_compare_exchange_weak_explicit(&at, &b1, false,
           std::memory_order_seq_cst, std::memory_order_seq_cst) == true);
    assert(std::atomic_compare_exchange_weak_explicit(&atv, &b2, false,
           std::memory_order_seq_cst, std::memory_order_seq_cst) == true);
    assert( b1 == true);
    assert( b2 == true);
    assert(at == false);
    ...
    assert(std::atomic_compare_exchange_weak_explicit(&at, &b1, true,
           std::memory_order_seq_cst, std::memory_order_seq_cst) == false);
    assert(std::atomic_compare_exchange_weak_explicit(&atv, &b2, true,
           std::memory_order_seq_cst, std::memory_order_seq_cst) == false);
    assert(b1 == false);
    assert(b2 == false);
    assert(at == false);
