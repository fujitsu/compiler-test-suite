...
template <class test_t> struct TC {
  ...
  test_t inv_test_r(const test_t *input1, const test_t *input2, int n);
};
...
test_t TC<test_t>::inv_test(const test_t *input1, const test_t *input2, int n) {
  ...
  for (test_t i = 0; i < n; i++) {
    tmp[i] = input1[i] + input2[i];
  }

#pragma clang loop vectorize(disable)
  for (int j = 0; j < n; j++) {
    if (tmp[j] != input1[j] + input2[j]) {
      printf("NG line(%d)\n", __LINE__);
      return 1;
    }
  }
...
template <class test_t>
test_t TC<test_t>::inv_test_r(const test_t *input1, const test_t *input2, int n) {
  test_t *tmp;
  tmp = new test_t[n];

  for (test_t i = 0; i < n; i++) {
    tmp[(int)i] = input1[(int)i] + input2[(int)i];
  }
...
int main(void) {
  ...
  res += r4obj.inv_test_r(r4, r4, 10);
  res += r4obj.inv_test_r(r4, r4, 9);
  res += r8obj.inv_test_r(r8, r8, 10);
  res += r8obj.inv_test_r(r8, r8, 9);
  res += r16obj.inv_test_r(r16, r16, 10);
  res += r16obj.inv_test_r(r16, r16, 9);
