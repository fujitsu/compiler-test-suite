...
template <int s> class Array {
public:
  Array() : size(s) { array = new (std::nothrow) float[size]; }

  Array(float value, int stride) : size(s) {
...
int main() {
  ...
  Array<LEN> a(any, frac), b(any, frac), c(one, frac), d(two, frac),
      e(half, frac);

  Array<LEN> a1, b1, c1, d1, x;
